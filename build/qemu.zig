...
pub fn register(ctx: *h.Ctx) void {
    ...
    _ = h.addScriptTestOpts(ctx, "fallback-census-ratchet-test", "Check C/LLVM function-body fallback census admission coverage does not regress", &.{ "bash", "tools/toolchain/fallback-census.sh", "--check" }, .{ .install = true });
    _ = h.addScriptTestOpts(ctx, "review-goal-status-test", "Check active review goals stay explicit until the code evidence proves completion", &.{ "python3", "tools/toolchain/review-goal-status-test.py" }, .{ .install = false });
    ...
    _ = h.addScriptTestOpts(ctx, "validation-scope-inventory-test", "Check validation docs/code stay scoped as language-validation workloads, not OS deliverables", &.{ "python3", "tools/toolchain/validation-scope-inventory.py" }, .{ .install = false });
