...
pub fn register(ctx: *h.Ctx) void {
    ...
    _ = h.addScriptTestOpts(ctx, "gate-manifest-test", "Check the pilot gate manifest matches registered build tiers", &.{ "python3", "tools/toolchain/gate-manifest-test.py" }, .{ .install = false });
    ...
    _ = h.addScriptTest(ctx, "selfhost-emit-test", "Build, link, and run the Phase-4 self-hosted C-code emitter (selfhost/emit_c.mc): lex->parse->emit->clang->run round-trip", &.{ "bash", "tools/toolchain/selfhost-emit-test.sh", "zig-out/bin/mcc" });
    ...
    _ = h.addScriptTest(ctx, "selfhost-import-test", "Build the mcc2 CLI and run the P5.4 self-hosted multi-module IMPORT gate (selfhost/main.mc loader + parser import_decl + emit_c prototypes): linear import round-trip + diamond dedup -> clang -> run", &.{ "bash", "tools/toolchain/selfhost-import-test.sh", "zig-out/bin/mcc" });
    _ = h.addScriptTest(ctx, "selfhost-generic-test", "Build, link, and run the P5.5 self-hosted GENERICS gate (selfhost parser+sema+emit_c): generic struct + generic fn (comptime T) monomorphized at two scalar types with dedup + wrong-arity reject -> clang -> run", &.{ "bash", "tools/toolchain/selfhost-generic-test.sh", "zig-out/bin/mcc" });
    _ = h.addScriptTest(ctx, "selfhost-array-test", "Build, link, and run the P5.6 self-hosted fixed [N]T ARRAY gate (selfhost parser+sema+emit_c): a [4]u32 local with a positional array literal + element read/write, plus a generic struct with a [4]T field monomorphized at u32 to 'uint32_t data[4]', + wrong-length array-literal reject -> clang -> run", &.{ "bash", "tools/toolchain/selfhost-array-test.sh", "zig-out/bin/mcc" });
    _ = h.addScriptTest(ctx, "selfhost-slice-test", "Build, link, and run the P5.7 self-hosted PROPER SLICE (fat-pointer) gate (selfhost parser+sema+emit_c): []const T lowered to 'typedef struct mc_slice_const_T { const T* ptr; size_t len; }', with .len, element indexing (s.ptr[i]), sub-slicing from array and slice bases, by-value passing, and mem.as_bytes(&arr) -> []const u8 + element-type-mismatch reject -> clang -> run", &.{ "bash", "tools/toolchain/selfhost-slice-test.sh", "zig-out/bin/mcc" });
    _ = h.addScriptTest(ctx, "selfhost-lowlevel-test", "Build, link, and run the P5.8 self-hosted LOW-LEVEL LAYER gate (selfhost parser+sema+emit_c): an 'extern \"C\" fn' prototype, an 'unsafe' block (lowered to a plain brace block), the raw.ptr/raw.load/raw.store<T> intrinsics (cast-through-pointer: raw.ptr<u32> -> (uint32_t*)(e), raw.load -> (*(uint32_t*)(e)), raw.store -> (*(uint32_t*)(e) = (v))), and pointer deref p.* -> (*(p)) -> clang -> run", &.{ "bash", "tools/toolchain/selfhost-lowlevel-test.sh", "zig-out/bin/mcc" });
    _ = h.addScriptTest(ctx, "selfhost-cast-test", "Build, link, and run the P5.9 self-hosted CASTS + sizeof/alignof gate (selfhost parser+sema+emit_c): `expr as TYPE` -> ((<ctype>)(operand)) (widening + narrowing, the latter enabling mixed-width arithmetic s_len as u32 + b), sizeof(TYPE) -> sizeof(<ctype>), alignof(TYPE) -> _Alignof(<ctype>), and sizeof(T) inside a generic fn monomorphized at two scalar types (substitution: sizeof(uint32_t)==4, sizeof(uint64_t)==8) -> clang -> run", &.{ "bash", "tools/toolchain/selfhost-cast-test.sh", "zig-out/bin/mcc" });
    _ = h.addScriptTest(ctx, "selfhost-trait-test", "Build, link, and run the P5.10 self-hosted TRAITS + `*mut dyn` DYNAMIC DISPATCH gate (selfhost parser+sema+emit_c): a `trait` decl -> a TRAIT__vtable fn-pointer typedef + a TRAIT__dyn {data,vtable} fat pointer; an `impl TRAIT for TYPE` -> each method desugared to a free fn TYPE__m (with self->field pointer access) + a void*-self thunk TYPE__m__dyn + a rodata `static const TRAIT__vtable TYPE__TRAIT__vtable`; a `*mut TYPE` arg coerced to a {data,vtable} fat pointer at the call site; and `d.m(args)` dispatched as (d).vtbl->m((d).data, args) -> clang -> run (run()==13)", &.{ "bash", "tools/toolchain/selfhost-trait-test.sh", "zig-out/bin/mcc" });
    ...
    _ = h.addScriptTest(ctx, "selfhost-bitwise-test", "Build, link, and run the self-hosted BITWISE/SHIFT + `unreachable` gate via the mcc2 CLI (selfhost parser+sema+emit_c): infix `& | ^` and `<< >>` with C precedence, a prefix `&x` address-of alongside infix `&`, and an `unreachable;` diverging terminator (-> mc_trap_Unreachable) -> clang -Werror -> run (band/shl_or/shr_and/tower/addr_and/clamp_small all correct)", &.{ "bash", "tools/toolchain/selfhost-bitwise-test.sh", "zig-out/bin/mcc" });

    _ = h.addScriptTest(ctx, "selfhost-addr-test", "Self-hosting CAPSTONE: mcc2 compiles a REAL std module (std/addr.mc) to clang-clean C, then a behavioral round-trip of the two enabling constructs — bool literals `true`/`false` and the builtin address-class model (`PAddr`/`VAddr`/`DmaAddr` word-backed opaque scalars + `phys()` minting + `as`-cast minting) incl. an address-class struct field and a `return .{...}` compound literal -> clang -Werror -> run (ge10/pa_roundtrip/va_roundtrip/built_len all correct)", &.{ "bash", "tools/toolchain/selfhost-addr-test.sh", "zig-out/bin/mcc" });
    _ = h.addScriptTest(ctx, "selfhost-mem-test", "Self-hosting VALUE-OPTIONAL milestone: after adding `?T` value optionals to mcc2's subset (G11 — `?usize` returns, `return null;`, `if let`, `== null`/`!= null`, lowered to the tagged `mc_opt_<T> {present,value}` matching the real backend), mcc2 compiles a SECOND real std module (std/mem.mc, whose byte-search helpers return `?usize`) to clang-clean C, plus a behavioral round-trip of a `?usize`-returning fn consumed via `if let` + `== null`/`!= null` -> clang -Werror -> run (iflet_or_zero/is_absent/is_present all correct)", &.{ "bash", "tools/toolchain/selfhost-mem-test.sh", "zig-out/bin/mcc" });
    _ = h.addScriptTest(ctx, "selfhost-result-test", "Self-hosting RESULT milestone: after adding `Result<T,E>` + `ok(x)`/`err(x)` construction + `switch r { ok(v)=>.., err(e)=>.. }` + `if let ok(v)`/`if let err(e)` + the postfix `expr?` error-propagation operator to mcc2's subset (lowered to the real backend's tagged `mc_result_<T>_<E> { bool is_ok; union { T ok; E err; } payload; }`), mcc2 compiles a Result fixture -> clang -Werror -> run (div_or_switch/div_iflet_ok/div_iflet_err/chain_or all correct); also emits std/hosted_io.mc's full Result machinery (Result<Fd,IoError>/Result<usize,IoError>, ok(.{..})/err(.Variant) ctor args, `?`) to C as a real-module check", &.{ "bash", "tools/toolchain/selfhost-result-test.sh", "zig-out/bin/mcc" });

    _ = h.addScriptTest(ctx, "selfhost-lexself-test", "Self-hosting LANDMARK: after adding CHARACTER LITERALS (`'a'`/`'\\n'`/`'\\\\'`/`'0'` -> a `char_literal` primary typed `u8`, emitted as a C char literal) and MODULE-LEVEL `const` (`const NAME: T = <const-expr>;`/`export const` -> a file-scope `static const`) to mcc2's subset, mcc2 compiles its OWN lexer selfhost/lexer.mc (plus its std deps mem/ascii/addr/alloc/dynarray — which also drove `const fn`, `move struct`, bool `switch`, `uninit`, string literals, and transitive generic-fn monomorphization) to clang-clean C (clang -std=gnu11 -c), plus a char-literal + module-const behavioral fixture -> clang -Werror -> run (classify/stride_of/newline_code/tab_byte all correct)", &.{ "bash", "tools/toolchain/selfhost-lexself-test.sh", "zig-out/bin/mcc" });

    _ = h.addScriptTest(ctx, "selfhost-parseself-test", "Self-hosting LANDMARK: after adding STRUCT-type-argument monomorphization (a generic container/function instantiated at a NAMED struct type — `Vec<Node>`, `vec_push(Node, ..)`, `vec_get(Node, ..)` — not just scalars) plus dependency-ordered struct emission (a struct embedding another BY VALUE, e.g. `Parser { tl: TokenList, nodes: Vec<Node> }`, is emitted AFTER what it embeds), mcc2 compiles its OWN parser selfhost/parser.mc (its index-arena AST + std/lexer deps) to clang-clean C (clang -std=gnu11 -c), plus a Vec<Pt> behavioral fixture (generic container over a STRUCT element: push/get struct values + read a field) -> clang -Werror -> run", &.{ "bash", "tools/toolchain/selfhost-parseself-test.sh", "zig-out/bin/mcc" });

    _ = h.addScriptTest(ctx, "selfhost-semaself-test", "Self-hosting LANDMARK (3rd module): after adding comma-less `}`-block switch arms, a `>>`-splitting generic close (nested `raw.ptr<Entry<V>>`), a visited-set in the transitive generic-instance collector (the lexeme-only type-param match otherwise blows up exponentially on hashmap's `V`-named generics + call cycle), `Entry<struct>` typedef induction from the generic fns that use it, `mem.bytes_equal` builtin lowering, module-qualified `mod.fn` calls, and `_` digit-separator stripping, mcc2 compiles its OWN sema selfhost/sema.mc (which instantiates `Vec`/`StrHashMap` over its own structs, incl. `StrHashMap<SmType>`/`Entry<SmType>`) to clang-clean C (clang -std=gnu11 -c), plus a comma-less block switch + `StrHashMap<Rec>` (hash map over a STRUCT value: put/get_or + absent-key fallback) behavioral fixture -> clang -Werror -> run", &.{ "bash", "tools/toolchain/selfhost-semaself-test.sh", "zig-out/bin/mcc" });

    _ = h.addScriptTest(ctx, "selfhost-emitself-test", "Self-hosting LANDMARK (4th module): after adding PREFIX pointer deref `*p` (C-style, distinct from postfix `p.*`, incl. the `&*p` re-borrow that passes a `*mut Vec<u32>` where a `*Vec<u32>` is wanted — `e_arg_present(p, &*out, ..)`), string-literal-to-`*const u8`-parameter coercion (string literals coerce to BOTH `[]const u8` and `*const u8` in MC (G12): sema accepts the arg leniently, the emitter lowers it to a bare C string, not a `[]const u8` fat-pointer slice — `sb_put_cstr(sb, \"...\")`), and renaming `ok`/`err`-shaped locals (Result-constructor keywords), mcc2 compiles its OWN emitter selfhost/emit_c.mc (its LARGEST module, ~3.4k lines: the monomorphizer + StrBuf C emitter) to clang-clean C (clang -std=gnu11 -c), plus a prefix-deref (`*p` read/write + `&*` re-borrow) + string-literal-to-`*const u8` behavioral fixture -> clang -Werror -> run", &.{ "bash", "tools/toolchain/selfhost-emitself-test.sh", "zig-out/bin/mcc" });

    _ = h.addScriptTest(ctx, "selfhost-mainself-test", "Self-hosting LANDMARK (5th and FINAL core module): after adding module-level `global NAME: T [= init];` — a MUTABLE file-scope variable (parser `global_decl`; sema registers it as a mutable global so uses resolve and whole-variable/`[i]`/`.f` writes + address-of `(&g) as usize` are allowed; emitter lowers `static T NAME [= init];`, NOT a `static const`) — plus an emitter fix so a chained sub-slice/index of a `mem.as_bytes(&g)` slice VALUE (`as_bytes(&g)[a..b]` / `[i]`) reads its `.ptr` fat-pointer field instead of mis-lowering as an array decay, mcc2 compiles its OWN CLI driver selfhost/main.mc (the standalone `mcc2` command: the whole lexer->parser->sema->emit_c front end + a textual-concatenation import loader + a hosted-I/O file reader, exporting `mc_main`) to clang-clean C (clang -std=gnu11 -c) — every one of the five mcc2 source modules now self-compiles — plus a module-level `global` (scalar-with-init, array, whole-variable + `[i]` writes + address-of) behavioral fixture -> clang -Werror -> run", &.{ "bash", "tools/toolchain/selfhost-mainself-test.sh", "zig-out/bin/mcc" });

    _ = h.addScriptTest(ctx, "selfhost-bootstrap-test", "Self-hosting CAPSTONE: the closed bootstrap loop. Where the five selfhost-*self-test gates each prove ONE mcc2 module self-compiles, this proves the whole thing — the Zig `mcc` builds first-gen mcc2, then mcc2 compiles its OWN full source (main + emit_c + sema + parser + lexer + all std deps, flattened by the concat loader into one C TU) with ZERO diagnostics, that C links into second-gen mcc2', mcc2 and mcc2' emit BYTE-IDENTICAL C for a program (the fixpoint), and mcc2''s output clang-compiles + runs correctly (add(2,3)==5). The definitive proof that mcc2 compiles mcc2.", &.{ "bash", "tools/toolchain/selfhost-bootstrap-test.sh", "zig-out/bin/mcc" });
