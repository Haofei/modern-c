...
pub fn register(ctx: *h.Ctx) void {
    ...
    m0_step.dependOn(ctx.cmd("wasm-biggrow-test"));
    m0_step.dependOn(ctx.cmd("llvm-wasm-biggrow-test"));
    m0_step.dependOn(ctx.cmd("mem-test"));
    m0_step.dependOn(ctx.cmd("llvm-mem-test"));
    m0_step.dependOn(ctx.cmd("sbrk-grow-test"));
    m0_step.dependOn(ctx.cmd("llvm-sbrk-grow-test"));
    m0_step.dependOn(ctx.cmd("sbrk-cap-test"));
    m0_step.dependOn(ctx.cmd("llvm-sbrk-cap-test"));
    ...
    m0_step.dependOn(ctx.cmd("selfhost-emitself-test"));
    ...
    m0_step.dependOn(ctx.cmd("selfhost-mainself-test"));
    // selfhost-bootstrap-test (SELF-HOSTING CAPSTONE): the closed bootstrap loop — the Zig `mcc` builds first-gen mcc2, mcc2 compiles its OWN full source (main + emit_c + sema + parser + lexer + all std deps, concat-loaded into one C TU) diagnostic-clean, that C links into second-gen mcc2', mcc2 and mcc2' emit BYTE-IDENTICAL C (the fixpoint), and mcc2''s output compiles+runs (add(2,3)==5). The definitive proof that mcc2 compiles mcc2.
    m0_step.dependOn(ctx.cmd("selfhost-bootstrap-test"));
