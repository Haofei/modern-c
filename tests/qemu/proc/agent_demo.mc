...
//     dispatch budget, is NOT dispatched, and is audited as a denial;
...
//   * AUDIT: every Allow/Deny verdict is recorded into the capability-use trace (caller pid + tool id
//     or rejection kind).
//
...
export fn agent_run() -> u32 {
    ...
    switch tool_register(&g_reg, 1, tool_secret) {
        ok(s) => { pass = 0; }
        err(e) => { if e != .Duplicate { pass = 0; } }
    }
    ...
    // (7) AUDIT: every verdict is recorded. Allows carry tag=tool id and size=0; rejected
    // requests carry a stable rejection tag and the raw requested tool id in size.
    let aud: *mut IpcTrace = cap_audit();
    if ipc_trace_len(aud) != 6 { pass = 0; }
    let expect_to: [6]u32 = .{ TOOL_AUDIT_ALLOW, TOOL_AUDIT_ALLOW, TOOL_AUDIT_DENY, TOOL_AUDIT_DENY, TOOL_AUDIT_ALLOW, TOOL_AUDIT_DENY };
    let expect_tags: [6]u32 = .{ 1, 2, TOOL_AUDIT_DENIED_TAG, TOOL_AUDIT_NO_SUCH_TAG, 1, TOOL_AUDIT_EXHAUSTED_TAG };
    let expect_size: [6]u32 = .{ 0, 0, 9, 5, 0, 2 };
    var i: usize = 0;
    while i < 6 {
        switch ipc_trace_drain(aud) {
            ok(ev) => {
                if ev.from != agent_pid { pass = 0; }       // caller = the agent
                if ev.to != expect_to[i] { pass = 0; }
                if ev.tag != expect_tags[i] { pass = 0; }
                if ev.size != expect_size[i] { pass = 0; }
            }
            err(e) => { pass = 0; }
        }
        i = i + 1;
    }
    if ipc_trace_len(aud) != 0 { pass = 0; } // drained dry
