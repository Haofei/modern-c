// Bare-metal riscv64 M-mode timer/trap wiring for the E6 multi-future async/await demo
// (tests/qemu/proc/async_multi_demo.mc), in PURE MC. The context-switch primitive, UART
...
//
// Like the single-future async-future demo, the timer is RE-ARMED by the ISR (`multi_on_timer`)
// after each completion, so a sequence of in-flight requests each get their own completion
// interrupt — drive_many sleeps in `wfi` between them, interleaving the three futures.
...
// MC entry points (tests/qemu/proc/async_multi_demo.mc).
extern fn multi_on_timer() -> void;
extern fn async_multi_demo(region_base: usize, region_len: usize) -> u32;
...
export fn trap_entry() -> void {
    ...
    if mcause == RT_MCAUSE_M_TIMER {
        disarm_timer();     // clear MTIE; multi_on_timer re-arms if there is more in flight
        multi_on_timer();   // complete the in-flight request (IRQ-safe), re-arm for the next
        return;
    }
    mc_halt();
}
...
export fn test_main() -> void {
    puts_("async-multi booting\n");
    let r: u32 = async_multi_demo(0, 0);
    puts_("\nresult=");
    putc_((48 + (r % 10)) as u8);
    putc_(10); // '\n'
    if r == 1 {
        puts_("ASYNC-MULTI-OK\n");
    } else {
        puts_("ASYNC-MULTI-FAIL\n");
    }
    mc_halt();
}
