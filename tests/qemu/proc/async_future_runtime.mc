...
// (tests/qemu/proc/async_future_demo.mc), in PURE MC. The context-switch primitive, UART
...
// (`future_on_timer`) after each completion, so a sequence of `await`s each get their own
...
// MC entry points (tests/qemu/proc/async_future_demo.mc).
extern fn future_on_timer() -> void;
extern fn async_future_demo(region_base: usize, region_len: usize) -> u32;
...
export fn trap_entry() -> void {
    ...
    if mcause == RT_MCAUSE_M_TIMER {
        disarm_timer();     // clear MTIE; future_on_timer re-arms if there is more in flight
        future_on_timer();  // complete the in-flight request (IRQ-safe), re-arm for the next
        return;
    }
    mc_halt();
}
...
export fn test_main() -> void {
    puts_("async-future booting\n");
    let r: u32 = async_future_demo(0, 0);
    puts_("\nresult=");
    putc_((48 + ((r / 10) % 10)) as u8);
    putc_((48 + (r % 10)) as u8);
    putc_(10); // '\n'
    if r == 42 {
        puts_("ASYNC-FUTURE-OK\n");
    } else {
        puts_("ASYNC-FUTURE-FAIL\n");
    }
    mc_halt();
}
