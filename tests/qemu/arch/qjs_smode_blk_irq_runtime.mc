// S-mode/OpenSBI QuickJS runtime with an IRQ-backed virtio-blk host_fs_read tool.
...
fn probe_virtio_blk() -> usize {
    ...
    while i < VIRTIO_MMIO_COUNT {
        ...
        if magic == VIRTIO_MMIO_MAGIC && devid == VIRTIO_ID_BLK {
            return slot;
        }
        i = i + 1;
    }
...
export fn s_entry() -> void {
    sbi_puts("kernel up in S-mode under OpenSBI: loading confined QuickJS IRQ-blk agent\n");
    usermode_setup();

    let regs: usize = probe_virtio_blk();
    if regs == 0 {
        sbi_puts("NODEV\n");
        sbi_shutdown();
    }
    app_blk_irq_config(regs);
    ...
    sbi_puts("kernel: entering confined QuickJS IRQ-blk agent\n");
