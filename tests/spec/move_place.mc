...
fn accept_reinitialize_array_element() -> u32 {
    ...
    arr[0] = mkres(y + 1);
    let a: Res = move arr[0];
    let b: Res = move arr[1];
    unsafe { forget_unchecked(arr); }
    return consume(move a) + consume(move b);
}
...
fn reject_defer_block_cleanup_local_captured_aggregate_call_result_after_consume() -> u32 {
    defer {
        ...
        consume(move r); // EXPECT_ERROR: E_USE_AFTER_MOVE
        peek_holder(h);
    };
    return 0;
}
...
fn accept_deferred_slice_bound_consume() -> []mut u8 {
    ...
    let r: Res = mkres(1);
    defer buf[consume_index(move r)..1];
    return buf[0..1];
}
...
fn reject_deferred_short_circuit_rhs_consume(flag: bool) -> u32 {
    let r: Res = mkres(1);
    defer flag && (consume(move r) != 0); // EXPECT_ERROR: E_MOVE_BRANCH_MISMATCH
    return 0;
}
...
fn reject_defer_dynamic_pointer_to_move_array_element(pa: *mut ResArray, i: usize) -> void {
    defer consume(move pa.*[i]); // EXPECT_ERROR: E_MOVE_ARRAY_UNSUPPORTED
}
...
fn reject_defer_dynamic_pointer_to_move_matrix_element(pm: *mut ResMatrix, i: usize) -> void {
    defer consume(move pm.*[i][0]); // EXPECT_ERROR: E_MOVE_ARRAY_UNSUPPORTED
}
...
fn accept_reinitialize_array_field_element() -> u32 {
    ...
    let a: Res = move box.items[0];
    let b: Res = move box.items[1];
    unsafe { forget_unchecked(box); }
    return consume(move a) + consume(move b);
}
...
fn accept_laundered_subplace_pointer_used_before_move() -> u32 {
    ...
    let y: Res = move p.b;
    unsafe { forget_unchecked(p); }
    return v + consume(move x) + consume(move y);
}
...
fn reject_branch_partial_field_move(cond: bool) -> u32 {
    let p: Pair = mk();
    if cond {
        let x: Res = move p.a; // EXPECT_ERROR: E_MOVE_BRANCH_MISMATCH
        let y: u32 = consume(move x);
    }
    unsafe { forget_unchecked(p); }
    return 0;
...
fn reject_branch_array_element_move(cond: bool) -> u32 {
    let arr: [2]Res = .{ mkres(1), mkres(2) };
    if cond {
        let x: Res = move arr[0]; // EXPECT_ERROR: E_MOVE_BRANCH_MISMATCH
        let y: u32 = consume(move x);
    }
    unsafe { forget_unchecked(arr); }
    return 0;
...
fn reject_branch_array_field_element_move(cond: bool) -> u32 {
    let box: ResArrayBox = mkbox();
    if cond {
        let x: Res = move box.items[0]; // EXPECT_ERROR: E_MOVE_BRANCH_MISMATCH
        let y: u32 = consume(move x);
    }
    unsafe { forget_unchecked(box); }
    return 0;
...
fn reject_short_circuit_partial_field_move(flag: bool) -> u32 {
    let p: Pair = mk();
    if flag && consume(move p.a) != 0 { // EXPECT_ERROR: E_MOVE_BRANCH_MISMATCH
        return 1;
    }
    return 0;
}
...
fn reject_short_circuit_array_element_move(flag: bool) -> u32 {
    let arr: ResArray = .{ mkres(1), mkres(2) };
    if flag && consume(move arr[0]) != 0 { // EXPECT_ERROR: E_MOVE_BRANCH_MISMATCH
        unsafe { forget_unchecked(arr); } // EXPECT_ERROR: E_USE_AFTER_MOVE
        return 1;
    }
    unsafe { forget_unchecked(arr); } // EXPECT_ERROR: E_USE_AFTER_MOVE
...
fn reject_short_circuit_array_field_element_move(flag: bool) -> u32 {
    let box: ResArrayBox = mkbox();
    if flag && consume(move box.items[0]) != 0 { // EXPECT_ERROR: E_MOVE_BRANCH_MISMATCH
        unsafe { forget_unchecked(box); } // EXPECT_ERROR: E_USE_AFTER_MOVE
        return 1;
    }
    unsafe { forget_unchecked(box); } // EXPECT_ERROR: E_USE_AFTER_MOVE
...
fn reject_loop_partial_field_move(flag: bool) -> u32 {
    let p: Pair = mk();
    while flag {
        let x: Res = move p.a; // EXPECT_ERROR: E_MOVE_LOOP_RESOURCE
        let y: u32 = consume(move x);
    }
    return 0;
}
...
fn reject_loop_array_element_move(flag: bool) -> u32 {
    let arr: [2]Res = .{ mkres(1), mkres(2) };
    while flag {
        let x: Res = move arr[0]; // EXPECT_ERROR: E_MOVE_LOOP_RESOURCE
        let y: u32 = consume(move x);
    }
    return 0;
}
...
fn reject_break_after_array_field_element_move(flag: bool) -> u32 {
    let box: ResArrayBox = mkbox();
    while flag {
        let x: Res = move box.items[0]; // EXPECT_ERROR: E_MOVE_LOOP_RESOURCE
        let y: u32 = consume(move x);
        break;
    }
    unsafe { forget_unchecked(box); }
...
fn reject_while_condition_partial_field_move() -> u32 {
    let p: Pair = mk();
    while consume(move p.a) != 0 { // EXPECT_ERROR: E_MOVE_LOOP_RESOURCE
    }
    return 0;
}
...
fn reject_while_condition_array_element_move() -> u32 {
    let arr: ResArray = .{ mkres(1), mkres(2) };
    while consume(move arr[0]) != 0 { // EXPECT_ERROR: E_MOVE_LOOP_RESOURCE
    }
    unsafe { forget_unchecked(arr); } // EXPECT_ERROR: E_USE_AFTER_MOVE
    return 0;
}
...
fn reject_while_condition_array_field_element_move() -> u32 {
    let box: ResArrayBox = mkbox();
    while consume(move box.items[0]) != 0 { // EXPECT_ERROR: E_MOVE_LOOP_RESOURCE
    }
    unsafe { forget_unchecked(box); } // EXPECT_ERROR: E_USE_AFTER_MOVE
    return 0;
}
