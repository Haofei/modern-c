...
import "kernel/core/uaccess_aarch64.mc";
import "std/addr.mc";
...
fn qa_sys_submit(req_ptr: u64, b: u64, c: u64) -> u64 {
    ...
    switch copy_from_user_pt(&qa_uas, pa((&req) as usize), qa_uptr(req_ptr as usize), rsz) {
        ok(v) => {}
        err(e) => { return bitcast<u64>(E_FAULT); }
    }
...
fn qa_sys_poll(ev_ptr: u64, b: u64, c: u64) -> u64 {
    qa_clock = qa_clock + 1;
    ...
    switch copy_to_user_pt(&qa_uas, qa_uptr(ev_ptr as usize), pa((&ev) as usize), esz) {
        ok(v) => {}
        err(e) => { return bitcast<u64>(E_FAULT); }
    }
