...
// Round-robin + static priority let a heavy process monopolize the CPU. Fair-share picks the
// *least-served* runnable slot — fewest ticks consumed this incarnation — so no process runs
// again until others catch up, bounding CPU per process. The per-slot `ticks` accumulator and
...
// throttled process compares as if it had consumed (ticks + penalty) ticks, pushing it to the back
...
// reuse (a reaped slot must not inherit the old process's throttle).

// A slot's effective fair-share cost: real ticks consumed plus any throttle penalty, weighted
// down by priority so a higher-priority process is allowed a larger share before deprioritizing.
// We compare ticks/max(priority,1): with equal priority this is pure least-served-first; a
// priority-2 process may consume twice the ticks of a priority-1 process before losing its turn.
...
// a misbehaving process back to the front. The process stays runnable (not killed/blocked) — this
...
// the wake primitive an ISR-style completion path reaches.
#[irq_context]
