...
pub fn ipv4_from_u32(raw: u32) -> Ipv4Addr {
    return .{ .raw = raw };
}

pub fn ipv4_eq(x: *Ipv4Addr, y: *Ipv4Addr) -> bool {
    return x.raw == y.raw;
}
...
pub fn cursor(buf: *CpuBuffer, base: usize) -> PacketCursor {
    return .{ .base = base, .limit = cpu_len(buf) };
}

// Does the frame hold at least `n` bytes past this cursor's base?
pub fn has(c: *PacketCursor, n: usize) -> bool {
    return (c.base + n) <= c.limit;
}

// A sub-cursor `offset` bytes further in (e.g. the IPv4 header within a frame).
pub fn at(c: *PacketCursor, offset: usize) -> PacketCursor {
    return .{ .base = c.base + offset, .limit = c.limit };
}

pub fn put_u8(c: *PacketCursor, buf: *CpuBuffer, off: usize, v: u8) -> void {
    write_u8(buf, c.base + off, v);
}
pub fn put_be16(c: *PacketCursor, buf: *CpuBuffer, off: usize, v: u16) -> void {
    write_be16(buf, c.base + off, v);
}
pub fn put_be32(c: *PacketCursor, buf: *CpuBuffer, off: usize, v: u32) -> void {
    write_be32(buf, c.base + off, v);
}
pub fn get_u8(c: *PacketCursor, buf: *CpuBuffer, off: usize) -> u8 {
    return read_u8(buf, c.base + off);
}
pub fn get_be16(c: *PacketCursor, buf: *CpuBuffer, off: usize) -> u16 {
    return read_be16(buf, c.base + off);
}
pub fn get_be32(c: *PacketCursor, buf: *CpuBuffer, off: usize) -> u32 {
