...
export fn mapping_is_executable(m: *LeafMapping) -> bool { return (m.flags & ATTR_UXN) == 0; }
...
// R/W/X intent into stage-1 leaf attributes. Executable user pages are always
// read-only and PXN; non-executable pages preserve the requested read-only or
// read-write AP encoding and set both PXN and UXN.
export fn pte_flags_for_user(r: bool, w: bool, x: bool) -> u64 {
    if x {
        return ATTR_AP_URO | ATTR_PXN;
    }
    if w {
        return ATTR_AP_URW | ATTR_PXN | ATTR_UXN;
    }
    return ATTR_AP_URO | ATTR_PXN | ATTR_UXN;
}
