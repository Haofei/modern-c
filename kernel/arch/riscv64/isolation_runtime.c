...
// table + page tables (MC isolation_setup), drops to S-mode, activates the kernel
...
void     isolation_setup(uintptr_t region, uintptr_t len);
uint64_t isolation_kernel_satp(void);
uint32_t isolation_run(void);

__attribute__((aligned(4096))) static uint8_t heap_region[512 * 1024];

__attribute__((used)) void s_main(void) {
    uint64_t ks = isolation_kernel_satp();
    __asm__ volatile("csrw satp, %0\n sfence.vma\n" ::"r"(ks) : "memory");
    if (isolation_run() == 1) puts_("ISO-OK\n");
    else puts_("ISO-BAD\n");
    *FINISHER = 0x5555;
    for (;;) {}
}
...
__attribute__((used)) void m_main(void) {
    puts_("isolation booting (M-mode)\n");
    isolation_setup((uintptr_t)heap_region, (uintptr_t)sizeof(heap_region));
