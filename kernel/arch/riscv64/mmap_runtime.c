...
uint64_t mmap_demo(uintptr_t region, uintptr_t len); // MC: build table -> satp

__attribute__((aligned(4096))) static uint8_t heap_region[256 * 1024];
static uint64_t g_satp;

#define VA1 0xC0000000UL       // first anonymous page
#define VA2 0xC0001000UL       // second anonymous page

// S-mode: turn on paging, then write + read both mmap'd anonymous pages and confirm
// they are independent demand-allocated memory (distinct values round-trip).
__attribute__((used)) void s_main(void) {
    __asm__ volatile("csrw satp, %0\n sfence.vma\n" ::"r"(g_satp) : "memory");
    volatile uint32_t *p1 = (volatile uint32_t *)VA1;
    volatile uint32_t *p2 = (volatile uint32_t *)VA2;
    *p1 = 0xAAAA1111u;
    *p2 = 0xBBBB2222u;
    puts_("MMAP p1="); puthex(*p1); puts_(" p2="); puthex(*p2); putc_('\n');
    if (*p1 == 0xAAAA1111u && *p2 == 0xBBBB2222u) puts_("MMAP-OK\n");
    else puts_("MMAP-BAD\n");
    *FINISHER = 0x5555;
    for (;;) {}
}
...
__attribute__((used)) void m_main(void) {
    puts_("mmap booting (M-mode)\n");
    g_satp = mmap_demo((uintptr_t)heap_region, (uintptr_t)sizeof(heap_region));
    puts_("mmap: table built, dropping to S-mode\n");
