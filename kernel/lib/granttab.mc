...
export fn grant_table_delegate(tab: *mut GrantTable, parent_id: usize, new_owner_slot: u32, new_owner_gen: u32, base: PAddr, len: usize) -> Result<usize, GrantTabError> {
...
export fn grant_table_revoke_cascade(tab: *mut GrantTable, id: usize) -> Result<usize, GrantTabError> {
    if id >= GRANTTAB_MAX {
        return err(.BadId);
    }
    if !tab.slots[id].present {
