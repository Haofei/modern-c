...
pub fn mailbox_init(comptime T: type, comptime N: usize, m: *mut Mailbox<T, N>) -> void {
    m.head = 0;
    m.count = 0;
}

pub fn mailbox_count(comptime T: type, comptime N: usize, m: *mut Mailbox<T, N>) -> usize {
    return m.count;
}
pub fn mailbox_is_full(comptime T: type, comptime N: usize, m: *mut Mailbox<T, N>) -> bool {
    return m.count == N;
}
...
pub fn mailbox_post(comptime T: type, comptime N: usize, m: *mut Mailbox<T, N>, message: T, src: u32) -> bool {
    if m.count == N {
        return false; // full
    }
...
pub fn mailbox_take(comptime T: type, comptime N: usize, m: *mut Mailbox<T, N>, out: *mut T) -> bool {
    if m.count == 0 {
        return false; // empty
    }
    out.* = m.msg[m.head];
...
pub fn mailbox_take_from(comptime T: type, comptime N: usize, m: *mut Mailbox<T, N>, src: u32, out: *mut T) -> bool {
    var k: usize = 0;
    while k < m.count {
        let pos: usize = (m.head + k) % N;
        if m.src[pos] == src {
...
pub fn mailbox_take_if(comptime T: type, comptime N: usize, m: *mut Mailbox<T, N>, keep: closure(*T) -> bool, out: *mut T) -> bool {
