...
#
# Usage: tools/paging-activate-test.sh <path-to-mcc>
...
RUNTIME="$HERE/kernel/arch/riscv64/contain_runtime.c"
...
skip() { echo "SKIP: paging-activate-test ($1)"; exit 0; }
...
if printf '%s' "$OUT" | grep -q "CONTAINED-OK"; then
    echo "PASS: contain-test — MMU crash containment: a server dereferenced an unmapped address; the fault handler contained it (redirected to recovery, no panic) and the system continued (CONTAINED-OK) under QEMU"
    exit 0
fi
echo "FAIL: paging-activate-test — expected 'CONTAINED-OK' in kernel output"
exit 1
