...
#
# Usage: tools/backtrace-test.sh <path-to-mcc>
# Skips (exit 0) when the riscv toolchain or QEMU is unavailable.
set -euo pipefail

MCC="${1:-zig-out/bin/mcc}"
HERE="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
SRC="$HERE/tests/qemu/symbols_demo.mc"
...
skip() { echo "SKIP: backtrace-test ($1)"; exit 0; }
...
if printf '%s' "$OUT" | grep -q "BT-OK"; then
    echo "PASS: backtrace-test — walked the frame-pointer chain and symbolized the captured frames under QEMU"
    exit 0
fi
echo "FAIL: backtrace-test — expected 'BT-OK' in kernel output"
exit 1
