...
SRC="$HERE/tests/qemu/tcp_server_demo.mc"
...
if printf '%s' "$OUT" | grep -q "TCPSRV-OK" \
   && printf '%s' "$OUT" | grep -q "TCPSRV-OK"; then
    echo "PASS: tcp-server-test — TCP as a user-mode server: a client drove a passive-open handshake (LISTEN/SYN/ACK) over IPC and the connection reached ESTABLISHED (TCPSRV-OK) under QEMU"
    exit 0
fi
echo "FAIL: process-test — expected TCPSRV-OK and TCPSRV-OK in kernel output"
exit 1
