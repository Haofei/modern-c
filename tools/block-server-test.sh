...
if printf '%s' "$OUT" | grep -q "BLK-SERVER-OK" \
   && printf '%s' "$OUT" | grep -q "BLK-SERVER-OK"; then
    echo "PASS: block-server-test — storage driver as a user-mode server: client write+read of a 512B block round-tripped through IPC to the disk-capability server (BLK-SERVER-OK) under QEMU"
    exit 0
fi
echo "FAIL: process-test — expected BLK-SERVER-OK and BLK-SERVER-OK in kernel output"
exit 1
