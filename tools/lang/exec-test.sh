...
#
# Usage: tools/lang/exec-test.sh <path-to-mcc>
...
skip() { echo "SKIP: exec-test ($1)"; exit 0; }
...
if printf '%s' "$OUT" | grep -q "AB" \
   && printf '%s' "$OUT" | grep -q "USER-EXIT from U"; then
    echo "PASS: exec-test — a U-mode program exec'd a second ELF; the kernel loaded + ran it (AB) under QEMU"
    exit 0
fi
echo "FAIL: exec-test — expected AB and 'USER-EXIT from U' in kernel output"
exit 1
