...
#
# Usage: tools/driver-test.sh <path-to-mcc>
# Skips (exit 0) when the riscv toolchain or QEMU is unavailable.
set -euo pipefail

MCC="${1:-zig-out/bin/mcc}"
HERE="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
SRC="$HERE/tests/qemu/driver_demo.mc"
RUNTIME="$HERE/kernel/arch/riscv64/driver_runtime.c"
...
skip() { echo "SKIP: driver-test ($1)"; exit 0; }
...
if printf '%s' "$OUT" | grep -q "DRV" \
   && printf '%s' "$OUT" | grep -q "DRIVER-OK"; then
    echo "PASS: driver-test — char-device driver framework dispatched putc through a function-pointer vtable (DRV) under QEMU"
    exit 0
fi
echo "FAIL: driver-test — expected DRV and DRIVER-OK in kernel output"
exit 1
