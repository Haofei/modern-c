...
#
# Usage: tools/process-test.sh <path-to-mcc>
# Skips (exit 0) when the riscv toolchain or QEMU is unavailable.
set -euo pipefail

MCC="${1:-zig-out/bin/mcc}"
HERE="$(d=$(CDPATH= cd -- "$(dirname -- "${BASH_SOURCE[0]}")" && pwd); while [ "$d" != / ] && [ ! -e "$d/build.zig" ]; do d=$(dirname "$d"); done; printf %s "$d")"
SRC="$HERE/tests/qemu/net/net_server_demo.mc"
RUNTIME="$HERE/kernel/arch/riscv64/net_server_runtime.c"
...
if printf '%s' "$OUT" | grep -q "NET-SERVER-OK" \
   && printf '%s' "$OUT" | grep -q "NET-SERVER-OK"; then
    echo "PASS: net-server-test — UDP socket layer as a user-mode server: a client bound a port, injected + received a datagram across the IPC boundary (NET-SERVER-OK) under QEMU"
    exit 0
fi
echo "FAIL: process-test — expected NET-SERVER-OK and NET-SERVER-OK in kernel output"
exit 1
