...
#
# Usage: tools/vmspace-test.sh <path-to-mcc>
# Skips (exit 0) when the riscv toolchain or QEMU is unavailable.
set -euo pipefail

MCC="${1:-zig-out/bin/mcc}"
HERE="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
SRC="$HERE/tests/qemu/vmspace_demo.mc"
...
skip() { echo "SKIP: vmspace-test ($1)"; exit 0; }
...
if printf '%s' "$OUT" | grep -q "VMSPACE-OK"; then
    echo "PASS: vmspace-test — gave each process its own page table (satp); each saw its own value at the same VA under QEMU"
    exit 0
fi
echo "FAIL: vmspace-test — expected 'VMSPACE-OK' in kernel output"
exit 1
