...
#
# Usage: tools/user-test.sh <path-to-mcc>
# Skips (exit 0) when the riscv toolchain or QEMU is unavailable.
set -euo pipefail

MCC="${1:-zig-out/bin/mcc}"
HERE="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
SRC="$HERE/tests/qemu/syscall_demo.mc"
RUNTIME="$HERE/kernel/arch/riscv64/user_runtime.c"
...
if printf '%s' "$OUT" | grep -q "USR" \
   ...
   && printf '%s' "$OUT" | grep -q "USER-EXIT from U"; then
    echo "PASS: user-test — U-mode task: syscalls (USR), validated copy_from_user (FROMUSER + rejected bad ptr), exit from U under QEMU"
    exit 0
fi
echo "FAIL: user-test — expected USR, FROMUSER, R, and 'USER-EXIT from U' in kernel output"
exit 1
