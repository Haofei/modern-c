...
SRC="$HERE/tests/qemu/proc/async_irq_demo.mc"
RUNTIME="$HERE/tests/qemu/proc/async_irq_runtime.mc"
SHARED="$HERE/tests/qemu/proc/context_runtime.mc"
LDSCRIPT="$HERE/tests/qemu/virt.ld"
TEST_NAME=$([ "$BACKEND" = llvm ] && echo "llvm-async-irq-test" || echo "async-irq-test")
...
if printf '%s' "$OUT" | grep -q "WIR" \
   && printf '%s' "$OUT" | grep -q "ASYNC-IRQ-OK"; then
    echo "PASS: $TEST_NAME — $BACKEND backend: a task PARKED in async_await_irq and a real M-mode TIMER interrupt completed its request from ISR context (WIR, ASYNC-IRQ-OK, result 42) under QEMU"
    exit 0
fi
echo "FAIL: $TEST_NAME — expected WIR and ASYNC-IRQ-OK in kernel output"
exit 1
