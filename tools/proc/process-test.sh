...
if printf '%s' "$OUT" | grep -q "ABC123" \
   && printf '%s' "$OUT" | grep -q "PROC-OK 3"; then
    echo "PASS: process-test — 3 processes ran + exited with codes (ABC), the bootstrap blocking-waited (proc_wait) + reaped all three (123, PROC-OK 3) under QEMU"
    exit 0
fi
echo "FAIL: process-test — expected ABC123 and PROC-OK 3 in kernel output"
exit 1
