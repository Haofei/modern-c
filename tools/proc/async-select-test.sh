...
SRC="$HERE/tests/qemu/proc/async_select_demo.mc"
RUNTIME="$HERE/tests/qemu/proc/async_select_runtime.mc"
SHARED="$HERE/tests/qemu/proc/context_runtime.mc"
LDSCRIPT="$HERE/tests/qemu/virt.ld"
TEST_NAME=$([ "$BACKEND" = llvm ] && echo "llvm-async-select-test" || echo "async-select-test")
...
if printf '%s' "$OUT" | grep -q "WR" \
   && printf '%s' "$OUT" | grep -q "ASYNC-SELECT-OK"; then
    echo "PASS: $TEST_NAME — $BACKEND backend: two in-flight broker requests were RACED; a timer ISR completed the winner; the race CANCELLED the loser and the active-slot count returned to 0 (WR, ASYNC-SELECT-OK) under QEMU"
    exit 0
fi
echo "FAIL: $TEST_NAME — expected WR and ASYNC-SELECT-OK in kernel output"
exit 1
