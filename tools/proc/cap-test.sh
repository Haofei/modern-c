...
if printf '%s' "$OUT" | grep -q "[HI]" \
   && printf '%s' "$OUT" | grep -q "CAP-OK"; then
    echo "PASS: cap-test — console driver as a user-mode server: it alone holds the console capability and prints [HI] via the UART; the client (no capability) prints only through IPC (CAP-OK) under QEMU"
    exit 0
fi
echo "FAIL: process-test — expected [HI] and CAP-OK in kernel output"
exit 1
