...
if printf '%s' "$OUT" | grep -q "ISO-OK"; then
    echo "PASS: isolation-test — per-server MMU isolation: two processes in separate page tables each see only their own frame at the shared VA, exchanging data only via cross-address-space IPC (ISO-OK) under QEMU"
    exit 0
fi
echo "FAIL: sched-vm-test — expected 'ISO-OK' in kernel output"
exit 1
