...
# runs it under QEMU.
#
# Usage: tools/proc/agent-confined-test.sh <path-to-mcc> [c|llvm]
...
SRC="$HERE/tests/qemu/proc/agent_confined_demo.mc"
RUNTIME="$HERE/kernel/arch/riscv64/agent_confined_runtime.mc"
SHARED="$HERE/tests/qemu/proc/context_runtime.mc"
LDSCRIPT="$HERE/tests/qemu/virt.ld"
TEST_NAME=$([ "$BACKEND" = llvm ] && echo "llvm-agent-confined-test" || echo "agent-confined-test")
...
if printf '%s' "$OUT" | grep -q "CONFINED: kernel unmapped in agent space" \
   && printf '%s' "$OUT" | grep -q "CONFINED: agent code is U-only" \
   && printf '%s' "$OUT" | grep -q "42" \
   && printf '%s' "$OUT" | grep -q "USER-EXIT from U"; then
    echo "PASS: $TEST_NAME — $BACKEND backend loaded a separate ELF into an isolated Sv39 address space (kernel unmapped) and ran it confined in U-mode; agent reached the kernel only via syscalls"
    exit 0
fi
echo "FAIL: $TEST_NAME — expected CONFINED markers, agent marker '42', and 'USER-EXIT from U' in kernel output"
exit 1
