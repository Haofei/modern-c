...
#
# Usage: tools/proc/sched-test.sh <path-to-mcc>
...
skip() { echo "SKIP: sched-test ($1)"; exit 0; }
...
if printf '%s' "$OUT" | grep -q "ABCABCABC" \
   && printf '%s' "$OUT" | grep -q "SCHED-OK 3"; then
    echo "PASS: sched-test — round-robin scheduler ran 3 heap-stacked threads (ABCABCABC) under QEMU"
    exit 0
fi
echo "FAIL: sched-test — expected 'ABCABCABC' and 'SCHED-OK 3' in kernel output"
exit 1
