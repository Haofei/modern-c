...
SRC="$HERE/tests/qemu/registry_demo.mc"
...
if printf '%s' "$OUT" | grep -q "REGISTRY-OK" \
   && printf '%s' "$OUT" | grep -q "REGISTRY-OK"; then
    echo "PASS: registry-test — name/registry server: an echo service registered by key, the client looked it up by name (not pid) and round-tripped 1234 (REGISTRY-OK) under QEMU"
    exit 0
fi
echo "FAIL: process-test — expected REGISTRY-OK and REGISTRY-OK in kernel output"
exit 1
