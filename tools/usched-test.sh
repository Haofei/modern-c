...
SRC="$HERE/tests/qemu/proc/usched_demo.mc"
RUNTIME="$HERE/kernel/arch/riscv64/usched_runtime.c"
...
if printf '%s' "$OUT" | grep -q "USCHED-OK" \
   && printf '%s' "$OUT" | grep -q "USCHED-OK"; then
    echo "PASS: usched-test — userspace-set scheduling policy: workers spawned A,B,C but run C,B,A by externally-assigned priority, not round-robin (USCHED-OK) under QEMU"
    exit 0
fi
echo "FAIL: process-test — expected USCHED-OK and USCHED-OK in kernel output"
exit 1
