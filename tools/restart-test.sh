...
if printf '%s' "$OUT" | grep -q "[XR]" \
   && printf '%s' "$OUT" | grep -q "RESTART-OK"; then
    echo "PASS: restart-test — reincarnation: a crashed server (X) was reaped by the supervisor and restarted (R); the second incarnation completed (RESTART-OK) under QEMU"
    exit 0
fi
echo "FAIL: process-test — expected [XR] and RESTART-OK in kernel output"
exit 1
