...
#
# Usage: tools/smp-lock-test.sh <path-to-mcc>
# Skips (exit 0) when the riscv toolchain or QEMU is unavailable.
set -euo pipefail

MCC="${1:-zig-out/bin/mcc}"
HERE="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
SRC="$HERE/tests/qemu/smp_lock_demo.mc"
...
skip() { echo "SKIP: smp-lock-test ($1)"; exit 0; }
...
if printf '%s' "$OUT" | grep -q "SMP-LOCK 4000"; then
    echo "PASS: smp-lock-test — a ticket spinlock gave 2 harts mutual exclusion (counter == harts*ITERS) under QEMU"
    exit 0
fi
echo "FAIL: smp-lock-test — expected 'SMP-LOCK 4000' in kernel output"
exit 1
