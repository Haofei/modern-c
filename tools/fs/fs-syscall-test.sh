...
#
# Lowers the file-syscall demo through the selected backend, links it with the
# asm user-mode runtime into a bare riscv64 image, and runs it under QEMU.
#
# Usage: tools/fs/fs-syscall-test.sh <path-to-mcc> [c|llvm]
...
RUNTIME="$HERE/tests/qemu/fs/fs_syscall_runtime.mc"
SHARED="$HERE/kernel/arch/riscv64/context_runtime.c"
LDSCRIPT="$HERE/tests/qemu/virt.ld"
TEST_NAME=$([ "$BACKEND" = llvm ] && echo "llvm-fs-syscall-test" || echo "fs-syscall-test")
...
if printf '%s' "$OUT" | grep -q "FHI" \
   && printf '%s' "$OUT" | grep -q "USER-EXIT from U"; then
    echo "PASS: $TEST_NAME — $BACKEND backend U-mode open/write/read/close round-tripped a file through the VFS (FHI) under QEMU"
    exit 0
fi
echo "FAIL: $TEST_NAME — expected FHI and 'USER-EXIT from U' in kernel output"
exit 1
