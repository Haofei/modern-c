...
RUNTIME="$HERE/tests/qemu/fs/block_server_runtime.mc"
...
kernel_boot_link_run "$TEST_NAME" "BLK-SERVER-OK" \
    "$BACKEND backend storage driver as a user-mode server: client write+read of a 512B block round-tripped through IPC to the disk-capability server (BLK-SERVER-OK) under QEMU" \
