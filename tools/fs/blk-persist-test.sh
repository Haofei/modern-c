...
# first boot to report PERSIST-WROTE and the second (fresh) boot to report PERSIST-OK.
#
# Usage: tools/fs/blk-persist-test.sh <path-to-mcc> [c|llvm]
...
SRC="$HERE/tests/qemu/arch/blk_persist_demo.mc"
PLATFORM="$HERE/kernel/arch/riscv64/mmode_dma_time.mc"
LDSCRIPT="$HERE/tests/qemu/virt.ld"
TEST_NAME=$([ "$BACKEND" = llvm ] && echo "llvm-blk-persist-test" || echo "blk-persist-test")
...
# A fresh, zeroed disk image — the sentinel is NOT present, so the first boot must write it.
dd if=/dev/zero of="$WORK/disk.img" bs=512 count=8 2>/dev/null
...
echo "=== boot 1 (fresh disk: expect PERSIST-WROTE) ==="
OUT1="$(run_boot)"; printf '%s\n' "$OUT1"
echo "=== boot 2 (same disk, fresh kernel+RAM: expect PERSIST-OK) ==="
OUT2="$(run_boot)"; printf '%s\n' "$OUT2"
echo "--------------------------"

if printf '%s' "$OUT1" | grep -q "PERSIST-WROTE" && printf '%s' "$OUT2" | grep -q "PERSIST-OK"; then
    echo "PASS: $TEST_NAME — $BACKEND backend: a sentinel written to virtio-blk on boot 1 was read back on a fresh boot 2 (durable storage survives reboot)"
    exit 0
fi
echo "FAIL: $TEST_NAME — expected PERSIST-WROTE on boot 1 and PERSIST-OK on boot 2 (boot1='$OUT1' boot2='$OUT2')"
exit 1
