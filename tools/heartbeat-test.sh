...
SRC="$HERE/tests/qemu/proc/heartbeat_demo.mc"
RUNTIME="$HERE/kernel/arch/riscv64/heartbeat_runtime.c"
...
if printf '%s' "$OUT" | grep -q "HB-OK" \
   && printf '%s' "$OUT" | grep -q "HB-OK"; then
    echo "PASS: heartbeat-test — reincarnation with heartbeat liveness: a missed heartbeat was detected via timeout and the worker restarted; the restart heartbeats healthy (HB-OK) under QEMU"
    exit 0
fi
echo "FAIL: process-test — expected HB-OK and HB-OK in kernel output"
exit 1
