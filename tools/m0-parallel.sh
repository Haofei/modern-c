...
pass=$(grep -c "^PASS " "$OUT/summary.txt" 2>/dev/null || echo 0)
echo "[m0-parallel] parallel pass: PASS=$pass FAIL=$(grep -c '^FAIL ' "$OUT/summary.txt" 2>/dev/null || echo 0)  wall=$((E - S))s  (-P $J)"
...
mapfile -t FAILED < <(grep "^FAIL " "$OUT/summary.txt" 2>/dev/null | awk '{print $2}')
real_fail=0
if [ "${#FAILED[@]}" -gt 0 ]; then
