...
#
# Usage: tools/smp-test.sh <path-to-mcc>
# Skips (exit 0) when the riscv toolchain or QEMU is unavailable.
set -euo pipefail

MCC="${1:-zig-out/bin/mcc}"
HERE="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
SRC="$HERE/tests/qemu/smp_demo.mc"
RUNTIME="$HERE/kernel/arch/riscv64/smp_runtime.c"
...
skip() { echo "SKIP: smp-test ($1)"; exit 0; }
...
if printf '%s' "$OUT" | grep -q "SMP-OK 2"; then
    echo "PASS: smp-test — 2 harts booted and synchronized on a shared atomic counter under QEMU"
    exit 0
fi
echo "FAIL: smp-test — expected 'SMP-OK 2' in kernel output"
exit 1
