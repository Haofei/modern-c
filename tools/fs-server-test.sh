...
SRC="$HERE/tests/qemu/fs/fs_server_demo.mc"
RUNTIME="$HERE/kernel/arch/riscv64/fs_server_runtime.c"
...
if printf '%s' "$OUT" | grep -q "FS-SERVER-OK" \
   && printf '%s' "$OUT" | grep -q "FS-SERVER-OK"; then
    echo "PASS: fs-server-test — filesystem as a user-mode server: a client open/write/read round-tripped a file across the IPC boundary to the VFS server (FS-SERVER-OK) under QEMU"
    exit 0
fi
echo "FAIL: process-test — expected FS-SERVER-OK and FS-SERVER-OK in kernel output"
exit 1
