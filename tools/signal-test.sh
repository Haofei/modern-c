...
if printf '%s' "$OUT" | grep -q "SIGNAL-OK" \
   && printf '%s' "$OUT" | grep -q "SIGNAL-OK"; then
    echo "PASS: signal-test — signals: one process delivered SIG_USR to another, which polled pending + took it (SIGNAL-OK) under QEMU"
    exit 0
fi
echo "FAIL: process-test — expected SIGNAL-OK and SIGNAL-OK in kernel output"
exit 1
