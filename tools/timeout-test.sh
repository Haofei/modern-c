...
SRC="$HERE/tests/qemu/ipc/timeout_demo.mc"
RUNTIME="$HERE/kernel/arch/riscv64/timeout_runtime.c"
...
if printf '%s' "$OUT" | grep -q "TIMEOUT-OK" \
   && printf '%s' "$OUT" | grep -q "TIMEOUT-OK"; then
    echo "PASS: timeout-test — ipc_receive_timeout returns a timeout instead of blocking forever when no message arrives (TIMEOUT-OK) under QEMU"
    exit 0
fi
echo "FAIL: process-test — expected TIMEOUT-OK and TIMEOUT-OK in kernel output"
exit 1
