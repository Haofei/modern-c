...
#
# Usage: tools/socket-syscall-test.sh <path-to-mcc>
# Skips (exit 0) when the riscv toolchain or QEMU is unavailable.
set -euo pipefail

MCC="${1:-zig-out/bin/mcc}"
HERE="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
SRC="$HERE/tests/qemu/socket_syscall_demo.mc"
...
skip() { echo "SKIP: socket-syscall-test ($1)"; exit 0; }
...
if printf '%s' "$OUT" | grep -q "RHELLO" \
   && printf '%s' "$OUT" | grep -q "USER-EXIT from U"; then
    echo "PASS: socket-syscall-test — U-mode recvfrom pulled a demultiplexed UDP datagram through the socket layer (RHELLO) under QEMU"
    exit 0
fi
echo "FAIL: socket-syscall-test — expected RHELLO and 'USER-EXIT from U' in kernel output"
exit 1
