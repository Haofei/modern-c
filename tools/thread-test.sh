...
#
# Usage: tools/thread-test.sh <path-to-mcc>
# Skips (exit 0) when the riscv toolchain or QEMU is unavailable.
set -euo pipefail

MCC="${1:-zig-out/bin/mcc}"
HERE="$(d=$(CDPATH= cd -- "$(dirname -- "${BASH_SOURCE[0]}")" && pwd); while [ "$d" != / ] && [ ! -e "$d/build.zig" ]; do d=$(dirname "$d"); done; printf %s "$d")"
SRC="$HERE/tests/qemu/proc/thread_demo.mc"
RUNTIME="$HERE/kernel/arch/riscv64/thread_runtime.c"
...
skip() { echo "SKIP: thread-test ($1)"; exit 0; }
...
if printf '%s' "$OUT" | grep -q "MWMWMW" \
   && printf '%s' "$OUT" | grep -q "THREADS-OK 3"; then
    echo "PASS: thread-test — cooperative context switch interleaved main/worker (MWMWMW) under QEMU"
    exit 0
fi
echo "FAIL: thread-test — expected interleaved 'MWMWMW' and 'THREADS-OK 3' in kernel output"
exit 1
