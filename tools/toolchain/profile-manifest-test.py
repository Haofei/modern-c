...
COMPONENT_MANIFEST = ROOT / "docs" / "component-manifest.json"
...
def main() -> None:
    ...
    if manifest.get("component_manifest") != "docs/component-manifest.json":
        fail("component_manifest must point at docs/component-manifest.json")

    component_manifest = load_json(COMPONENT_MANIFEST)
    if component_manifest.get("schema_version") != 1:
        fail("docs/component-manifest.json schema_version must be 1")
    if component_manifest.get("source_of_truth") != "docs/component-manifest.json":
        fail("docs/component-manifest.json source_of_truth must point at itself")
    components = component_manifest.get("components")
    if not isinstance(components, list) or not components:
        fail("docs/component-manifest.json must define a non-empty components list")

    component_by_id: dict[str, dict[str, Any]] = {}
    for component in components:
        if not isinstance(component, dict):
            fail("each component must be an object")
        component_id = component.get("id")
        if not isinstance(component_id, str) or not component_id:
            fail("each component must define a non-empty id")
        if component_id in component_by_id:
            fail(f"duplicate component id {component_id}")
        component_by_id[component_id] = component
        for field in ("category", "owner", "summary"):
            if not isinstance(component.get(field), str) or not component[field]:
                fail(f"component {component_id} must define {field}")
        component_profiles = component.get("profiles")
        if not isinstance(component_profiles, list) or not component_profiles:
            fail(f"component {component_id} must list owning profiles")
        for profile_id in component_profiles:
            if not isinstance(profile_id, str) or not profile_id:
                fail(f"component {component_id} has invalid profile reference {profile_id!r}")
    ...
    for profile in profiles:
        ...
        if unknown_components:
            fail(f"profile {profile_id} references unknown components: {', '.join(unknown_components)}")
        for component_id in profile_components:
            ...
            if profile_id not in component_profiles:
                fail(f"component {component_id} does not list owning profile {profile_id}")
    ...
    for component_id, component in component_by_id.items():
        ...
        if unknown_component_profiles:
            fail(f"component {component_id} references unknown profiles: {', '.join(unknown_component_profiles)}")
    ...
    print(
        ...
        f"{len(profiles)} profiles, {len(component_by_id)} components, "
