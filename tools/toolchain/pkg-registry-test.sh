...
#   6. build       — the installed tree builds: `mcc-cc app.mc` lowers + compiles the app
...
if command -v "$CLANG" >/dev/null 2>&1; then
    ...
    echo "PASS: pkg-registry-test — publish/resolve(^,=)/install/lockfile-reproducibility/frozen (build step skipped: clang absent)"
fi
