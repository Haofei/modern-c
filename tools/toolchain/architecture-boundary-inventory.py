...
EXACT_FILE_COUNTS = {
    ...
    ("src/lower_c_emitter.zig", "targetTypeFactAtOwnedWithModuleFallback"): 1,
    ("src/lower_llvm.zig", "targetTypeFactAtWithModuleFallback"): 1,
    ("src/lower_llvm.zig", "targetTypeFactAtOwnedWithModuleFallback"): 1,
...
def main() -> int:
    ...
    if len(sources) != 55:
        fail(f"backend source inventory has {len(sources)} files, expected 55")
