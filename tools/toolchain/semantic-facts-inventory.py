...
EXACT_COUNTS: dict[str, dict[str, int]] = {
    ...
    "src/lower_c_emitter.zig": {
        ...
        "fn arrayDirectCallResultTypeForInferredLocal(": 1,
        "const array_ty = self.arrayReturnTypeForExpr(initializer) orelse return false;": 0,
        "const array_ty = self.arrayDirectCallResultTypeForInferredLocal(initializer) orelse return false;": 1,
        "fn sliceDirectCallResultTypeForInferredLocal(": 1,
        "const slice_ty = self.sliceReturnTypeForExpr(initializer, locals) orelse return false;": 0,
        "const slice_ty = self.sliceDirectCallResultTypeForInferredLocal(initializer) orelse return false;": 1,
        "fn resultDirectCallResultTypeForInferredLocal(": 1,
        "const result_ty = self.resultTypeForExpr(initializer, locals) orelse return false;": 0,
        "const result_ty = self.resultDirectCallResultTypeForInferredLocal(initializer) orelse return false;": 1,
        "fn nullableDirectCallResultTypeForInferredLocal(": 1,
        "const nullable_ty = self.nullableReturnTypeForExpr(initializer) orelse return false;": 0,
        "const nullable_ty = self.nullableDirectCallResultTypeForInferredLocal(initializer) orelse return false;": 1,
        ...
        "mirTargetTypeFactAtOwned(.direct_call_result": 5,
