...
SRC="$HERE/tests/qemu/ipc2_demo.mc"
...
if printf '%s' "$OUT" | grep -q "IPC2-OK" \
   && printf '%s' "$OUT" | grep -q "IPC2-OK"; then
    echo "PASS: ipc2-test — IPC completeness: multi-slot mailbox queues two clients, ipc_receive_from filters B before A, async ipc_notify delivered (IPC2-OK) under QEMU"
    exit 0
fi
echo "FAIL: process-test — expected IPC2-OK and IPC2-OK in kernel output"
exit 1
