...
RUNTIME="$HERE/kernel/arch/riscv64/demand_runtime.c"
...
if printf '%s' "$OUT" | grep -q "DEMAND-OK"; then
    echo "PASS: demand-test — demand paging: a store to an unmapped page faulted, the S-mode handler mapped it, and the instruction retried transparently (DEMAND-OK) under QEMU"
    exit 0
fi
echo "FAIL: paging-activate-test — expected 'DEMAND-OK' in kernel output"
exit 1
