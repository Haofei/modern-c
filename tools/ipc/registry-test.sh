...
#
# Lowers the registry server demo through the selected backend, links it with the
# scheduler/context runtime into a bare riscv64 image, and runs it under QEMU.
#
# Usage: tools/ipc/registry-test.sh <path-to-mcc> [c|llvm]
...
RUNTIME="$HERE/tests/qemu/ipc/registry_runtime.mc"
SHARED="$HERE/kernel/arch/riscv64/context_runtime.c"
LDSCRIPT="$HERE/tests/qemu/virt.ld"
TEST_NAME=$([ "$BACKEND" = llvm ] && echo "llvm-registry-test" || echo "registry-test")
...
if printf '%s' "$OUT" | grep -q "REGISTRY-OK"; then
    echo "PASS: $TEST_NAME — $BACKEND backend name/registry server: an echo service registered by key, the client looked it up by name (not pid) and round-tripped 1234 (REGISTRY-OK) under QEMU"
    exit 0
fi
echo "FAIL: $TEST_NAME — expected REGISTRY-OK in kernel output"
exit 1
