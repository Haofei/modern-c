...
#
# Usage: tools/elf-run-test.sh <path-to-mcc>
# Skips (exit 0) when the riscv toolchain or QEMU is unavailable.
set -euo pipefail

MCC="${1:-zig-out/bin/mcc}"
HERE="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
SRC="$HERE/tests/qemu/elf_run_demo.mc"
...
skip() { echo "SKIP: elf-run-test ($1)"; exit 0; }
...
if printf '%s' "$OUT" | grep -q "OK" \
   && printf '%s' "$OUT" | grep -q "USER-EXIT from U"; then
    echo "PASS: elf-run-test — loaded an ELF64, ran its code in U-mode (printed OK via syscalls), exited from U under QEMU"
    exit 0
fi
echo "FAIL: elf-run-test — expected OK and 'USER-EXIT from U' in kernel output"
exit 1
