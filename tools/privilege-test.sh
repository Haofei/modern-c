...
SRC="$HERE/tests/qemu/privilege_demo.mc"
RUNTIME="$HERE/kernel/arch/riscv64/privilege_runtime.c"
...
if printf '%s' "$OUT" | grep -q "PRIV-OK" \
   && printf '%s' "$OUT" | grep -q "PRIV-OK"; then
    echo "PASS: privilege-test — least privilege: forbidden IPC peer rejected + kernel call outside the mask Denied (PRIV-OK) under QEMU"
    exit 0
fi
echo "FAIL: process-test — expected PRIV-OK and PRIV-OK in kernel output"
exit 1
