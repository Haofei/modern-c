...
#
# Usage: tools/ipi-test.sh <path-to-mcc>
# Skips (exit 0) when the riscv toolchain or QEMU is unavailable.
set -euo pipefail

MCC="${1:-zig-out/bin/mcc}"
HERE="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
SRC="$HERE/tests/qemu/ipi_demo.mc"
...
skip() { echo "SKIP: ipi-test ($1)"; exit 0; }
...
if printf '%s' "$OUT" | grep -q "IPI-OK"; then
    echo "PASS: ipi-test — hart 0 raised a CLINT software interrupt (IPI) and hart 1 handled it under QEMU"
    exit 0
fi
echo "FAIL: ipi-test — expected 'IPI-OK' in kernel output"
exit 1
