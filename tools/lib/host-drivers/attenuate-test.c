...
// host has no real CPU contexts — the test only exercises the capability-mask bookkeeping path).
...
extern uint32_t attenuate_run(void);
int main(void){ return attenuate_run()==1 ? 0 : 1; }
