...
// contexts — the test only exercises the fair-share pick + throttle bookkeeping path).
...
extern uint32_t fairsched_run(void);
int main(void){ return fairsched_run()==1 ? 0 : 1; }
