...
if printf '%s' "$OUT" | grep -q "DEMAND-OK"; then
    echo "PASS: $TEST_NAME — $BACKEND backend demand paging: a store to an unmapped page faulted, the S-mode handler mapped it, and the instruction retried transparently (DEMAND-OK) under QEMU"
    exit 0
fi
echo "FAIL: $TEST_NAME — expected 'DEMAND-OK' in kernel output"
exit 1
