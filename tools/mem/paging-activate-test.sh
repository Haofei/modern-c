...
if printf '%s' "$OUT" | grep -q "PAGING-OK"; then
    echo "PASS: paging-activate-test — activated Sv39 satp and read a translation-only virtual address under QEMU"
    exit 0
fi
echo "FAIL: paging-activate-test — expected 'PAGING-OK' in kernel output"
exit 1
