...
SRC="$HERE/kernel/core/cow.mc"
RUNTIME="$HERE/kernel/arch/riscv64/cow_runtime.c"
...
if printf '%s' "$OUT" | grep -q "COW-OK"; then
    echo "PASS: cow-test — copy-on-write: a store to a shared read-only page faulted, the handler gave the writer a private copy; the other space still sees the original (COW-OK) under QEMU"
    exit 0
fi
echo "FAIL: paging-activate-test — expected 'COW-OK' in kernel output"
exit 1
