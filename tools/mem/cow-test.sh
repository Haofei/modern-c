...
kernel_boot_link_run "$TEST_NAME" "COW-OK" \
    "$BACKEND backend copy-on-write: a store to a shared read-only page faulted, the handler gave the writer a private copy; the other space still sees the original (COW-OK) under QEMU" \
