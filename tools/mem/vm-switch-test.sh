...
#
# Usage: tools/mem/vm-switch-test.sh <path-to-mcc>
...
skip() { echo "SKIP: vm-switch-test ($1)"; exit 0; }
...
if printf '%s' "$OUT" | grep -q "VMSWITCH-OK"; then
    echo "PASS: vm-switch-test — switched satp between two address spaces; the same VA resolved to different frames under QEMU"
    exit 0
fi
echo "FAIL: vm-switch-test — expected 'VMSWITCH-OK' in kernel output"
exit 1
