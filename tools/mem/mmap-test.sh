...
#
# Usage: tools/mem/paging-activate-test.sh <path-to-mcc>
# Skips (exit 0) when the riscv toolchain or QEMU is unavailable.
set -euo pipefail

MCC="${1:-zig-out/bin/mcc}"
HERE="$(d=$(CDPATH= cd -- "$(dirname -- "${BASH_SOURCE[0]}")" && pwd); while [ "$d" != / ] && [ ! -e "$d/build.zig" ]; do d=$(dirname "$d"); done; printf %s "$d")"
SRC="$HERE/tests/qemu/mem/mmap_demo.mc"
RUNTIME="$HERE/kernel/arch/riscv64/mmap_runtime.c"
...
if printf '%s' "$OUT" | grep -q "MMAP-OK"; then
    echo "PASS: mmap-test — mmap two anonymous pages into a page table; under active satp both VAs read/write independent RAM (MMAP-OK) under QEMU"
    exit 0
fi
echo "FAIL: paging-activate-test — expected 'MMAP-OK' in kernel output"
exit 1
