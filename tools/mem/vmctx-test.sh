...
#
# Usage: tools/mem/vmctx-test.sh <path-to-mcc>
...
skip() { echo "SKIP: vmctx-test ($1)"; exit 0; }
...
if printf '%s' "$OUT" | grep -q "VMCTX-OK"; then
    echo "PASS: vmctx-test — context switch swapped satp per thread; A saw 0xa and B saw 0xb at the same VA under QEMU"
    exit 0
fi
echo "FAIL: vmctx-test — expected 'VMCTX-OK' in kernel output"
exit 1
