...
#
# Usage:
#   tools/mcc-pkg.sh info  [manifest|dir]   # print the parsed manifest
#   tools/mcc-pkg.sh deps  [manifest|dir]   # resolve + version-check deps
#   tools/mcc-pkg.sh build [manifest|dir]   # build entry -> output object
#
...
case "$cmd" in
    ...
    build)
        ...
        MCC="$MCC" "$HERE/tools/mcc-cc.sh" "$ENTRY" -o "$OUT" >/dev/null
