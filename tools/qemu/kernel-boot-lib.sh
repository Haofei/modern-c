...
kernel_boot_compile_rt() {
    local out="$1"
    # The freestanding mem*/str* lib is now PURE MC (tests/qemu/support/lib/freestanding.mc). Lower it via
    ...
    local dir
    dir="$(dirname "$out")"
    "$MCC" emit-c "$HERE/tests/qemu/support/lib/freestanding.mc" > "$dir/freestanding.c"
