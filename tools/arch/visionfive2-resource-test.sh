#!/usr/bin/env bash
# VisionFive 2 resource fixture: boot under OpenSBI on QEMU `virt`, preserve
...
TEST_NAME=$([ "$BACKEND" = llvm ] && echo "llvm-visionfive2-resource-test" || echo "visionfive2-resource-test")
...
kernel_boot_compile_mc_object "$BACKEND" "$HERE/tests/qemu/arch/visionfive2_resource_demo.mc" "$WORK/mc.o" "$WORK"
...
if printf '%s' "$OUT" | grep -qi "OpenSBI" \
   ...
   && printf '%s' "$OUT" | grep -q "VF2-QEMU-SURROGATE-OK"; then
    echo "PASS: $TEST_NAME — $BACKEND backend validated the VisionFive 2 FDT-resource fixture against the QEMU/OpenSBI surrogate"
    exit 0
fi
