#!/usr/bin/env bash
# Runnable RTC MMIO test.
#
# Lowers the RTC demo through the selected backend, links it with the runtime
# into a bare riscv64 image, and runs it under QEMU.
#
# Usage: tools/arch/rtc-test.sh <path-to-mcc> [c|llvm]
...
RUNTIME="$HERE/tests/qemu/arch/rtc_runtime.mc"
SHARED="$HERE/tests/qemu/proc/context_runtime.mc"
LDSCRIPT="$HERE/tests/qemu/virt.ld"
TEST_NAME=$([ "$BACKEND" = llvm ] && echo "llvm-rtc-test" || echo "rtc-test")
