...
EXPECT="${4:-net-irq: ok}"
NAME_BASE="${5:-qjs-smode-net-irq-tool}"
CLANG="${CLANG:-clang}"
LLD="${LLD:-ld.lld}"
LLC="${LLC:-llc}"
...
NETIRQ="$HERE/tests/qemu/arch/app_run_net_irq.mc"
RUNTIME="$HERE/tests/qemu/arch/qjs_smode_net_irq_runtime.mc"
...
mkdir -p "$WORK/app" "$WORK/netirq" "$WORK/runtime" "$WORK/platform" "$WORK/ctx"
...
if printf '%s' "$OUT" | grep -qi "OpenSBI" \
   ...
   && printf '%s' "$OUT" | grep -q "USER-EXIT from U"; then
    echo "PASS: $TEST_NAME — $BACKEND backend pure-JS host_net_fetch resolved through the gated SYS_POLL fixture from a real S-mode virtio-net PLIC interrupt."
    exit 0
fi
