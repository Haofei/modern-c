#!/usr/bin/env bash
# Interrupt-backed async virtio-net RX completion under REAL OpenSBI in S-mode.
#
...
SRC="$HERE/tests/qemu/arch/net_smode_rx_irq_demo.mc"
...
TEST_NAME=$([ "$BACKEND" = llvm ] && echo "llvm-net-smode-rx-irq-test" || echo "net-smode-rx-irq-test")
...
"$LLD" -T "$LDSCRIPT" "$WORK/freestanding.o" "$WORK/demo.o" "$WORK/context.o" "$WORK/platform.o" $SUPPORT_OBJ -o "$WORK/net-smode-rx-irq.elf"

OUT="$(timeout 30 "$QEMU" -machine virt -m 256M -nographic \
        -global virtio-mmio.force-legacy=false \
        -netdev user,id=n0 \
        -device virtio-net-device,netdev=n0 \
        -kernel "$WORK/net-smode-rx-irq.elf" 2>/dev/null || true)"

echo "--- OpenSBI + S-mode async net RX IRQ output ---"
printf '%s\n' "$OUT"
echo "------------------------------------------------"

IRQS="$(printf '%s' "$OUT" | sed -n 's/.*NET-SMODE-RXIRQ TX=[0-9][0-9]* RX=[0-9][0-9]* IRQS=\([0-9][0-9]*\).*/\1/p' | tail -n1)"
REAPED="$(printf '%s' "$OUT" | sed -n 's/.*REAPED=\([0-9][0-9]*\).*/\1/p' | tail -n1)"
RX_LEN="$(printf '%s' "$OUT" | sed -n 's/.*NET-SMODE-RXIRQ TX=[0-9][0-9]* RX=\([0-9][0-9]*\).*/\1/p' | tail -n1)"
if printf '%s' "$OUT" | grep -qi "OpenSBI" \
   && printf '%s' "$OUT" | grep -q "NET-SMODE-RXIRQ TX=1" \
   && printf '%s' "$OUT" | grep -q "NET-SMODE-RXIRQ-OK" \
