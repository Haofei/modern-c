#!/usr/bin/env bash
# Deterministic in-kernel REAL TLS 1.2 HTTPS GET under REAL OpenSBI in S-mode.
#
# Same EXISTING in-kernel BearSSL TLS 1.2 client + MC TCP transport
# (tests/qemu/tls/tls_demo.mc) as the M-mode https-get-test, but linked with the
# S-mode/OpenSBI runtime (kernel/arch/riscv64/https_get_smode_runtime.c) and the
...
# MC-KERNEL-HTTPS-OK and "200". DETERMINISTIC: the TLS peer is local (slirp
# loopback), no live internet egress. (satp=0 Bare; OpenSBI PMP permits S-mode
# virtio-mmio + RAM DMA + the goldfish-RTC; time via the rdtime CSR since the CLINT
# mtime MMIO faults under OpenSBI.)
#
# PASS requires the OpenSBI banner AND all of: UART HTTPS-GET-OK +
# CERT-CHAIN-VALIDATED + decrypted token + "200", the python access log shows the
# GET 200 (served over TLS), and a non-empty pcap (TLS records, not plaintext).
#
# Usage: tools/arch/https-smode-test.sh <path-to-mcc> [c|llvm]
...
PORT=18444                         # local HTTPS server port (slirp -> host loopback)
...
OUT="$(timeout 90 "$QEMU" -machine virt -m 256M -nographic \
...
echo "--- OpenSBI + guest UART output ---"
printf '%s\n' "$OUT"
echo "-----------------------------------"
...
# 6. PASS requires all proofs (incl. the OpenSBI banner proving S-mode boot).
SBI_OK=0; UART_OK=0; CERT_OK=0; TOKEN_OK=0; STATUS_OK=0; LOG_OK=0; PCAP_OK=0
...
if [ "$SBI_OK" = 1 ] && [ "$UART_OK" = 1 ] && [ "$CERT_OK" = 1 ] && [ "$TOKEN_OK" = 1 ] \
   && [ "$STATUS_OK" = 1 ] && [ "$LOG_OK" = 1 ] && [ "$PCAP_OK" = 1 ]; then
    echo "PASS: $TEST_NAME — $BACKEND backend: REAL BearSSL TLS 1.2 handshake validated the"
    echo "  self-signed '$SERVERNAME' trust anchor and decrypted a real HTTPS GET under REAL OpenSBI in S-mode."
...
echo "  OpenSBI banner: $SBI_OK   UART HTTPS-GET-OK: $UART_OK   cert-validated: $CERT_OK   decrypted token: $TOKEN_OK"
