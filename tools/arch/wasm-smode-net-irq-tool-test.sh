...
# WASM peer of qjs-smode-net-irq-tool-test.sh. Same confined WASM agent ELF (wasm3 + WASI P1 shim +
...
#
# Usage: tools/arch/wasm-smode-net-irq-tool-test.sh <mcc> [c|llvm] [guest.c] [expect] [name-base]
set -euo pipefail

MCC="${1:-zig-out/bin/mcc}"
BACKEND="${2:-c}"
GUEST_REL="${3:-examples/apps/wasm/wasi_net_irq.c}"
EXPECT="${4:-net-irq: ok}"
NAME_BASE="${5:-wasm-smode-net-irq-tool}"
GUEST_KIND="${6:-wasi}"
CLANG="${CLANG:-clang}"
LLD="${LLD:-ld.lld}"
LLC="${LLC:-llc}"
...
NETIRQ="$HERE/tests/qemu/arch/app_run_net_irq.mc"             # virtio-net IRQ wiring (agent-agnostic)
RUNTIME="$HERE/tests/qemu/arch/qjs_smode_net_irq_runtime.mc"  # S-mode net-IRQ bring-up
...
mkdir -p "$WORK/app" "$WORK/netirq" "$WORK/runtime" "$WORK/platform" "$WORK/ctx" "$WORK/um"
kernel_boot_compile_mc_object "$BACKEND" "$SRC" "$WORK/app.o" "$WORK/app"
kernel_boot_compile_mc_object "$BACKEND" "$NETIRQ" "$WORK/netirq.o" "$WORK/netirq"
...
"$LLD" --allow-multiple-definition -T "$LDSCRIPT" \
    ...
    "$WORK/netirq.o" "$WORK/platform.o" "$WORK/ctx_stubs.o" \
...
echo "--- OpenSBI + S-mode IRQ net tool output ---"
printf '%s\n' "$OUT"
echo "--------------------------------------------"
