...
# lives under .wamr-cache/aarch64/ to avoid colliding with the other arches' objects.
...
WDEF=(-DBH_PLATFORM_MC -DBUILD_TARGET_AARCH64 -DWASM_ENABLE_INTERP=1
...
if printf '%s' "$OUT" | grep -qa "CONFINED: kernel mapped EL1-only" \
   ...
   && printf '%s' "$OUT" | grep -qa "USER-EXIT"; then
    echo "PASS: $TEST_NAME — $BACKEND backend ran a STOCK wasm32-wasi guest on WAMR confined in an isolated aarch64 EL0 space under QEMU, with async host I/O over svc SYS_SUBMIT/SYS_POLL; the kernel is mapped EL1-only (unreachable from EL0) and the agent reached it only via svc #0"
    exit 0
fi
