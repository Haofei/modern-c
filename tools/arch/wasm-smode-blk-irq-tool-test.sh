...
# serviced through a REAL S-mode virtio-blk PLIC interrupt and delivered via the gated SYS_POLL fixture — the
...
if printf '%s' "$OUT" | grep -qi "OpenSBI" \
   ...
   && printf '%s' "$OUT" | grep -q "USER-EXIT from U"; then
    echo "PASS: $TEST_NAME — $BACKEND backend: a confined WASM guest's brokered fs_read completed through the gated SYS_POLL fixture from a real S-mode virtio-blk PLIC interrupt, under REAL OpenSBI."
    exit 0
fi
