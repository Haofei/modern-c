...
SRC="$HERE/tests/qemu/ipc_demo.mc"
...
if printf '%s' "$OUT" | grep -q "CSR" \
   && printf '%s' "$OUT" | grep -q "IPC-OK"; then
    echo "PASS: ipc-test — kernel-mediated IPC: a client and a user-mode service round-tripped a request/reply through ipc_send/ipc_receive (CSR, IPC-OK, 21*2==42) under QEMU"
    exit 0
fi
echo "FAIL: process-test — expected CSR and IPC-OK in kernel output"
exit 1
