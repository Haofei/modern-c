...
| `E_ADDRESS_CLASS_OPERATION` | MIR verifier found illegal operation on opaque address class<br>opaque address classes do not support this operator | `src/mir.zig:1780`<br>`src/sema.zig:3858`<br>`src/sema.zig:3896` |
...
| `E_BACKEND_UNSUPPORTED` | C backend does not yet support {s}<br>LLVM backend does not yet support {s}<br>{s} backend does not yet support this construct | `src/lower_c_emitter.zig:4779`<br>`src/lower_c_emitter.zig:5309`<br>`src/lower_llvm.zig:2659`<br>`src/lower_llvm.zig:2666`<br>`src/main.zig:1267` |
...
| `E_MIR_CFG` | MIR verifier found malformed control-flow graph | `src/mir.zig:12204` |
| `E_MIR_IDENTITY` | MIR verifier found malformed instruction identity | `src/mir.zig:1941` |
| `E_MIR_OWNERSHIP_EVENT` | MIR verifier found inconsistent ownership event sequence<br>MIR verifier found malformed ownership event | `src/mir.zig:2461`<br>`src/mir.zig:2468` |
| `E_MIR_SYMBOL_ID` | MIR verifier found malformed symbol identity table | `src/mir.zig:1929` |
| `E_MMIO_ACCESS_FORBIDDEN` | MIR verifier found MMIO register access disallowed by Reg/RegBits mode<br>MMIO register access mode does not allow read<br>MMIO register access mode does not allow write | `src/mir.zig:1808`<br>`src/sema.zig:4730`<br>`src/sema.zig:4740` |
| `E_MMIO_ACCESS_MODE` | MMIO register access mode must be .read, .write, or .read_write | `src/sema.zig:4710`<br>`src/sema.zig:4715` |
| `E_MMIO_DIRECT_ASSIGN` | MIR verifier found direct assignment to an MMIO register<br>MMIO registers must be accessed through typed read/write methods | `src/mir.zig:1802`<br>`src/sema.zig:3443` |
...
| `E_NEVER_FALLTHROUGH` | function declared -&gt; never can fall off the end | `src/hir_inspection.zig:187`<br>`src/mir.zig:1726`<br>`src/sema.zig:2413` |
...
| `E_NO_LANG_TRAP_EDGE` | HIR verifier found language trap edge {s} before C emission<br>MIR verifier found language trap edge {s}<br>assert may emit a language trap in #[no_lang_trap]<br>_+9 more_ | `src/hir_inspection.zig:201`<br>`src/mir.zig:1735`<br>`src/sema.zig:3426`<br>`src/sema.zig:3798`<br>`src/sema.zig:3823`<br>`src/sema.zig:3840`<br>_+7 more_ |
...
| `E_REPRESENTATION_CHECK_MISSING` | MIR verifier found representation-sensitive value use without dominating check | `src/mir.zig:1816`<br>`src/mir.zig:1823` |
...
| `E_RETURN_MISSING` | function return type requires all paths to return a value | `src/hir_inspection.zig:187`<br>`src/mir.zig:1726`<br>`src/sema.zig:2415` |
...
| `E_UNCHECKED_OUTSIDE_CONTRACT` | MIR verifier found unchecked optimizer assumption outside matching contract region<br>unchecked operation requires matching #[unsafe_contract] | `src/mir.zig:1746`<br>`src/sema.zig:4009` |
...
| `E_UNSAFE_REQUIRED` | MIR verifier found unsafe machine effect outside unsafe context<br>a \`#[unsafe_ffi]\` boundary cannot be converted to a plain function pointer; wrap it in an audited safe MC function<br>a \`#[unsafe_ffi]\` boundary cannot be inferred as a plain function pointer; wrap it in an audited safe MC function<br>_+16 more_ | `src/mir.zig:1753`<br>`src/sema.zig:3326`<br>`src/sema.zig:4013`<br>`src/sema.zig:4020`<br>`src/sema.zig:4132`<br>`src/sema.zig:4262`<br>_+17 more_ |
