...
| `E_BACKEND_UNSUPPORTED` | C backend does not yet support {s}<br>LLVM backend does not yet support {s}<br>{s} backend does not yet support this construct | `src/lower_c_emitter.zig:6599`<br>`src/lower_c_emitter.zig:7129`<br>`src/lower_llvm.zig:4482`<br>`src/lower_llvm.zig:4489`<br>`src/main.zig:1267` |
...
| `E_MIR_CFG` | MIR verifier found malformed control-flow graph | `src/mir.zig:12206` |
