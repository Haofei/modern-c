...
| `E_ADDRESS_CLASS_OPERATION` | MIR verifier found illegal operation on opaque address class<br>opaque address classes do not support this operator | `src/mir.zig:3892`<br>`src/sema.zig:3852`<br>`src/sema.zig:3890` |
...
| `E_BACKEND_UNSUPPORTED` | C backend does not yet support {s}<br>LLVM backend does not yet support {s}<br>{s} backend does not yet support this construct | `src/driver_build.zig:151`<br>`src/lower_c_emitter.zig:2110`<br>`src/lower_llvm.zig:1194`<br>`src/lower_llvm.zig:1201` |
...
| `E_DUPLICATE_LOCAL` | local bindings must have unique names in the current scope | `src/async_lower.zig:1407`<br>`src/sema.zig:3576`<br>`src/sema.zig:8288` |
...
| `E_DUPLICATE_PARAMETER` | function parameter names must be unique | `src/async_lower.zig:1426`<br>`src/sema.zig:2319` |
...
| `E_MIR_ACCESS_FACT` | MIR verifier found duplicate resolved access fact<br>MIR verifier found index instruction without resolved access fact<br>MIR verifier found malformed resolved access fact | `src/mir.zig:4091`<br>`src/mir.zig:4100`<br>`src/mir.zig:4119` |
| `E_MIR_CFG` | MIR verifier found malformed control-flow graph | `src/mir.zig:22690` |
| `E_MIR_IDENTITY` | MIR verifier found malformed instruction identity | `src/mir.zig:4077` |
| `E_MIR_OWNERSHIP_EVENT` | MIR verifier found inconsistent ownership event sequence<br>MIR verifier found malformed ownership event | `src/mir.zig:5067`<br>`src/mir.zig:5074` |
| `E_MIR_SOURCE_ID` | MIR verifier found malformed per-file source identity | `src/mir.zig:4065` |
| `E_MIR_SYMBOL_ID` | MIR verifier found malformed symbol identity table | `src/mir.zig:4041` |
| `E_MMIO_ACCESS_FORBIDDEN` | MIR verifier found MMIO register access disallowed by Reg/RegBits mode<br>MMIO register access mode does not allow read<br>MMIO register access mode does not allow write | `src/mir.zig:3920`<br>`src/sema.zig:4724`<br>`src/sema.zig:4734` |
| `E_MMIO_ACCESS_MODE` | MMIO register access mode must be .read, .write, or .read_write | `src/sema.zig:4704`<br>`src/sema.zig:4709` |
| `E_MMIO_DIRECT_ASSIGN` | MIR verifier found direct assignment to an MMIO register<br>MMIO registers must be accessed through typed read/write methods | `src/mir.zig:3914`<br>`src/sema.zig:3437` |
...
| `E_NEVER_FALLTHROUGH` | function declared -&gt; never can fall off the end | `src/hir_inspection.zig:187`<br>`src/mir.zig:3836`<br>`src/sema.zig:2407` |
...
| `E_NO_LANG_TRAP_EDGE` | HIR verifier found language trap edge {s} before C emission<br>MIR verifier found language trap edge {s}<br>assert may emit a language trap in #[no_lang_trap]<br>_+9 more_ | `src/hir_inspection.zig:201`<br>`src/mir.zig:3846`<br>`src/sema.zig:3420`<br>`src/sema.zig:3792`<br>`src/sema.zig:3817`<br>`src/sema.zig:3834`<br>_+7 more_ |
...
| `E_REPRESENTATION_CHECK_MISSING` | MIR verifier found representation-sensitive value use without dominating check | `src/mir.zig:3928`<br>`src/mir.zig:3935` |
...
| `E_RETURN_MISSING` | function return type requires all paths to return a value | `src/hir_inspection.zig:187`<br>`src/mir.zig:3836`<br>`src/sema.zig:2409` |
...
| `E_UNCHECKED_OUTSIDE_CONTRACT` | MIR verifier found unchecked optimizer assumption outside matching contract region<br>unchecked operation requires matching #[unsafe_contract] | `src/mir.zig:3858`<br>`src/sema.zig:4003` |
...
| `E_UNSAFE_REQUIRED` | MIR verifier found unsafe machine effect outside unsafe context<br>a \`#[unsafe_ffi]\` boundary cannot be converted to a plain function pointer; wrap it in an audited safe MC function<br>a \`#[unsafe_ffi]\` boundary cannot be inferred as a plain function pointer; wrap it in an audited safe MC function<br>_+16 more_ | `src/mir.zig:3865`<br>`src/sema.zig:3320`<br>`src/sema.zig:4007`<br>`src/sema.zig:4014`<br>`src/sema.zig:4126`<br>`src/sema.zig:4256`<br>_+17 more_ |
