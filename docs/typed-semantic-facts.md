...
The latest bounded call/type migration gives `mmio.map<T>(PAddr)` an exact call
...
typing no longer reconstruct `MmioPtr<T>` from the AST type argument. Missing
or stale facts fail MIR admission. This closes the MMIO mapping operation
family, not the broader call-target, expression-type, pointer-provenance, or
typed-HIR/MIR workstreams.
