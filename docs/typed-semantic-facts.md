...
The latest bounded call/type migration gives all accepted varargs operations
complete cursor/payload/result facts. Sema validates exact call shape and cursor
type, MIR owns the canonical ABI cursor type, and C/LLVM no longer recover that
type from backend-local expression inference. Missing identities or each
applicable type family fail MIR admission independently. This closes the
varargs operation family; broader expression typing, representation,
