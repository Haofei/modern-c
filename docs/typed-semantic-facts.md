...
| MIR typed instruction metadata | `src/mir_model.zig` `ValueType`, `Instruction`, and optional `value_id` / `contract_region_id` carry typed operation metadata. `Function` owns `contract_regions`, `range_facts`, `call_target_facts`, `pointer_provenance_facts`, `representation_facts`, and `elided_bounds`. | Function construction transfers owned slices in `FunctionBuilder.finish`. `CallTargetFact` records bounded `reduce`, `const_get`, atomic member, `MaybeUninit` member, exact bitcast, exact `phys`, exact `raw.load` / `raw.ptr` / `raw.store`, and exact `cpu.pause` target kinds with the MIR result type and source identity; atomic facts cover direct and pointer receivers and both atomic stores, MaybeUninit writes, raw stores, and cpu pause record `void`, phys records `PAddr`, and raw value facts preserve the raw operation result type. Each row matches a dedicated `.call_target` metadata instruction, so ordinary same-named user calls cannot collide. Exact validation rejects missing, stale, and duplicate rows. C and LLVM query the fact kind to select these lowerings. `Instruction.value_id` is populated by `addInstrWithValue`; the same builder path appends typed `RepresentationFact` rows for representation-sensitive typed loads, checks, uses, and aggregate field/element representation obligations with stable value identities such as `value_id=cast`. `PointerProvenanceFact.field_path` rows own their aggregate path strings, cover direct aggregate pointer-field and pointer-array element facts from initializers, direct assignments, same-struct initializer and assignment copies including nested field and pointer-array element paths, whole-aggregate struct-literal reassignments, and nested aggregate member struct-literal reassignments, and are freed with the MIR module. The MIR representation verifier uses the instruction `value_id` identities to match checks to uses. | `mcc lower-mir` dispatches through `src/main.zig` and prints `mir.appendDumpOpt`. `appendDumpOpt` prints function summaries including `representation_facts=N` and `call_target_facts=N`, instructions including `value_id=...`, trap edges, owned `mir representation_fact` / `mir call_target_fact` rows, range facts, pointer-provenance facts including optional `field=...`, and explicit elided-bounds fact rows. | C and LLVM consume owned representation, integer, and bounded call-target fact slices at backend entry and during these bounded call lowerings. Broader backend semantic decisions remain limited to the fact families below plus backend-local fallback inference. |
...
| `c-call-target-classification` | `src/lower_c_call.zig` sequenced/bitcast/extern-nonnull call emitters; `src/lower_c_builtin.zig` builtin classifiers | C call emission and special builtin lowering | Registered backend inference. | Unknown call targets stay ordinary calls or unsupported; no provenance/range fact may be inferred from a call spelling. |
...
| `llvm-expression-type-inference` | `src/lower_llvm.zig` `exprType` / `derefPointeeType`; shared `src/ast_query.zig` `qualifiedTaggedUnionConstructorType`, `enumVariantPathType`, `rawLoadCallReturnType`, `rawPtrCallReturnType`, `vaCallReturnType`, `byteViewCallReturnType`, `reflectionValueCallReturnType`, `isDeclassifyCall`, `resultConstructorCallTag`, `isBindCallNode`, and `isBindCallExpr` | LLVM expression, call, deref, constructor, and MMIO emission | Registered backend inference; qualified tagged-union constructor typing, enum variant-path typing, raw load/pointer builtin result typing, value-producing varargs builtin result typing, byte-view builtin result typing, value-producing reflection result typing, declassify/reveal call recognition, Result constructor call recognition, and closure bind call recognition use shared AST queries. Bounded `reduce`, `atomic`, `MaybeUninit`, `bitcast`, and `phys` classification comes from MIR `CallTargetFact.kind`; `const_get` uses its AST query only to extract confirmed operands. The inventory rejects reintroducing the retired LLVM AST call-target classifiers. The broader family still needs typed facts/MIR migration. | Unsupported or unknown shapes return null/unsupported and must not infer facts beyond sema-accepted source meaning. |
