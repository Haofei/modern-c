...
The latest bounded call/type migration gives `const_get` complete base/result
types and a MIR-owned compile-time index paired with its typed instruction. C
and LLVM no longer read the AST index as semantic authority; LLVM also stops
reconstructing the array, element, and length from backend expression typing.
Missing or stale facts fail MIR admission. This closes `const_get`, not the
broader call-target or expression-type inference families.
