...
The latest bounded call-type migration adds `bitcast_source`, `bitcast_target`,
and `phys_result` target-type rows. C and LLVM now require those facts instead
of inferring a bitcast operand/result from the AST or synthesizing `PAddr` in a
backend; missing rows fail MIR admission and exact-count inventory gates prevent
the retired fallbacks from returning.
