...
| MIR inferred local copy/cast/binary/literal type facts | For a one-name unannotated local initialized directly from an existing local value, an explicit `value as T` cast, a typed arithmetic/comparison/logical binary, or a targetless integer (`u32`) / bool literal, MIR records an owned `inferred_local` target-type fact keyed by the binding name and initializer source point. | Generic target-type validation matches the owned fact and instruction. C and LLVM compare only the sema-defined local/cast/binary/literal type to reject stale prebuilt MIR; the fact selects the destination binding type. | `lower-mir` prints the owner alongside the target-type row. | C uses the fact for inferred local-copy, direct-cast, checked-arithmetic, comparison, short-circuit logical, integer-literal, and bool-literal declarations; LLVM uses it before allocating each binding. Removing the fact fails MIR admission and retargeting it fails backend emission. Calls and broader computed initializers remain in the registered broader expression-inference boundary. |
