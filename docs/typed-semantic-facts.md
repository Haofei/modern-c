...
| LLVM backend-local pointer/global race provenance | `LlvmEmitter` owns backend-local maps for `global_pointer_locals`, local function/aggregate/pointer-array aliases, aggregate field facts, array element facts, slice facts, return summaries, and parameter summaries (`src/lower_llvm.zig:295-306`, `src/lower_llvm.zig:448-460`). It collects summary facts in `collectGlobalPointerProvenanceSummaries` (`src/lower_llvm.zig:679-734`) and updates local facts through functions such as `updatePointerGlobalProvenance` (`src/lower_llvm.zig:5630-5640`), `updateAggregatePointerAliasProvenance` (`src/lower_llvm.zig:5672-5698`), and array/field/slice helpers. | This is the largest duplicated inference class. It resets or clears facts on block/contract/loop/switch boundaries and asm (`src/lower_llvm.zig:1489-1501`, `src/lower_llvm.zig:1537-1570`), on local declaration/assignment tracking (`src/lower_llvm.zig:1584-1632`), on aggregate pointer deref assignment (`src/lower_llvm.zig:5700-5704`), and on array/slice backing writes (`src/lower_llvm.zig:4605-4619`). | No `mcc facts`/`lower-mir` typed artifact prints this provenance today. Tests inspect emitted LLVM and fixture behavior instead. | LLVM consumes these backend-local maps via `pointerExprHasGlobalStorageProvenance` (`src/lower_llvm.zig:5706-5745`) to choose unordered atomic loads/stores for pointer-mediated global storage (`src/lower_llvm.zig:3464-3472`, `src/lower_llvm.zig:4318-4322`, `src/lower_llvm.zig:4422-4450`). This is the recommended Phase 2 migration target because the facts are not produced by sema/MIR and are not shared with C. |
