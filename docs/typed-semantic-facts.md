...
Array, slice, enum, tagged-union, Result, and nullable direct-call inferred
locals now consume the MIR-owned `direct_call_result` fact before validating the
owned `inferred_local` row. Qualified tagged-union constructor inferred locals
