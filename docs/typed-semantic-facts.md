...
output, and options; it carries no `EarlyDeclarationArtifacts`. Source-map rows
come from syntax-free `Module.source_map_declarations` facts admitted with the
same MIR as lowering. The separate `EmitMapRequest` carries no declaration
slice or artifact array, so source-map mechanics cannot select ordinary
codegen semantics.
...
typed semantic source of truth. Semantic authority is the verified MIR facts
and their `TypeId`/`SpanId`/`SymbolId` identity tables; both backends consume
those directly. The retired `src/mir_facts_view.zig` and
`src/mir_source_bridge.zig` compatibility layers cannot reintroduce
source-span, owner-spelling, or module-wide fallback queries. Missing or stale
facts are rejected by MIR admission.

- `mcc facts` parses a module and prints `src/ir_inspection.zig`'s fact collector output via
...
- `mcc inspect-ir` prints the same `src/ir_inspection.zig` model as a compact textual
...
- Large classes of backend behavior still carry semantic-adjacent emission
  ...
  `src/lower_c_emitter.zig`, `src/lower_c_shape.zig`, and related helpers; the
...
7. Textual artifacts stay debug surfaces. `mcc facts`, `inspect-ir`, and
   `lower-mir` may print facts for humans/tests, but backend lowering consumes
...
- `src/ir_inspection.zig`: `appendFacts`, `appendLowerIr`, trap/contract/MMIO/race text;
...
|---|---|---|---|---|
| MIR explicit-trap call facts | `explicitTrapCallTargetKind` resolves the eight accepted `trap(.Reason)` forms into exact identities with `never` result type. Sema rejects type arguments, wrong arity, and unknown or non-literal reasons before MIR construction. The executable-body projection converts the typed target identity into an exact `TrapKind`; terminal explicit traps and `unreachable` become canonical CFG terminators. | Generic call-target validation requires one matching identity instruction/fact pair. A stale reason identity fails validation even though the compatibility control-flow edge remains generically classified as `.ExplicitTrap`. The executable-body verifier requires the source block to jump to the exact terminal trap block and validates its typed span identity. | `lower-mir` prints the exact trap identity at the call source point. | C and LLVM mechanically render the executable `TrapKind`; they no longer select the explicit-trap helper by scanning the source expression. Missing or stale facts reject prebuilt MIR, and the legacy `simple_trap` plans have been removed from both backends. |
...
| MIR direct, indirect, and dynamic-dispatch call type facts | An identifier-form call resolved to a declared ordinary function or extern records `direct_call_result` at the callee source point and `direct_call_argument` at each argument source point. Fixed-argument types are copied from the sema-visible declaration summary; variadic tail argument types are copied from the already-resolved expression type. A function-pointer or closure call records one complete `indirect_call_callee` signature fact at its callee source point. For a bounded direct-identifier argument call whose callee is a parameter, global function pointer, or global struct-field function pointer, MIR also records indexed `indirect_call_argument` rows and canonical callee-root identity plus optional field projection. A `*dyn Trait` call records `dyn_dispatch_argument` for every fixed user argument; non-`void` calls additionally record `dyn_dispatch_result`. Dynamic facts are owned by trait name and vtable slot/argument identity, while member calls remain excluded from direct facts so they cannot bind to an unrelated same-named free-function summary. | Generic target-type validation requires one matching complete fact per metadata instruction. Direct-call facts additionally carry a callee owner and optional fixed-argument index; each indirect-argument row carries the exact callee `SpanId`, argument index, and operand `ValueId`; dynamic-dispatch facts carry the trait owner and vtable slot/argument identity. This prevents generated calls with identical source coordinates from cross-matching. Ordinary source call-target facts carry the exact callee span, including offset/length, rather than only the whole call expression's line/column; this prevents nested same-start calls such as `read_state().raw()` from cross-consuming the outer member-call target kind. C and LLVM additionally compare consumed fixed direct/dynamic facts with the declaration or trait signature, using those only as ABI/stale-fact checks rather than missing-fact fallbacks; variadic direct tail facts have no declared parameter to compare against. | `lower-mir` prints `target_owner`, `target_index`, `typed_callee_span_id`, and `typed_operand_value_id` on applicable rows and prints the complete indirect callee signature row with the other target facts. | C and LLVM consume direct facts, including variadic direct-call tail argument facts, in ordinary, inferred-result, sequenced, extern-nonnull, value, and void/never paths. Their checked-MIR function-pointer and closure paths consume `indirect_call_callee`; a shared MIR plan additionally consumes the indexed operand and callee-root facts for parameter/global/global-field indirect calls returned immediately. Dynamic-dispatch paths consume `dyn_dispatch_argument` plus `dyn_dispatch_result` when applicable. Missing facts fail MIR admission; stale complete types, invalid typed operand identities, and malformed stale indirect signatures fail admission. Non-leaf indirect arguments, non-dynamic member calls, and variadic dynamic dispatch remain outside this bounded migration. LLVM's `expectedTyForCallArg` and direct `fn_sigs` return fallback are exact-zero gated; the async generated-source collision fixture gates owner-aware identity, and the MIR test gates same-named member exclusion. |
| Text IR inspection facts | `src/ir_inspection.zig:510` `Collector.appendFacts`, `src/ir_inspection.zig:525` `appendFacts`, and `src/ir_inspection.zig:590` `ModuleFactCollector.appendFacts` walk the AST and emit line-oriented `fact ...` rows for trap edges, `#[no_lang_trap]`, unsafe contracts, unchecked calls, ordinary/racing access, MMIO calls/access/order, and direct MMIO assignment. | AST-derived only; there is no typed in-memory table behind this surface. MMIO global/parameter discovery is held in `ModuleFactCollector.globals` / `mmio_structs` (`src/ir_inspection.zig:571-576`) and helper lookups such as `mmioAccess` (`src/ir_inspection.zig:664`) and `mmioRegisterTarget` (`src/ir_inspection.zig:746`). | `mcc facts` dispatches through `src/main.zig:291` and prints `ir.appendFacts` at `src/main.zig:645-648`. Representative rows include `fact checked_arithmetic_trap` (`src/ir_inspection.zig:1059`), `fact ordinary_access` (`src/ir_inspection.zig:1116`), `fact racing_load_semantics` (`src/ir_inspection.zig:1121`), `fact non_atomic_rmw` (`src/ir_inspection.zig:1165`), and `fact mmio_access` (`src/ir_inspection.zig:1199`). | No backend consumes this text. It is evidence for tests/spec fixtures and must not become a reparsed backend input. |
| Inspect-IR contract/trap artifact | `src/ir_inspection.zig:128` `appendLowerIr` builds `IrFunction` records with `trap_edges`, `safe_no_trap_ops`, `contract_regions`, and `unchecked_calls`; `FunctionIrBuilder.collectContractBlock` records contract regions at `src/ir_inspection.zig:378`. | Textual inspection model only; contract activity is tracked by `FunctionIrBuilder.active_contract` / `active_contract_region_id` (`src/ir_inspection.zig:197-199`) while walking the AST. | `mcc inspect-ir` dispatches through `src/main.zig` and prints `ir.appendLowerIr`. It emits `ir contract_region`, `ir unchecked_call`, `ir trap_edge`, `ir post_contract_trap_edge`, and `ir safe_no_trap` rows (`src/ir_inspection.zig:137-180`). | No backend consumes this artifact. Contract scope also has a C inspection artifact in `src/c_inspection.zig`, but that is an inspection artifact, not the source of backend semantic authority. |
| MIR typed instruction metadata | `src/mir_model.zig` `ValueType`, `Instruction`, and optional `value_id` / `contract_region_id` / `const_index` carry typed operation metadata. `Instruction.typed_span_id` is the sole instruction source identity; diagnostics, dumps, and source maps resolve coordinates through the owning `SpanIdentity` table. Generated `TypeExpr` nodes and generic argument slices are builder-local and released before a `Function` is admitted. | `CallTargetFact` records bounded builtin/call identities, including closure bind, Result constructors, reductions, enum representation reads, arithmetic-domain calls, `const_get`, DMA calls, raw-many pointer offsets, MMIO mapping and register reads/writes, raw memory, varargs, reflection, byte-view, semantic-escape, atomic initialization/member operations, MaybeUninit operations, and scalar/domain conversion result types. Call-target facts use their opaque `SpanId` as their only source identity; call groups retain their own kind/result/multiplicity semantics. `TargetTypeFact` records its complete constructor/literal/expression target shape as a module-owned `SignatureTypeId`, never as an AST `TypeExpr`; C/LLVM can materialize a transient rendering view only from the verified signature table. It covers constructors, contextual literals/aggregates, self-typed qualified-union and enum-path expressions, member/index/slice/dereference and unary/binary expression results, mapped-try error expressions, null/optional/dyn coercions, scalar/domain conversion source/targets, reduction elements, enum raw source/results, arithmetic-domain source/payload/result/interval types, const-get base/results, DMA buffer/payload/results, raw-many offset receiver/element/results, MMIO-map source/payload/results, MMIO register struct/storage/value/results, raw-memory address/payload/results, varargs cursor/payload/results, byte-view source/results, reflection target/results, explicit-cast source/targets, implicit single-pointer/slice const-narrowing source/targets, semantic-escape source/results, atomic-init payload/results, and atomic/MaybeUninit member payloads. Assignment, target-typed constructor, and qualified tagged-union constructor call instructions carry the resolved MIR-owned lowering type instead of `.unknown`. `expression_result` rows use the full source span, not just line/column, so nested expressions on one line cannot cross-match. Explicit casts, semantic escapes, MMIO mapping, and varargs cursors cover generated type nodes with explicit MIR lifetime ownership; domain calls and scalar `try_from` conversion call targets similarly own generated `Result` shapes. Atomic-init payload/result pairs use a unique owner/index group so same-source generated calls cannot cross-match. A `? else MAPPED` expression is built with the enclosing `Result<T,E>` error type, so target-typed mapped enum literals no longer depend on C inference. Implicit const narrowing emits paired facts only for `*mut T` to `*const T` and `[]mut T` to `[]const T`; pass-through values and raw-many pointers emit none. Metadata/fact multiplicities must match and same-key complete types must be structurally equal. | `mcc lower-mir` prints function summaries plus owned target rows. | `VerifiedProgram.init` validates lowering admission before C/LLVM emission; call-target, target-type, representation, integer, range, function-return, terminator, and all runtime/fact-bearing instruction type positions reject `.unknown` type placeholders. Diagnostic-only check instructions remain the explicit `.unknown` allowlist. Closure bind, Result constructors, reductions, enum representation reads, arithmetic-domain calls, `const_get`, DMA calls, raw-many pointer offsets, MMIO mapping/register calls, conversion, explicit-cast, mapped-try, self-typed expressions, member/index/slice/dereference and unary/binary type queries, implicit view const-narrowing, semantic escapes, atomic initialization/member operations, MaybeUninit, raw memory, varargs, reflection, and byte-view lowering consume exact MIR identities/types where migrated; removing any `expression_result` row rejects prebuilt MIR. Broader direct expression emission remains a registered migration boundary. |
| MIR no-overflow range facts | `src/mir_model.zig:201` `RangeFact` records `region_id`, `target`, `op`, operands, result type, and opaque `typed_span_id`; display coordinates resolve through the owning function span table. Producers are `FunctionBuilder.addRangeFactForUncheckedCall` (`src/mir.zig`) and `addAggregateRangeFactForUncheckedExpr` (`src/mir.zig`) inside active `#[unsafe_contract(no_overflow)]` regions. | These facts are appended for unchecked no-overflow calls in contract regions. They are not the same as transient optimizer `proven_facts`; they persist in `Function.range_facts` until the MIR module is freed. Admission rejects a missing, invalid, or unmatched SpanId before either backend starts lowering. | `appendDumpOpt`, verification output, and the C map resolve display coordinates through `SpanId`; LLVM emission prints non-semantic `; mir range_fact consumed ... assumption=no_overflow` comments when it consumes a fact; C emission prints `/* MC_MIR_RANGE no_overflow ... */` comments from its MIR-gated unchecked paths. The inventory checker anchors the owned `RangeFact` model, the dump row, and exact C/LLVM no-overflow range consumer call counts. | The C backend wires `has_mir_no_overflow_range_fact` through `arithContext` and matches facts in `hasMirNoOverflowRangeFact`; its generic builtin dispatcher rejects unchecked no-overflow calls instead of emitting plain arithmetic without a matching fact. C and LLVM both have target-label coverage for return values, inferred locals, assignments, call arguments, binary operands, array literal elements, and struct literal fields. LLVM requires `requireMirNoOverflowRangeFact` for `unchecked.add/sub/mul` and matches target identity through `current_mir_range_target` for those contexts. Removing a matching `RangeFact`, retargeting it to a different target label, or corrupting its SpanId in prebuilt MIR makes both C and LLVM fail closed instead of trusting the AST call, including non-`value` inferred-local, assignment, call-argument, binary-operand, aggregate-element, and aggregate-field targets; absence coverage explicitly spans those non-`value` contexts in both backends. Broader range/bounds facts are still not a complete typed fact family. |
| MIR bounds facts | `src/mir_model.zig` `BoundsFact` records legacy index and slice check evidence by the owning function's canonical `SpanId`; display coordinates are decoded through that table. | Optimized proofs remain represented by `elided_bounds`. The canonical executable body owns complete bounds/trap semantics; when a legacy bounds fact is present, admission requires one matching resolved access fact and rejects invalid, stale, or duplicate facts. | `mcc lower-mir` prints the function `bounds_facts=N` count and stable `mir bounds_fact fn=... kind=index|slice recorded=true line=... column=...` rows. The inventory checker anchors the owned `BoundsFact` model, dump row, and C/LLVM `requireMirBoundsFact` consumers with exact call counts. | Canonical C/LLVM lowering consumes checked executable operations directly. Legacy AST-shaped paths convert their requested span to `SpanId` before calling `requireMirBoundsFact`; no consumer matches fact source coordinates. |
| MIR integer literal facts | `src/mir_model.zig` `IntegerFact` records accepted target-typed integer literal conversion with literal text, canonical `TypeId`, and canonical `SpanId`. `FunctionBuilder.addIntegerLiteralFact` appends an `integer_literal_conversion` metadata instruction plus the owned fact when `integerLiteralFitsTarget` accepts a conversion. The fact duplicates neither `ValueType` nor `SourcePoint`; consumers resolve type and display source through the owning function's identity tables. | These facts are admission evidence for target-typed integer literal lowering. Out-of-range literals still produce MIR conversion diagnostics; accepted literals now have a positive MIR row instead of leaving backends to trust only AST target context. | `mcc lower-mir` prints `integer_facts=N` and stable `mir integer_fact fn=... literal=... target_type=... target_type_id=... typed_span_id=... recorded=true` rows. The inventory checker anchors the model, producer, dump row, unified verified-program admission call, and missing/stale tests. | `VerifiedProgram.init` calls `validateLoweringAdmission`, which includes `validateIntegerFactsForLowering` and resolves the fact type through `TypeId` before either backend lowers. Missing, unmatched, invalid, or retargeted integer facts return `InvalidMirIntegerFacts` before backend emission. This is a bounded integer/default fact slice for target-typed integer literals, not full typed HIR replacement. |
...
| `c-type-shape-classification` | `src/lower_c_shape.zig` inspection/type-shape helpers; `src/mir_executable_c.zig` renders verified value shapes | C declaration mechanics and inspection output | The old AST local/global body classification module is deleted. Complete C function bodies consume executable MIR value shapes; declaration mechanics retain only the bounded shape handling needed to render admitted aggregate definitions. C aggregate-global representation is an accepted internal target policy. | Unknown aggregate/scalar/race-helper shapes must reject or use conservative recursive lowering. Missing migrated facts reject prebuilt MIR. |
| `c-abi-aggregate-lowering` | `src/lower_c_aggregate_deps.zig` declaration dependency ordering; `src/sema.zig` `checkExternExportStructAbi`; `src/lower_llvm.zig` `cAbiExtension` | C aggregate declarations and explicit C ABI boundary admission | Complete C function bodies no longer lower source array, struct, Result, or tagged-union constructors through an AST helper. The source struct literals carry a MIR-owned construction class; aggregate values are executable-MIR operations, while the C declaration pass retains only dependency ordering for already-admitted aggregate definitions. Explicit `extern "C"` declarations and unmarked exports reject every currently unclassified by-value family. Bare `extern fn` and `#[mc_abi]` exports remain backend-private. LLVM C ABI scalar definitions, declarations, and direct calls share target-aware extension attributes. | Unsupported internal aggregate forms fail backend emission; `E_EXTERN_STRUCT_BY_VALUE` requires pointers/out parameters for unclassified C ABI values until per-target aggregate classification exists. |
| `c-call-target-classification` | `src/mir_executable_c.zig` renders typed executable call, atomic, raw-memory, byte-view, DMA, fence, conversion, and domain operations; `src/lower_c_collect.zig` / `src/lower_c_emitter.zig` consume declaration and signature facts; `src/lower_c_mmio_defs.zig` retains MMIO declaration layout mechanics | C call emission and special builtin lowering | **MIR-owned; no longer in the backend AST-inference budget.** Complete function bodies are rendered only from executable MIR. The retired AST builtin dispatcher, byte-view/DMA/MaybeUninit body emitter, atomic member emitter, reflection body emitter, conversion emitter, domain emitter, MMIO emitter, and control-flow helper stack are deleted; atomic/MMIO ordering, raw-memory, DMA, byte-view, semantic escapes, conversions, and domain identities reach C codegen only through verified executable operations and facts, while reflection is folded into those verified MIR values before C rendering. | Unknown or unsupported executable operations reject lowering; no provenance/range fact may be inferred from a call spelling. Missing, stale, or `.unknown` typed facts for migrated categories reject prebuilt MIR before emission. |
...
| `mir-bounds-range-producers` | `src/mir.zig` no-overflow range, bounds, and elided-check producers | C/LLVM range/bounds/check-elision consumers | MIR-owned for current bounded subset. | Missing optimization proof keeps runtime check; unchecked no-overflow consumption requires a matching fact. |
...
| Stable identity key | Facts match by instruction kind, result type, opaque `SpanId`, detail, and `ValueId`; the value spelling and display coordinates are resolved only through the owning function's identity tables. |
...
The backend source surface is now an exact inventory, excluding
...
aggregate fields/elements. Both backends consume that family as an
...
clear implementation value and a visible duplication problem.
...
narrow fact families described above. Current backends still use
...
- Do not replace the parser, AST, HIR, MIR, and all backend type queries in one
  change.
- Do not make `mcc facts` or `inspect-ir` the backend input format.
