...
The latest bounded call-identity migration adds exact `result_ok` / `result_err`
rows beside the existing contextual Result target-type facts. C normal emission,
C Result-try hoisting, and LLVM value emission consume those identities and MIR
types; none calls `resultConstructorCallTag`. Missing identities fail MIR
admission for ordinary and `?`-hoisted constructors. AST remains only in the
semantic/MIR producer and for extracting an already-confirmed payload operand.
Call-target admission compares matching instruction/fact multiplicities so
async-generated constructors that share `(0,0)` remain valid, while one-sided
duplicates and different call kinds at one source point fail closed.
