...
| MIR inferred local-copy/cast type facts | For a one-name unannotated local initialized directly from an existing local value or an explicit `value as T` cast, MIR records an owned `inferred_local` target-type fact keyed by the binding name and initializer source point. | Generic target-type validation matches the owned fact and instruction. C and LLVM compare only an already-known local/cast type to reject stale prebuilt MIR; the fact selects the destination binding type. | `lower-mir` prints the owner alongside the target-type row. | C uses the fact for inferred local-copy and direct-cast declarations; LLVM uses it before allocating either binding. Removing the fact fails MIR admission and retargeting it fails backend emission. Literals, calls, and computed initializers remain in the registered broader expression-inference boundary. |
