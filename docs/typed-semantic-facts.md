...
The latest bounded call/type migration gives `wrap<T>.residue()` and every
serial/counter operation an exact identity plus complete domain, payload,
result, and where applicable interval types. C and LLVM now use MIR instead of
resolving domain aliases or rebuilding `wrap`, `Duration`, and `Result` types.
Missing or stale facts fail MIR admission. This closes arithmetic-domain calls,
not the broader call-target or expression-type inference families.
...
| `c-call-target-classification` | `src/lower_c_call.zig` sequenced/bitcast/extern-nonnull call emitters plus MIR-gated semantic escapes; `src/lower_c_collect.zig` / `src/lower_c_emitter.zig` bind and reduction consumers; `src/lower_c_builtin_emit.zig` enum raw consumer; `src/lower_c_arith.zig` / `src/lower_c_domain.zig` arithmetic-domain consumers; `src/lower_c_emitter.zig` / `src/lower_c_try.zig` Result consumers; `src/lower_c_atomic.zig` / `src/lower_c_memory.zig` MIR payload consumers; `src/lower_c_builtin.zig` builtin classifiers; `src/lower_c_reflect.zig` MIR-gated reflection emission; `src/lower_c_convert.zig` MIR-gated conversion emission | C call emission and special builtin lowering | Registered backend inference. Closure bind, Result constructors, reductions, enum representation reads, arithmetic-domain calls, bitcast, physical-address, reflection, byte-view, semantic escapes, atomic/MaybeUninit, raw memory operations, varargs operations, and all six scalar/domain conversion operations are MIR-owned call identities. Their migrated source/target/result/payload/element/interval types are MIR-owned through `TargetTypeFact` rows; C uses AST only to validate confirmed operands and select emission mechanics. Other registered call categories remain to migrate. | Unknown call targets stay ordinary calls or unsupported; no provenance/range fact may be inferred from a call spelling. Missing or stale facts for migrated categories reject prebuilt MIR before emission. |
