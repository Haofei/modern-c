...
| MIR direct address result facts | A direct `&function` expression records a complete `expression_result` function-pointer signature from the resolved function summary. A bounded direct data-address expression records `*const T` or `*mut T` from the established local/mutable-global place and mutability rule; admitted field/fixed-array projections and direct local pointer/raw-many-pointer pointees use the same rule. Immutable globals, slice indexes, pointer chains, and call results remain factless. | Generic target-type validation requires the source-span instruction/fact pair. Data-address retargeting is rejected by the existing C/LLVM admitted-place checks before emission. | `lower-mir` prints the owned `expression_result` row. | C and LLVM consume the fact for code-pointer and bounded data-pointer typing; missing facts fail admission and stale facts fail backend lowering. |
