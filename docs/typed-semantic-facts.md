...
The latest bounded call/type migration gives raw-many pointer `.offset(index)`
an exact call identity and complete receiver/element/result types. C and LLVM
no longer infer this operation from backend-local receiver typing or alias
resolution. Missing or stale facts fail MIR admission. This closes the raw-many
offset operation family, not the broader pointer-provenance, call-target, or
expression-type inference families; nonzero and dynamic offsets intentionally
retain conservative storage provenance.
...
| `c-call-target-classification` | `src/lower_c_call.zig` sequenced/bitcast/extern-nonnull call emitters plus MIR-gated semantic escapes; `src/lower_c_collect.zig` / `src/lower_c_emitter.zig` bind and reduction consumers; `src/lower_c_builtin_emit.zig` enum raw consumer; `src/lower_c_arith.zig` / `src/lower_c_domain.zig` arithmetic-domain consumers; `src/lower_c_access.zig` const-get and raw-many offset consumers; `src/lower_c_emitter.zig` / `src/lower_c_try.zig` Result consumers; `src/lower_c_atomic.zig` / `src/lower_c_memory.zig` MIR payload and DMA consumers; `src/lower_c_builtin.zig` builtin classifiers; `src/lower_c_reflect.zig` MIR-gated reflection emission; `src/lower_c_convert.zig` MIR-gated conversion emission | C call emission and special builtin lowering | Registered backend inference. Closure bind, Result constructors, reductions, enum representation reads, arithmetic-domain calls, `const_get`, DMA calls, raw-many pointer offsets, bitcast, physical-address, reflection, byte-view, semantic escapes, atomic/MaybeUninit, raw memory operations, varargs operations, and all six scalar/domain conversion operations are MIR-owned call identities. Their migrated source/target/result/payload/element/interval types and const-get index are MIR-owned; C uses AST only to validate confirmed operands and select emission mechanics. Other registered call categories remain to migrate. | Unknown call targets stay ordinary calls or unsupported; no provenance/range fact may be inferred from a call spelling. Missing or stale facts for migrated categories reject prebuilt MIR before emission. |
