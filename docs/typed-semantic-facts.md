...
| `llvm-expression-type-inference` | `src/lower_llvm.zig` `exprType` / `derefPointeeType`; shared `src/ast_query.zig` `qualifiedTaggedUnionConstructorType`, `enumVariantPathType`, `rawLoadCallReturnType`, `rawPtrCallReturnType`, `bitcastCallReturnType`, `vaCallReturnType`, `byteViewCallReturnType`, `reflectionValueCallReturnType`, `isDeclassifyCall`, `resultConstructorCallTag`, `isPhysCall`, `isBindCallNode`, and `isBindCallExpr` | LLVM expression, call, deref, constructor, and MMIO emission | Registered backend inference; qualified tagged-union constructor typing, enum variant-path typing, raw load/pointer builtin result typing, bitcast result typing, value-producing varargs builtin result typing, byte-view builtin result typing, value-producing reflection result typing, declassify/reveal call recognition, Result constructor call recognition, physical-address constructor recognition, and closure bind call recognition use shared AST queries. Bounded `reduce`, `atomic`, and `MaybeUninit` classification comes from MIR `CallTargetFact.kind`; `const_get` uses its AST query only to extract confirmed operands. The inventory rejects reintroducing the retired LLVM AST call-target classifiers. The broader family still needs typed facts/MIR migration. | Unsupported or unknown shapes return null/unsupported and must not infer facts beyond sema-accepted source meaning. |
