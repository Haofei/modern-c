...
| MIR typed instruction metadata | `src/mir_model.zig` `ValueType`, `Instruction`, and optional `value_id` / `contract_region_id` carry typed operation metadata. `Function` owns `contract_regions`, `range_facts`, `call_target_facts`, `target_type_facts`, `pointer_provenance_facts`, `representation_facts`, and `elided_bounds`. | `CallTargetFact` records bounded builtin/call identities. `TargetTypeFact` records the complete target `ast.TypeExpr` for `bind`, Result `ok`/`err`, bare tagged-union constructors, null literals, non-null value-to-optional and concrete-pointer-to-dyn coercions, enum/string/float literals, and array/struct literals including packed bits. Already-optional and already-dyn pass-through values do not produce coercion facts; optional aggregate payloads retain both wrapper and child aggregate targets. Float targets preserve f32-vs-f64 operation/literal semantics. Propagation covers global/local/assignment/return/direct-call/builtin-member-call/nested-aggregate contexts, Result/union/coercion payloads, cast operands, comparisons, and contextual binary operands. Metadata/fact multiplicities must match and same-key complete types must be structurally equal. Static-initializer alias expansion may reuse agreeing non-synthetic source facts across MIR owners; `(0,0)` facts remain owner-local. | `mcc lower-mir` prints function summaries plus owned target rows. | C and LLVM validate before emission. Runtime/global null, optional, dyn, aggregate, and literal lowering plus C's unchecked sequencing consume target facts; broader semantic decisions remain in registered fallback families. |
...
| `c-call-target-classification` | `src/lower_c_call.zig` sequenced/bitcast/extern-nonnull call emitters plus MIR-gated semantic escapes; `src/lower_c_builtin.zig` builtin classifiers; `src/lower_c_reflect.zig` MIR-gated reflection emission; `src/lower_c_memory.zig` MIR-gated byte-view emission; `src/lower_c_convert.zig` MIR-gated conversion emission | C call emission and special builtin lowering | Registered backend inference. The five value-producing reflection intrinsics, both byte-view intrinsics, `declassify`/`reveal`, `compiler.assume_noalias_unchecked`, and all six scalar/domain conversion operations are MIR-owned call identities: C uses the AST only to extract operands and select mechanics after an exact `CallTargetFact.kind` match. Conversion source/target representation typing and other registered call categories remain to migrate. | Unknown call targets stay ordinary calls or unsupported; no provenance/range fact may be inferred from a call spelling. Missing or stale facts for migrated categories reject prebuilt MIR before emission. |
