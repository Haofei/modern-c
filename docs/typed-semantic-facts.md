...
joins before a final return, and multiple sequential CFG joins before a final
...
4. Complete for named unsupported producer shapes: nested control flow, loop
   prefixes, `for` prefixes, deferred cleanup prefixes, `if` joins before a
   final return, sequential CFG joins, exported aggregate returns, mixed paths,
