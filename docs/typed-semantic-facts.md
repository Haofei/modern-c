...
| `llvm-expression-type-inference` | `src/lower_llvm.zig` `exprType` / `derefPointeeType`; shared `src/ast_query.zig` expression/call-shape queries | LLVM expression, call, deref, constructor, and MMIO emission | Registered backend inference. `bind`, Result `ok`/`err`, and bare tagged-union constructors use AST only to extract the confirmed operand/case shape; their complete target type and constructor identity come from MIR `TargetTypeFact`. The former LLVM `bindClosureType` reconstruction and expected-type bare-union fallback are retired. Qualified self-typed union constructors remain an explicit AST-owned form. Reflection, byte-view, semantic escapes and bounded builtin families retain their MIR call-target gates; enum paths, raw/varargs and broader expression typing remain open. | Unsupported or unknown shapes return null/unsupported. Missing/stale target-type or migrated call-target facts reject prebuilt MIR before emission. |
