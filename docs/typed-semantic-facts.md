...
The latest bounded call/type migration gives both byte-view operations complete
source/result types. C no longer asks its expression inference callbacks for the
as-bytes object type, and LLVM no longer calls `exprType` or synthesizes a local
byte-slice type for either operation. Missing identities or types fail MIR
admission. This closes the byte-view operation family, not the broader
call-target, expression-type, pointer-provenance, or typed-HIR/MIR workstreams.
