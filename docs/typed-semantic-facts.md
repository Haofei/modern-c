...
| `llvm-expression-type-inference` | `src/lower_llvm.zig` `exprType` / `derefPointeeType`; shared `src/ast_query.zig` `qualifiedTaggedUnionConstructorType`, `enumVariantPathType`, `rawLoadCallReturnType`, `rawPtrCallReturnType`, `bitcastCallReturnType`, and `vaCallReturnType` | LLVM expression, call, deref, constructor, and MMIO emission | Registered backend inference; qualified tagged-union constructor typing, enum variant-path typing, raw load/pointer builtin result typing, bitcast result typing, and value-producing varargs builtin result typing now use shared AST queries instead of backend-local classifiers, and the inventory forbids local classifier reintroduction where retired, but the broader family still needs typed facts/MIR migration. | Unsupported or unknown shapes return null/unsupported and must not infer facts beyond sema-accepted source meaning. |
