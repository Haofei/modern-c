...
|---|---|---|---|---|
| `c-expression-type-inference` | `src/lower_c_infer.zig` `operandEmitType`, `derefPointeeType`, return/type classifiers | C expression, aggregate, call, and deref emission | Registered backend inference; migrate high-risk lowering decisions into typed facts as families are touched. | Unsupported or unknown shapes return null/unsupported and must not invent a semantic fact. |
...
| `llvm-expression-type-inference` | `src/lower_llvm.zig` `exprType` / `derefPointeeType`; shared `src/ast_query.zig` `qualifiedTaggedUnionConstructorType` | LLVM expression, call, deref, constructor, and MMIO emission | Registered backend inference; qualified tagged-union constructor typing now uses the shared AST query instead of an LLVM-local classifier, but the broader family still needs typed facts/MIR migration. | Unsupported or unknown shapes return null/unsupported and must not infer facts beyond sema-accepted source meaning. |
