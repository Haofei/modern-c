...
The latest bounded call/type migration gives function-body calls to all five
value-producing reflection operations complete target/result types. C call
emission no longer reads the type argument as semantic authority, and LLVM call
emission no longer hands the AST call to its backend-local reflection evaluator.
Missing identities or types fail MIR admission. Type-level reflection used by
backend array-length/layout folding remains open because those expressions do
not yet enter function MIR; the broader representation, expression-type,
pointer-provenance, and typed-HIR/MIR workstreams therefore remain open.
