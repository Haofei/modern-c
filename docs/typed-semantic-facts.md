...
| MIR inferred local copy/call/cast/unary/binary/literal type facts | For a one-name unannotated local initialized directly from an existing local value, an ordinary direct function/extern call, a function-pointer or closure call with a complete `indirect_call_callee` signature, an explicit `value as T` cast, a typed unary/binary expression, or a targetless integer (`u32`) / bool literal, MIR records an owned `inferred_local` target-type fact keyed by the binding name and initializer source point. | Generic target-type validation matches the owned fact and instruction. C and LLVM compare the sema-defined local/direct-call/indirect-signature/cast/unary/binary/literal type to reject stale prebuilt MIR; the fact selects the destination binding type. | `lower-mir` prints the owner alongside the target-type row. | C uses the fact for inferred local-copy, direct/indirect call, direct-cast, checked-unary, checked-arithmetic, comparison, short-circuit logical, integer-literal, and bool-literal declarations; LLVM uses it before allocating each binding. Removing the fact fails MIR admission and retargeting it fails backend emission. Builtins, member/dynamic calls, and broader computed initializers remain in the registered broader expression-inference boundary. |
...
| MIR direct and indirect call type facts | An identifier-form call resolved to a declared ordinary function or extern records `direct_call_result` at the callee source point and `direct_call_argument` at each fixed argument source point. Types are copied from the sema-visible declaration summary; a variadic tail intentionally has no target fact. A function-pointer or closure call records one complete `indirect_call_callee` signature fact at its callee source point. Checked C consumes it for closure dispatch and indirect result typing; LLVM also uses it to emit the indirect-call parameter types. Member calls are excluded from direct facts so they cannot bind to an unrelated same-named free-function summary; dynamic dispatch remains separate. | Generic target-type validation requires one matching complete fact per metadata instruction. Direct-call facts additionally carry a callee owner and optional fixed-argument index; ordinary target queries exclude owned facts, and owned queries match both fields. This prevents generated calls with identical source coordinates from cross-matching. C and LLVM additionally compare each consumed direct fact with the declaration signature, using the declaration only as an ABI/stale-fact check rather than a missing-fact fallback. | `lower-mir` prints `target_owner` and `target_index` on direct result/fixed-argument rows and prints the complete indirect callee signature row with the other target facts. | C and LLVM consume direct facts in ordinary, inferred-result, sequenced, extern-nonnull, value, and void/never paths; their checked-MIR function-pointer and closure paths consume `indirect_call_callee` instead of backend expression-type inference. Missing direct or indirect facts fail MIR admission; stale direct complete types and malformed stale indirect signatures fail backend admission. LLVM's `expectedTyForCallArg` and direct `fn_sigs` return fallback are exact-zero gated; the async generated-source collision fixture gates owner-aware identity, and the MIR test gates same-named member exclusion. |
