...
The latest bounded call-type migration adds paired source/result facts for
`declassify` and `compiler.assume_noalias_unchecked`. MIR owns generated
address-of pointer type nodes, C no longer infers noalias return types from the
first argument, and LLVM no longer derives either semantic escape's source or
result type through `exprType`. Missing rows fail MIR admission, and exact-count
inventory gates prevent the retired fallbacks from returning.
...
| `c-call-target-classification` | `src/lower_c_call.zig` sequenced/bitcast/extern-nonnull call emitters plus MIR-gated semantic escapes; `src/lower_c_builtin.zig` builtin classifiers; `src/lower_c_reflect.zig` MIR-gated reflection emission; `src/lower_c_memory.zig` MIR-gated byte-view emission; `src/lower_c_convert.zig` MIR-gated conversion emission | C call emission and special builtin lowering | Registered backend inference. Bitcast, physical-address, reflection, byte-view, semantic escapes, raw memory operations, varargs operations, and all six scalar/domain conversion operations are MIR-owned call identities. Bitcast source/target, physical-address result, semantic-escape source/results, conversion source/target, and value-producing raw/varargs/reflection/byte-view result types are MIR-owned through `TargetTypeFact` rows; C uses AST only to validate confirmed operand counts and select mechanics. Other registered call categories remain to migrate. | Unknown call targets stay ordinary calls or unsupported; no provenance/range fact may be inferred from a call spelling. Missing or stale facts for migrated categories reject prebuilt MIR before emission. |
...
| `llvm-expression-type-inference` | `src/lower_llvm.zig` `exprType` / `derefPointeeType`; shared `src/ast_query.zig` expression/call-shape queries | LLVM expression, call, deref, constructor, and MMIO emission | Registered backend inference. Constructors, contextual coercions/literals/aggregates, conversion and bitcast builtins, physical-address results, semantic-escape source/results, explicit casts, implicit single-pointer/slice const narrowing, raw/varargs/reflection/byte-view results, qualified tagged-union constructors, and enum variant paths now obtain complete semantic types from MIR facts. Runtime/global cast lowering, cast-result queries, semantic-escape emission/typing, implicit const-narrowing coercion, migrated `callReturnType` paths, and self-typed union/enum paths no longer infer semantic result types from AST. Broader expression typing remains open. | Unsupported or unknown shapes return null/unsupported. Missing/stale target-type or migrated call-target facts reject prebuilt MIR before emission. |
