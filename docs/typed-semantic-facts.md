...
| MIR typed instruction metadata | `src/mir_model.zig` `ValueType`, `Instruction`, and optional `value_id` / `contract_region_id` carry typed operation metadata. `Function` owns the typed fact tables and any generated `TypeExpr` child nodes referenced by them. | `CallTargetFact` records bounded builtin/call identities, including closure bind, Result constructors, reductions, enum representation reads, raw memory, varargs, reflection, byte-view, semantic-escape, atomic, and MaybeUninit operations. `TargetTypeFact` records complete types for constructors, contextual literals/aggregates, self-typed qualified-union and enum-path expressions, mapped-try error expressions, null/optional/dyn coercions, scalar/domain conversion source/targets, reduction elements, enum raw source/results, explicit-cast source/targets, implicit single-pointer/slice const-narrowing source/targets, semantic-escape source/results, atomic/MaybeUninit payloads, and raw/varargs/reflection/byte-view value-producing call results. Explicit casts and semantic escapes cover generated address-of pointer types with explicit MIR lifetime ownership. A `? else MAPPED` expression is built with the enclosing `Result<T,E>` error type, so target-typed mapped enum literals no longer depend on C inference. Implicit const narrowing emits paired facts only for `*mut T` to `*const T` and `[]mut T` to `[]const T`; pass-through values and raw-many pointers emit none. Metadata/fact multiplicities must match and same-key complete types must be structurally equal. | `mcc lower-mir` prints function summaries plus owned target rows. | C and LLVM validate before emission. Closure bind, Result constructors, reductions, enum representation reads, conversion, explicit-cast, mapped-try, self-typed expressions, implicit view const-narrowing, semantic escapes, atomic/MaybeUninit, raw memory, varargs, reflection, and byte-view lowering consume exact MIR identities/types; removing migrated call or result facts rejects prebuilt MIR instead of restoring backend source-type inference. Broader semantic decisions remain in registered fallback families. |
...
The latest bounded call/type migration gives enum `.raw()` an exact identity and
complete source/result types. C and LLVM now use MIR for receiver and repr type
authority across closed, open, default-repr, and variant-path forms. Backend
enum lookup remains only for unrelated enum layout/case mechanics. Missing facts
fail MIR admission. This closes enum representation reads, not the broader
call-target or expression-type inference families.
...
| `c-call-target-classification` | `src/lower_c_call.zig` sequenced/bitcast/extern-nonnull call emitters plus MIR-gated semantic escapes; `src/lower_c_collect.zig` / `src/lower_c_emitter.zig` bind and reduction consumers; `src/lower_c_builtin_emit.zig` enum raw consumer; `src/lower_c_emitter.zig` / `src/lower_c_try.zig` Result consumers; `src/lower_c_atomic.zig` / `src/lower_c_memory.zig` MIR payload consumers; `src/lower_c_builtin.zig` builtin classifiers; `src/lower_c_reflect.zig` MIR-gated reflection emission; `src/lower_c_convert.zig` MIR-gated conversion emission | C call emission and special builtin lowering | Registered backend inference. Closure bind, Result constructors, reductions, enum representation reads, bitcast, physical-address, reflection, byte-view, semantic escapes, atomic/MaybeUninit, raw memory operations, varargs operations, and all six scalar/domain conversion operations are MIR-owned call identities. Their migrated source/target/result/payload/element types are MIR-owned through `TargetTypeFact` rows; C uses AST only to validate confirmed operands and select emission mechanics. Other registered call categories remain to migrate. | Unknown call targets stay ordinary calls or unsupported; no provenance/range fact may be inferred from a call spelling. Missing or stale facts for migrated categories reject prebuilt MIR before emission. |
...
| `llvm-expression-type-inference` | `src/lower_llvm.zig` `exprType` / `derefPointeeType`; shared `src/ast_query.zig` expression/call-shape queries | LLVM expression, call, deref, constructor, and MMIO emission | Registered backend inference. Closure bind and Result constructors now require MIR identities and contextual target types; constructors, contextual coercions/literals/aggregates, reduction elements/results, enum raw source/results, conversion/bitcast/semantic-escape operations, atomic/MaybeUninit payloads, physical-address results, explicit casts, implicit single-pointer/slice const narrowing, raw/varargs/reflection/byte-view results, qualified tagged-union constructors, and enum variant paths obtain their migrated semantic types from MIR facts. Reduction result construction is determined by the MIR call identity and element fact; enum `.raw()` no longer derives receiver or repr types from LLVM-local expression/enum lookup; atomic/MaybeUninit call-info no longer derives payloads from receiver types. Broader expression typing remains open. | Unsupported or unknown shapes return null/unsupported. Missing/stale target-type or migrated call-target facts reject prebuilt MIR before emission. |
