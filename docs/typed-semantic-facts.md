...
Array, slice, enum, tagged-union, and Result direct-call inferred locals now
consume the MIR-owned `direct_call_result` fact before validating the owned
`inferred_local` row. Nullable inferred locals use the common MIR-owned
call-return path before validating the owned `inferred_local` row. Qualified
