...
|---|---|---|---|---|
| `c-expression-type-inference` | `src/lower_c_infer.zig` `operandEmitType`, `derefPointeeType`, and remaining return/type classifiers | C expression, aggregate, call, and deref emission | Registered backend inference. Qualified tagged-union constructor and enum variant-path result typing now consume MIR facts; backend declaration scans for those semantic types are retired. Typed member/index/slice/dereference and unary result queries use span-identified `expression_result` rows where the C classifier already has an independently recovered storage type. Numeric binary inferred-local initialization now requires its complete `expression_result` row and uses operand recovery only to reject a stale row, rather than falling back when the row is absent. Fixed-array direct-call bases now consume the MIR direct-call result type before C index lowering, so `make_matrix()[0]` becomes wrapper-array `.elems[...]` access rather than invalid C array subscripting. Raw-many offset return/deref typing consumes MIR result/element facts, and the recursive local/function-return receiver inference is retired. Ordinary declared direct-call result/fixed-argument typing and checked-MIR function-pointer/closure call signatures now consume complete MIR facts; declarations only confirm direct-call ABI/stale-fact equality. Address-of remains outside this family because its pointer type is target-context dependent in current sema, not a standalone expression result. The broader family still needs typed-fact/MIR migration. | Unsupported or unknown shapes return null/unsupported and must not invent a semantic fact. |
...
| `c-abi-aggregate-lowering` | `src/lower_c_aggregate.zig` array/struct/tagged-union literal emitters; `src/sema.zig` `checkExternExportStructAbi` | C internal aggregate values and ABI-shaped constructors; external ABI boundary admission | Registered backend mechanics for internal aggregate construction. Extern/export struct-by-value boundaries are an accepted fail-closed diagnostic policy: sema rejects plain and `extern` structs, aliases, and generic instantiations before either backend selects a target ABI. | Unsupported internal aggregate forms return `UnsupportedCEmission`; `E_EXTERN_STRUCT_BY_VALUE` requires pointers or out parameters at external boundaries until per-target ABI classification exists. |
...
| `c-abi-aggregate-lowering` | Backend AST inference budget | Internal aggregate construction consumes typed layout/ABI facts or rejects unsupported forms; extern/export struct-by-value is an inventory-gated diagnostic boundary until target ABI facts exist. |
