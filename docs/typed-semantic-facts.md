...
| `llvm-expression-type-inference` | `src/lower_llvm.zig` `exprType` / `derefPointeeType`; shared `src/ast_query.zig` `qualifiedTaggedUnionConstructorType`, `enumVariantPathType`, `rawLoadCallReturnType`, `rawPtrCallReturnType`, `bitcastCallReturnType`, `vaCallReturnType`, `byteViewCallReturnType`, `reflectionValueCallReturnType`, `atomicCallMemberOp`, `maybeUninitCallMemberOp`, `isDeclassifyCall`, `resultConstructorCallTag`, `isPhysCall`, `isBindCallNode`, and `isBindCallExpr` | LLVM expression, call, deref, constructor, and MMIO emission | Registered backend inference; qualified tagged-union constructor typing, enum variant-path typing, raw load/pointer builtin result typing, bitcast result typing, value-producing varargs builtin result typing, byte-view builtin result typing, value-producing reflection result typing, atomic/MaybeUninit call-target classification, declassify/reveal call recognition, Result constructor call recognition, physical-address constructor recognition, and closure bind call recognition use shared AST queries. Bounded `reduce` and `const_get` classification now comes from MIR `CallTargetFact.kind`; their AST queries only extract confirmed operands. The broader family still needs typed facts/MIR migration. | Unsupported or unknown shapes return null/unsupported and must not infer facts beyond sema-accepted source meaning. |
