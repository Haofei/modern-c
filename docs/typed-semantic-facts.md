...
| `llvm-expression-type-inference` | `src/lower_llvm.zig` `exprType` / `derefPointeeType`; shared `src/ast_query.zig` `qualifiedTaggedUnionConstructorType` and `enumVariantPathType` | LLVM expression, call, deref, constructor, and MMIO emission | Registered backend inference; qualified tagged-union constructor typing and enum variant-path typing now use shared AST queries instead of LLVM-local classifiers, but the broader family still needs typed facts/MIR migration. | Unsupported or unknown shapes return null/unsupported and must not infer facts beyond sema-accepted source meaning. |
