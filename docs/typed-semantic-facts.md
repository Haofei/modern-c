...
from `src/lower_c_infer.zig` and is exact-count gated at zero references. The
...
checks, now goes through MIR-aware array/slice/operand queries. Source
member-field, pointer-member-path, race-tolerant aggregate receiver, and
dyn-dispatch receiver base typing now uses MIR-aware operand or call-result
queries. Dyn-coercion vtable source typing is MIR-owned through
`dyn_coercion_source`. Pointer-to-`PAddr` target casts consume
`paddr_coercion_source` for target-typed unsafe coercions and
`explicit_cast_source` for explicit casts. By-value aggregate member copy
source typing now consumes the member expression's `expression_result` fact.
...
wrapper with its own typed row; direct local nullable values use their declared
`LocalInfo.source_ty` for pass-through detection.
...
checking the requested shape; numeric source-cast emission also requires the
cast expression's own `expression_result` row to match that target. Enum and
tagged-union source-cast classifiers follow the same rule. The AST cast target
remains only for generated zero-span casts. LLVM source-cast expression typing
now follows the same target/result rule instead of accepting
...
`string_literal`; enum literal expression typing consumes `enum_literal`. C inference call-result
