...
| `llvm-expression-type-inference` | `src/lower_llvm.zig` `exprType` / `derefPointeeType`; shared `src/ast_query.zig` expression/call-shape queries | LLVM expression, call, deref, constructor, and MMIO emission | Registered backend inference. `bind`, Result `ok`/`err`, bare tagged-union constructors, and target-typed enum literals use AST only to extract the confirmed operand/case spelling; their complete target type and expression identity come from MIR `TargetTypeFact`. The former LLVM `bindClosureType`, expected-type bare-union fallback, and expected-type enum lookup are retired. Qualified self-typed union/enum paths remain explicit AST-owned forms. Reflection, byte-view, semantic escapes and bounded builtin families retain their MIR call-target gates; raw/varargs and broader expression typing remain open. | Unsupported or unknown shapes return null/unsupported. Missing/stale target-type or migrated call-target facts reject prebuilt MIR before emission. |
