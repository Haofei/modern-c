...
when those paired files are supplied. The CLI/build gates reject C, LLVM, and
host-executable `.mcmeta` sidecars if they are paired with the wrong artifact
class, wrong backend, wrong generated bytes, or wrong loaded-source bytes.
...
- Large classes of backend behavior still carry semantic-adjacent emission
  queries. The retired `src/lower_c_infer.zig` module is exact-zero gated, but
...
out of `lower_c_infer` as well; the C emitter owns the remaining
...
identifier helper has also moved into the C emitter; `lower_c_infer` is now a
retired inventory anchor with its helper surface exact-count gated at zero, so
new backend-local expression inference helpers fail inventory unless they are
