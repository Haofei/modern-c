...
| MIR typed instruction metadata | `src/mir_model.zig` `ValueType`, `Instruction`, and optional `value_id` / `contract_region_id` carry typed operation metadata. `Function` owns `contract_regions`, `range_facts`, `call_target_facts`, `target_type_facts`, `pointer_provenance_facts`, `representation_facts`, and `elided_bounds`. | `CallTargetFact` records bounded builtin/call identities. `TargetTypeFact` records the complete target `ast.TypeExpr` for `bind`, Result `ok`/`err`, bare tagged-union constructors, enum/string/float literals, and array/struct literals including packed bits. Float targets preserve f32-vs-f64 operation/literal semantics. Propagation covers global/local/assignment/return/direct-call/builtin-member-call/nested-aggregate contexts, Result/union payloads, cast operands, and contextual binary operands. Metadata/fact multiplicities must match and same-key complete types must be structurally equal. Static-initializer alias expansion may reuse agreeing non-synthetic source facts across MIR owners; `(0,0)` facts remain owner-local. | `mcc lower-mir` prints function summaries plus owned target rows. | C and LLVM validate before emission. Runtime/global aggregate and float lowering plus C's unchecked sequencing consume target facts; broader semantic decisions remain in registered fallback families. |
...
| `llvm-expression-type-inference` | `src/lower_llvm.zig` `exprType` / `derefPointeeType`; shared `src/ast_query.zig` expression/call-shape queries | LLVM expression, call, deref, constructor, and MMIO emission | Registered backend inference. `bind`, Result `ok`/`err`, bare tagged-union constructors, enum/string/float literals, and array/struct literals use AST only to extract confirmed operand syntax; complete target and representation identity come from MIR `TargetTypeFact`. The former LLVM `bindClosureType` and expected-type constructor/literal/aggregate choices, including f32-vs-f64 literal normalization, are retired. Qualified self-typed union/enum paths remain explicit AST-owned forms. Reflection, byte-view, semantic escapes and bounded builtin families retain MIR call-target gates; raw/varargs and broader expression typing remain open. | Unsupported or unknown shapes return null/unsupported. Missing/stale target-type or migrated call-target facts reject prebuilt MIR before emission. |
