...
| `c-abi-aggregate-lowering` | `src/lower_c_aggregate.zig` array/struct/tagged-union literal emitters; `src/sema.zig` `checkExternExportStructAbi` | C internal aggregate values and ABI-shaped constructors; external ABI boundary admission | Registered backend mechanics for internal aggregate construction. Explicit C ABI declarations and exported functions reject named structs and tagged/fat optional values. Bare `extern fn` remains an internal backend-specific MC ABI; other aggregate families remain target-classification debt rather than a portable C-ABI claim. | Unsupported internal aggregate forms return `UnsupportedCEmission`; `E_EXTERN_STRUCT_BY_VALUE` requires pointers or out parameters for the guarded struct/optional boundary until per-target ABI classification exists. |
