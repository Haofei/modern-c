...
| MIR inferred local-copy type facts | For a one-name unannotated local initialized directly from an existing local value, MIR records an owned `inferred_local` target-type fact keyed by the binding name and initializer source point. | Generic target-type validation matches the owned fact and instruction. C and LLVM compare only an already-known local type to reject stale prebuilt MIR; the fact selects the destination binding type. | `lower-mir` prints the owner alongside the target-type row. | C uses the fact for inferred local-copy declarations; LLVM uses it before allocating the copied local. Removing the fact fails MIR admission and retargeting it fails backend emission. Literals, calls, casts, and computed initializers remain in the registered broader expression-inference boundary. |
