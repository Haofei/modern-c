...
The latest bounded call-type migration adds `atomic_payload` and
`maybe_uninit_payload` rows for every MIR-certified receiver operation. C removes
its receiver/local payload classifiers, and LLVM call-info consumes the MIR type
while retaining receiver inspection only for address mechanics. Missing rows
fail MIR admission. The inventory checker now executes every `EXACT_COUNTS` file
independently of the basic anchor table, so all retired-fallback guards run.
...
| `llvm-expression-type-inference` | `src/lower_llvm.zig` `exprType` / `derefPointeeType`; shared `src/ast_query.zig` expression/call-shape queries | LLVM expression, call, deref, constructor, and MMIO emission | Registered backend inference. Constructors, contextual coercions/literals/aggregates, conversion/bitcast/semantic-escape operations, atomic/MaybeUninit payloads, physical-address results, explicit casts, implicit single-pointer/slice const narrowing, raw/varargs/reflection/byte-view results, qualified tagged-union constructors, and enum variant paths now obtain complete semantic types from MIR facts. Atomic/MaybeUninit call-info no longer derives payloads from receiver types; broader expression typing remains open. | Unsupported or unknown shapes return null/unsupported. Missing/stale target-type or migrated call-target facts reject prebuilt MIR before emission. |
