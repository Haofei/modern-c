...
| MIR typed instruction metadata | `src/mir_model.zig` `ValueType`, `Instruction`, and optional `value_id` / `contract_region_id` carry typed operation metadata. `Function` owns `contract_regions`, `range_facts`, `call_target_facts`, `pointer_provenance_facts`, `representation_facts`, and `elided_bounds`. | `CallTargetFact` records bounded reductions, `const_get`, atomic and `MaybeUninit` members, exact bitcast/phys/raw/cpu-pause calls, and exact full/release/acquire fence calls with the MIR result type and source identity. Every row matches a dedicated `.call_target` instruction; exact validation rejects missing, stale, and duplicate rows, and C/LLVM query the fact kind for lowering. | `mcc lower-mir` prints function summaries and owned `mir call_target_fact` rows. | C and LLVM consume bounded call-target facts at admission/lowering; broader semantic decisions remain in the registered fallback families. |
...
| `c-call-target-classification` | `src/lower_c_call.zig` sequenced/bitcast/extern-nonnull call emitters; `src/lower_c_builtin.zig` builtin classifiers; `src/lower_c_reflect.zig` MIR-gated reflection emission | C call emission and special builtin lowering | Registered backend inference. The five value-producing reflection intrinsics are MIR-owned: C uses the AST only to select emission mechanics after an exact `CallTargetFact.kind` match. Other registered call categories remain to migrate. | Unknown call targets stay ordinary calls or unsupported; no provenance/range fact may be inferred from a call spelling. Missing or stale reflection facts reject prebuilt MIR before emission. |
