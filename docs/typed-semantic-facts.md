...
- straight-line declaration and assignment prefixes only; calls, member writes,
  ...
  or two-level nested aggregate field assignments before a checked trailing
  return;
...
direct field model, fallthrough dynamic-index, nested-array, deeper nested
member, or dereference writes or nested control flow, and any path with a
...
4. Remaining: fallthrough dynamic-index, nested-array, deeper nested-member,
   or dereference writes or nested control flow, mixed, exported,
