...
| MIR typed instruction metadata | `src/mir_model.zig` `ValueType`, `Instruction`, and optional `value_id` / `contract_region_id` carry typed operation metadata. `Function` owns `contract_regions`, `range_facts`, `call_target_facts`, `target_type_facts`, `pointer_provenance_facts`, `representation_facts`, and `elided_bounds`. | `CallTargetFact` records bounded builtin/call identities. `TargetTypeFact` records complete `ast.TypeExpr` values for constructors, contextual literals/aggregates, null/optional/dyn coercions, and both source and target sides of scalar/domain conversion builtins. Conversion facts cover all six operations and give targetless positive integer operands an explicit `u32` source representation. Already-optional and already-dyn pass-through values do not produce coercion facts; optional aggregate payloads retain both wrapper and child aggregate targets. Float targets preserve f32-vs-f64 operation/literal semantics. Metadata/fact multiplicities must match and same-key complete types must be structurally equal. Static-initializer alias expansion may reuse agreeing non-synthetic source facts across MIR owners; `(0,0)` facts remain owner-local. | `mcc lower-mir` prints function summaries plus owned target rows. | C and LLVM validate before emission. Conversion lowering consumes exact operation/source/target facts; runtime/global null, optional, dyn, aggregate, and literal lowering plus C's unchecked sequencing consume target facts; broader semantic decisions remain in registered fallback families. |
...
| `c-call-target-classification` | `src/lower_c_call.zig` sequenced/bitcast/extern-nonnull call emitters plus MIR-gated semantic escapes; `src/lower_c_builtin.zig` builtin classifiers; `src/lower_c_reflect.zig` MIR-gated reflection emission; `src/lower_c_memory.zig` MIR-gated byte-view emission; `src/lower_c_convert.zig` MIR-gated conversion emission | C call emission and special builtin lowering | Registered backend inference. Reflection, byte-view, semantic escapes, and all six scalar/domain conversion operations are MIR-owned call identities. Conversion source/target representation is also MIR-owned through paired `TargetTypeFact` rows; C uses AST only to extract the confirmed operand and select mechanics. Other registered call categories remain to migrate. | Unknown call targets stay ordinary calls or unsupported; no provenance/range fact may be inferred from a call spelling. Missing or stale facts for migrated categories reject prebuilt MIR before emission. |
...
| `llvm-expression-type-inference` | `src/lower_llvm.zig` `exprType` / `derefPointeeType`; shared `src/ast_query.zig` expression/call-shape queries | LLVM expression, call, deref, constructor, and MMIO emission | Registered backend inference. `bind`, Result `ok`/`err`, bare tagged-union constructors, null/value-optional/dyn coercions, enum/string/float literals, and array/struct literals use AST only to extract confirmed operand syntax or identify pass-through values; complete target and representation identity come from MIR `TargetTypeFact`. The former LLVM `bindClosureType` and expected-type constructor/literal/aggregate/coercion choices, including f32-vs-f64 literal normalization, are retired. Qualified self-typed union/enum paths remain explicit AST-owned forms. Reflection, byte-view, semantic escapes and bounded builtin families retain MIR call-target gates; raw/varargs and broader expression typing remain open. | Unsupported or unknown shapes return null/unsupported. Missing/stale target-type or migrated call-target facts reject prebuilt MIR before emission. |
