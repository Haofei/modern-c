...
The latest bounded call/type migration gives all three raw-memory operations an
exact identity and complete address/payload/result types. C and LLVM no longer
derive their semantic types from `call.type_args[0]` or a backend-local `PAddr`
default, and `raw.store` now has the same complete-type admission gate as the
value-producing operations. Missing identities or types fail MIR admission.
This closes the raw-memory operation family, not the broader call-target,
expression-type, pointer-provenance, or typed-HIR/MIR workstreams.
