...
The latest bounded call-identity migration adds `.bind` beside the existing
contextual closure target-type fact. C emission and scalar-env thunk collection,
plus LLVM call emission and return typing, now consume the MIR identity/type;
backend `isBindCall*` calls are retired. The unreachable LLVM global-bind
initializer fallback is removed under the existing static-global restriction.
Missing identities fail MIR admission. The inventory also rejects duplicate
file keys inside one semantic family so a new migration cannot silently
overwrite an existing anchor list.
...
| `llvm-expression-type-inference` | `src/lower_llvm.zig` `exprType` / `derefPointeeType`; shared `src/ast_query.zig` expression/call-shape queries | LLVM expression, call, deref, constructor, and MMIO emission | Registered backend inference. Closure bind and Result constructors now require MIR identities and contextual target types; constructors, contextual coercions/literals/aggregates, conversion/bitcast/semantic-escape operations, atomic/MaybeUninit payloads, physical-address results, explicit casts, implicit single-pointer/slice const narrowing, raw/varargs/reflection/byte-view results, qualified tagged-union constructors, and enum variant paths obtain their migrated semantic types from MIR facts. Atomic/MaybeUninit call-info no longer derives payloads from receiver types; broader expression typing remains open. | Unsupported or unknown shapes return null/unsupported. Missing/stale target-type or migrated call-target facts reject prebuilt MIR before emission. |
