...
| Integer, float, and access facts join to instructions by identity, not by source span, with an exactly-one invariant. | `src/mir_verify.zig`, `src/mir_tests.zig` |
...
| P1 | Convert golden tests on internal MIR dumps and emitted text into fixture files with expected outputs. | The in-Zig golden strings are the largest consumer of the legacy instruction stream and the reason representation changes turn into mass test rewrites. |
