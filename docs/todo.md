...
| P1 | Persistence and recovery | Filesystems, block storage, checkpoint-like primitives, lifecycle, liveupdate demos, and BlobStore-backed policy/audit checkpointing exist, but production persistence is not complete. | Move policy/audit checkpointing to the production block-backed reboot path, add watchdog/reboot-reason flow, and run long QEMU plus real-board soak tests. |
