...
| P1 | Persistence and recovery | Filesystems, block storage, checkpoint-like primitives, lifecycle, BlobStore-backed policy/audit checkpointing, and BlockDevice-backed policy/audit checkpointing are gated. | Keep only storage/recovery pieces required by active kernel demos; defer product reboot/update policy. |
