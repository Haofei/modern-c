...
| `[~]` | LLVM backend | 已有初始 `emit-llvm`：MIR 验证后输出 scalar 函数 / call / checked integer arithmetic 的 textual LLVM IR，并用 `llvm-as` 验证 smoke | 扩 control flow、aggregate/memory、ABI/object emission |
