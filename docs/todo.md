...
| P1 | Updates and recovery | `production-ops-test` gates bundle metadata rejection semantics and two-slot rollback state as prototype fixtures. | Define product OTA/recovery policy and wire only the required update path into actual agent startup. |
