...
| `[~]` | LLVM backend | 已有初始 `emit-llvm`：MIR 验证后输出 scalar 函数 / call / checked integer arithmetic / bool switch-if control flow 的 textual LLVM IR，并用 `llvm-as` 验证 smoke | 扩 loop/mutable local、aggregate/memory、ABI/object emission |
