...
| `[~]` | Production MIR optimizer use | The fact-gated optimizer framework now exists (spec §E.4): `mir.buildOpt`/`verifyOpt` take an `optimize` flag, exposed as `mcc verify|lower-mir --optimize` and **off by default**, so the standard pipeline is byte-identical. The first transform — **const-index bounds-check elision** — is implemented with a stated proof obligation (`k < N`, both compile-time constants), gated and tested: a `mir.zig` unit test asserts the `Bounds` trap edge is dropped for `a[2]` on `[4]u32` and kept for a variable index, and `opt-test` (in `m0`) asserts `verify` rejects / `verify --optimize` accepts a `#[no_lang_trap]` const-index module while a variable index stays rejected. Sema and MIR apply the same proof so they agree. | Land the next transforms one at a time per the §E.4 discipline; in particular, have the C/LLVM backends consume the optimized MIR to elide the *emitted* runtime check (not just refine verification), gated by the same flag and a `diff-backend` C-vs-LLVM equivalence test. |
