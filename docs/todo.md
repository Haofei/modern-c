...
| `[~]` | LLVM backend | 已有初始 `emit-llvm`：MIR 验证后输出 scalar 函数 / call / checked integer arithmetic / bool switch-if control flow / simple scalar local / while loop 的 textual LLVM IR，并用 `llvm-as` 验证 smoke | 扩 aggregate/memory、ABI/object emission、broader CFG joins、debug info |
