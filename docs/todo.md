...
| P1 | Kernel product surface | POSIX/VFS/TCP/DNS/socket/shell/userland/exec product fixtures have been removed from the current language-oriented scope. | Keep kernel validation focused on language, ABI, MMIO, driver, async, ownership, and backend-lowering evidence. |
