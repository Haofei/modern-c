...
| `[x]` | LLVM backend | 已有初始 `emit-llvm`：MIR 验证后输出 scalar 函数 / call / void call+expr stmt+assert+block workflow+unchecked contract arithmetic / trap+unreachable+never return / comptime block omission / uninit materialization + MaybeUninit payload storage / opaque-address same-repr casts + pointer-to-usize/isize casts + UserPtr/PhysPtr ABI + std/addr helpers / DMA/IRQ markers(DmaAddr、DmaBuf、cache clean/invalidate、dma_addr/as_slice、IrqOff) / nullable pointer workflow(null、? unwrap、if-let、switch) / Result ABI+constructors+? trap unwrap+if-let/switch narrowing+? propagation / defer cleanup(return/fallthrough/break/continue) / unsafe contract scope(no_overflow/noalias) / unsafe machine ops(raw load/store、raw.ptr、phys、cpu.pause、opaque asm、precise asm operands、raw-many offset) / unsafe-block raw stores / typed MMIO(Reg/RegBits、MmioPtr、@offset、volatile read/write、acquire/release fence、irq-context fixture、irq-context atomic aliases、mmio.map<T>(pa)?) / type alias + enum representation lowering / checked integer arithmetic+signed unary negation / checked div-rem / scalar casts+bitwise+checked shifts / target-typed integer/enum coercion / wrap<T>+sat<T>+serial<T>+counter<T>+Duration<T> payload ABI、domain conversions/residue、wrap modular ops、serial/counter modular ops、serial.compare、counter.elapsed_bounded、try_from Result、unsigned sat arithmetic / reduce.sum_checked、reduce.sum_left、reduce.sum_fast / char literal+checked u8 arithmetic / string literal u8-pointer constants / byte views(mem.as_bytes、mem.bytes_equal) / packed-bits integer ABI+field masks+dynamic literals+field updates / overlay union byte storage+field reflection / tagged union aligned ABI+constructors+switch payload bindings / reflection(sizeof/alignof/repr_of/field_type) / short-circuit bool ops / f32+f64 scalar ops / bool switch-if control flow with simple joins / scalar switch / simple scalar local / inferred local / while+for loop CFG / break+continue / richer iterable forms(arrays/slices from params、globals、nested rows、fields、direct-call aggregate fields/results) / basic pointer load-store / scalar-pointer global / static global scalar copy+cast copy+string pointer arrays+constant address globals+default aggregate zeros / closure fat-value ABI+bind+indirect call+closure fields in globals / local fixed scalar array / local scalar struct / scalar aggregate global / scalar aggregate ABI / aggregate layout ordering / nested aggregate access / aggregate rvalue materialization / aggregate assignment+copy / generic struct monomorphized aggregate ABI / comptime-parameter array specialization / monomorphized layout reflection / core slice value+index+range+store / direct-call/local array-slice indexing / pointer-slice identity / atomic<T> init+load+store+fetch_add+fetch_sub / fn pointer value+static init+copied aggregate global+indirect call / statement-scoped LLVM debug metadata(calls/returns/stores/loads including ordinary pointer/global/index access, aggregate literal/member field, MaybeUninit, volatile raw/MMIO, atomics/RMWs, and fences; precise asm output stores/loop branches/switch/if-let dispatch+joins、nullable/Result/tagged-union/for-loop binding stores、aggregate rvalue/tagged-union constructor materialization stores、tagged-union switch subject stores/tag loads、trap-path, ? propagation, short-circuit bool branches, and helper-loop branches) + `llvm-debug-test` DWARF file/function/line gate 覆盖 calls、control flow、atomics/fences、nullable/Result narrowing / textual LLVM IR，并用 `llvm-as` 验证 smoke；`mcc-llvm-cc.sh` 已通过 `llc` 产出代表性 object；`llvm-c-sweep` 和 `llvm-c-obj-sweep` valid `tests/c_emit` 109/109 通过；`llvm-opt-sweep` valid spec 67 fixtures/534 functions + `tests/c_emit` 109 fixtures 通过 hidden-assumption、LLVM verifier、`default<O2>` pipeline，并将每个 optimized O2 结果通过 `llc` 降到非空 object；`llvm-cc-test`/`llvm-move-test` link+run 通过；`llvm-runtime-test` imported generic std/stack + std/sync + fn-pointer link+run 通过；`llvm-toolchain-test` import/std merge + monomorphization + generic struct link+run 与 reflection LLVM object 通过；`llvm-std-test` multi-object std link+run 通过；`llvm-pkg-test` package manifest build link+run 通过；`llvm-demo-test` framebuffer/gpio/irq/spi/timer/uart hardware demos 与 hosted elementwise demo 通过 LLVM object lowering 并复用 hidden assumption gate；`llvm-hosted-demo-test` hosted elementwise demo 通过 LLVM+libc+libm link/run 与二进制 stdin/stdout f32 round-trip；`llvm-host-suite-test` 全量 `tools/lib/host-tests.tsv` host-driver manifest 通过 LLVM object link/run；valid spec LLVM sweep 67 个 fixture、534 个 valid function 全部通过，无 LLVM allowlist；`llvm-spec-obj-sweep` valid spec 67 个 fixture、534 个 valid function 通过 `llc` object lowering；`llvm-sweep`/`llvm-c-sweep` 会拒绝隐藏 optimizer assumption token (`nuw`/`nsw`/`nonnull`/`noalias`/`noundef`/`poison`/`inbounds`/`undef` 和 hidden fast-math flags，仅显式 `reduce.sum_fast` 浮点 reduction 允许 `reassoc`) | 当前 spec surface 已完成；source/MIR-quality native debugger mapping 作为后续 debug-info 工作 |
