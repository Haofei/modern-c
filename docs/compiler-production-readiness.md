...
| Typed semantic fact table / typed MIR | The design slice, Phase 1 inventory/stabilization, Phase 2 narrow MIR pointer-provenance table, and Phase 3 LLVM narrow subset consumption are complete in `docs/typed-semantic-facts.md`, but C still does not consume this fact family and LLVM still carries broader duplicated provenance inference for unsupported shapes. | Implement Phase 4 C backend parity for the narrow `PointerProvenanceFact` family, then do the remaining Phase 3/5 cleanup that retires duplicated LLVM AST inference only where typed facts cover equivalent behavior. |
