...
Current ledger: **updated 2026-07-05, based on implementation through `5f115920`**.
...
| Typed semantic fact table / typed MIR | The design slice is now complete in `docs/typed-semantic-facts.md`, but backends still re-derive too much from raw AST. No typed fact family has yet been migrated end-to-end from sema/MIR production through both backend consumers with duplicated AST inference retired. | Implement Phase 1 inventory/stabilization from `docs/typed-semantic-facts.md`, then migrate one narrow fact family through typed production, C/LLVM consumption, parity tests, and removal of the corresponding backend-only inference. |
