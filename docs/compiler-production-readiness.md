...
Ledger count: **57 finished and pushed, 0 in progress, 4 pending**.
...
| Typed semantic facts Phase 3 LLVM narrow pointer-provenance consumption is complete | LLVM lowering now consumes MIR `PointerProvenanceFact` rows for direct pointer-like locals and direct fixed local pointer-array elements, seeding unordered atomic pointer-mediated global loads only for live `global_storage` rows and clearing/avoiding facts for `local_storage`, `unknown`, dynamic-index invalidation, and call invalidation. This closes only the LLVM narrow subset; C direct-local parity is now tracked by Phase 4, while C fixed pointer-array element consumption and broader LLVM provenance cleanup remain pending. | `src/lower_llvm.zig` `applyMirPointerProvenanceForLocalInitializer`, `applyMirPointerProvenanceForAssignment`, `applyMirPointerProvenanceForIndexAssignment`, and `applyMirPointerProvenanceInvalidationsAtCall`; `src/lower_llvm_tests.zig`; `tests/spec/data_race_semantics.mc` `pointer_global_invalidated_by_call_stays_plain`; `docs/typed-semantic-facts.md` Phase 3 status; `python3 tools/toolchain/semantic-facts-inventory.py`; `zig test src/lower_llvm_tests.zig`; `zig build test`; `git diff --check`. |
| Typed semantic facts Phase 4 C direct-local pointer-provenance consumption is complete | C lowering now consumes MIR `PointerProvenanceFact` rows for direct pointer-like locals at initializer, assignment, and call-invalidation source points. Live `global_storage` facts route scalar pointer deref loads/stores through `mc_race_load_*` / `mc_race_store_*` using the pointer expression; `local_storage`, `unknown`, and invalidated facts keep the existing plain deref behavior. This closes only the direct-local scalar C slice; C fixed pointer-array element consumption and broader LLVM cleanup remain pending. | `src/lower_c_emitter.zig` `applyMirPointerProvenanceForLocalInitializer`, `applyMirPointerProvenanceForAssignment`, `applyMirPointerProvenanceInvalidationsAtCall`, and `mirPointerProvenanceDerefRaceInfo`; `src/lower_c_tests.zig` `lower-c consumes MIR pointer provenance facts for direct scalar pointer derefs`; `docs/typed-semantic-facts.md` Phase 4 status; `python3 tools/toolchain/semantic-facts-inventory.py`; `zig test src/lower_c_tests.zig`; `zig build test`; `git diff --check`. |
...
| Typed semantic fact table / typed MIR | The design slice, Phase 1 inventory/stabilization, Phase 2 narrow MIR pointer-provenance table, Phase 3 LLVM narrow subset consumption, and Phase 4 C direct-local scalar deref consumption are complete in `docs/typed-semantic-facts.md`, but C still does not consume fixed pointer-array element facts and LLVM still carries broader duplicated provenance inference for unsupported shapes. | Decide whether to migrate C fixed pointer-array element facts next, then do the remaining Phase 5 cleanup that retires duplicated LLVM AST inference only where typed facts cover equivalent behavior. |
...
| Item | Why | Effort |
|---|---|---|
| Typed fact table: sema resolves once, backends consume (mcc2's proven pattern) | Design slice, Phase 1 inventory, Phase 2 narrow pointer-provenance MIR table, and Phase 3 LLVM narrow consumption are complete in [`typed-semantic-facts.md`](typed-semantic-facts.md); next work is Phase 4 C backend parity plus remaining cleanup for that fact family. Kills the drift class (root cause #1); prerequisite for parallel + incremental; retires per-backend inference | L |
