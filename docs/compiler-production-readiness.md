...
Ledger count: **60 finished and pushed, 0 in progress, 4 pending**.
...
| Typed semantic fact table / typed MIR | The design slice, Phase 1 inventory/stabilization, Phase 2 narrow MIR pointer-provenance table including direct raw-many local `.offset(0)`, Phase 3 LLVM narrow subset consumption, Phase 4 C narrow subset consumption, and Phase 5 narrow LLVM direct-local cleanup are complete in `docs/typed-semantic-facts.md`, but LLVM still carries broader duplicated provenance inference for unsupported shapes. | Continue Phase 5 by retiring duplicated LLVM AST inference only where typed facts cover equivalent behavior; keep fallback inference for unsupported shapes until equivalent typed facts exist. |
