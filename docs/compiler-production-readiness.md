...
| Typed semantic fact table / typed MIR | The design slice, Phase 1 inventory/stabilization, and Phase 2 narrow MIR pointer-provenance fact table are complete in `docs/typed-semantic-facts.md`, but backends still re-derive too much from raw AST. No typed fact family has yet been migrated through backend consumption with duplicated AST inference retired. | Implement Phase 3 from `docs/typed-semantic-facts.md`: migrate the first backend, preferably LLVM, to consume the narrow `PointerProvenanceFact` family for the supported subset while preserving fail-closed behavior when facts are absent or stale. |
...
| Item | Why | Effort |
|---|---|---|
| Typed fact table: sema resolves once, backends consume (mcc2's proven pattern) | Design slice, Phase 1 inventory, and Phase 2 narrow pointer-provenance MIR table are complete in [`typed-semantic-facts.md`](typed-semantic-facts.md); next work is Phase 3 backend consumption for that fact family. Kills the drift class (root cause #1); prerequisite for parallel + incremental; retires per-backend inference | L |
