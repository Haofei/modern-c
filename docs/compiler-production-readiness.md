...
Current ledger: **updated 2026-07-05, based on `d679659b`**.
...
|---|---|---|
| Broader pointer-provenance race lowering | The LLVM backend now handles direct address-of global derefs, simple local pointer copies, internal helpers proven to return global-backed pointers, and bounded internal direct-call pointer parameters, but computed, escaped, indirect-call, exported/external, and aggregate-contained pointers are not yet proven as global-backed. | Decide whether to add sema/MIR provenance facts or conservatively unordered-atomic all scalar derefs whose storage cannot be proven local/raw/MMIO. |
