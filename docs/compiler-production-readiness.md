...
| Typed semantic fact table / typed MIR | The design slice and Phase 1 inventory/stabilization are now complete in `docs/typed-semantic-facts.md`, but backends still re-derive too much from raw AST. No typed fact family has yet been migrated end-to-end from sema/MIR production through both backend consumers with duplicated AST inference retired. | Implement Phase 2 from `docs/typed-semantic-facts.md`: add a typed fact table for one narrow fact family, preferably the pointer/global race provenance subset, then migrate C/LLVM consumption, parity tests, and removal of the corresponding backend-only inference in later phases. |
...
| Item | Why | Effort |
|---|---|---|
| Typed fact table: sema resolves once, backends consume (mcc2's proven pattern) | Design slice and Phase 1 inventory complete in [`typed-semantic-facts.md`](typed-semantic-facts.md); next work is the Phase 2 typed fact table for one narrow fact family. Kills the drift class (root cause #1); prerequisite for parallel + incremental; retires per-backend inference | L |
