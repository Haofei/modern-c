...
| G33 | post-P5 | selfhost-sema | **Duplicate top-level declaration accepted** — `sm_collect` overwrote the symbol-table entry, so a repeated top-level name type-checked (exit 0) and the emitter output duplicate C definitions. Found across three review passes (fn-only → same-kind struct/const/global/enum → cross-namespace). | `fn f`×2 / `struct S`+`fn S` / `const X`+`fn X` / `struct X`+`const X` / `enum Y`+`struct Y` → dup C | ~~medium~~ **FIXED (all kinds + cross-namespace)** | every top-level registration in collect calls `sm_toplevel_taken(name)`, which checks ALL four tables (`fns`/`structs`/`enums`/`globals`); a clash in either declaration order emits `duplicate_decl` (SmErr 17). MC has one flat top-level namespace (G22), so this matches the language. |
