...
| 12. Long-running lifecycle | Partial | Core lifecycle exists: `proc_spawn` / `proc_exit` / `proc_kill` (+ `proc_signals`) / `proc_reap` (parent reaps a dead child — crash cleanup) / pause/resume, and `liveupdate.mc` (version handoff). **Added (2026-06-30):** supervision heartbeat-liveness detection — `proc_supervise`/`proc_heartbeat`/`proc_liveness_expired`/`proc_unsupervise` (per-slot deadline; detects a missed heartbeat so a supervisor can restart/kill); gated in `scheduler-test`. Gap: the **restart/crash-loop policy + supervision trees + leases + persistent identity** on top of this detection. |
