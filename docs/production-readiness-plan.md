...
| --- | --- | --- |
| 1. Preemptive scheduling | **In progress** | Correction: `proc_sched.mc` already has priorities (`proc_set_priority`/`proc_schedctl`), per-process quantum/timeslice (`proc_tick`), fair-share least-served pick (`proc_pick_fair`), and throttle — so priorities + per-agent CPU budget are *mechanism-complete and gated*; the gap was timer-driven preemption. **Landed (2026-06-30):** the timer-driven preemption-decision layer — `proc_preempt_tick` (irq-safe: account tick, raise `need_resched` on quantum expiry), `proc_preempt_pending`/`proc_preempt_clear`, and the safe `proc_preempt_point` (reschedules at a non-ISR point via `proc_yield_priority`); gated in `scheduler-test`. **Remaining:** wire the real platform timer ISR to `proc_preempt_tick`, and the fully-async switch from the interrupted trap frame (full-frame save in the asm vector → mret into another process). |
