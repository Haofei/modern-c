...
| 269 | Move direct IR inspection tests to resolved-source inputs. | Complete: `ir_tests.zig` now drives facts and lower-IR artifact assertions through `ResolvedSourceDatabase`, `appendFactsFromResolvedSources(...)`, and `buildModuleIrFromDecls(...)`; the remaining module-shaped `buildModuleIr(...)` coverage is limited to allocator-failure tests. |
