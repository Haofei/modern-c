...
| 69 | Store `VerifiedProgram` syntax escapes as named views. | Complete: `VerifiedProgram` no longer stores a generic declaration slice directly; it stores `DeclarationMetadataView`, while source-map syntax mechanics are passed only to the `emit-map` path through `SourceMapMechanicsView`, so remaining syntax-shaped access must go through named mechanics views and the old raw declaration field/count is exact-gated. |
