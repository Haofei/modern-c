...
| 208 | Stop backend-local auto-drop register/cancel events. | Complete: C and LLVM lowerers now validate `OwnershipCleanupPlan`/`CleanupCfg` once at function entry via `backend_cleanup.validateFunctionCleanupAuthority`; AST declaration and move/drop expression traversal no longer calls backend cleanup register/cancel hooks, and the old event-level helper functions are exact-zero-gated. |
| 209 | Route backend vtables through codegen request objects. | Complete: `LowerRequest` and `EmitMapRequest` moved to `codegen_request.zig`; `backend.zig` no longer imports `legacy_backend_syntax.zig`, and CLI/test callers use request-based lowering/map emission instead of passing legacy syntax mechanics as standalone backend parameters. |
| 210 | Split source-map syntax mechanics from backend declaration mechanics. | Complete: `SourceMapMechanicsView` moved to `source_map_mechanics.zig`; `legacy_backend_syntax.zig` now owns only the declaration-slice transition, while map-only AST row enumeration is named separately behind `EmitMapRequest.source_map_mechanics`. |
| 211 | Split early declaration metadata from the generic legacy declaration slice. | Complete: C/LLVM early declaration prepasses now request `EarlyDeclarationMetadataView` through `LegacyDeclarationSlice.earlyDeclarationMetadata()`; direct `LegacyDeclarationSlice.declsForEarlyDeclarationScan()` access is removed and exact-zero-gated so the remaining AST declaration dependency is narrower and named by use case. |
| 212 | Carry early declaration metadata through codegen requests. | Complete: `LowerRequest` now carries `declaration_artifacts: EarlyDeclarationArtifacts` instead of `legacy_declarations: LegacyDeclarationSlice`; C/LLVM backend vtables no longer receive the generic declaration slice, and the remaining wrappers construct the narrow declaration artifacts at the syntax compatibility edge. |
...
| 214 | Rename declaration metadata mechanics by current role. | Complete: `EarlyDeclarationMetadataView` moved from the obsolete `legacy_backend_syntax.zig` container to `early_declaration_metadata.zig`; backend-facing requests and the CLI now import `declaration_artifacts.zig`, while tests still collect transitional artifacts at the syntax compatibility edge. |
...
| 216 | Retire MIR target-type module fallback. | Complete: `MirFactsView` target-type queries no longer scan other functions by source span; current-function compatibility entrypoints are exact-count gated, and MIR tests assert cross-function lookups return null. |
...
| 236 | Field-normalize global declaration artifacts. | Complete: `GlobalArtifact` now stores the global name/type/initializer/linkage fields directly, C/LLVM collected-global queues carry that artifact instead of `ast_bridge.GlobalDecl`, and only the declaration-artifact collector/comptime compatibility view reconstructs an `ast.GlobalDecl`. |
| 237 | Field-normalize backend function request artifacts. | Complete: `FunctionArtifact` now stores function signature/body/linkage fields directly instead of carrying `fn_decl: ast.FnDecl`; C/LLVM prepasses reconstruct a temporary declaration only at the remaining function-lowering compatibility edge, and inventory gates prevent the backend request artifact from regressing to a raw function declaration payload. |
| 238 | Field-normalize ownership function declaration artifacts. | Complete: `mir_ownership_authority.FunctionDeclArtifact` now stores function fields directly instead of carrying `fn_decl: ast.FnDecl`; C/LLVM collected-function queues build that artifact via `fromDecl(...)`, and drop-glue matching uses the artifact name directly before the remaining temporary ABI-shape compatibility conversion. |
| 239 | Split and field-normalize trait declaration artifacts. | Complete: the mixed `TraitArtifact` union was replaced by separate `TraitDeclArtifact` and `ImplTraitArtifact` slices; C/LLVM trait collection consumes explicit trait/impl artifact streams instead of switching over a mixed AST union, with compatibility `toDecl()` only at the trait-object lowering edge. |
| 240 | Store trait declaration maps as trait artifacts. | Complete: C/LLVM trait declaration maps now store `TraitDeclArtifact` directly; C vtable helpers, C dyn trait type emission, LLVM vtable emission, and LLVM dyn dispatch read artifact methods without reconstructing `ast.TraitDecl`, and inventory gates keep the retired `trait_decl.toDecl()` compatibility path at zero. |
| 241 | Use field-normalized function artifacts for backend metadata collection. | Complete: C and LLVM now collect function signatures, drop-glue validation, backend-name metadata, debug metadata, bind-thunk body scans, and ownership function artifacts directly from `FunctionArtifact`/`FunctionDeclArtifact` fields; inventory gates keep the old `collectFnDeclArtifact(function.toDecl())`, `FunctionDeclArtifact.fromDecl(...)`, and AST-shaped drop-parameter lookup paths at zero. |
| 242 | Emit C/LLVM functions from function declaration artifacts. | Complete: C function declaration/definition loops and LLVM callable emission now pass `FunctionDeclArtifact` directly into artifact-aware signature/body helpers instead of reconstructing `ast.FnDecl` at each emission site; `FunctionDeclArtifact.toDecl()` was deleted and inventory gates keep the old `artifact.toDecl()` emission loops at zero. |
| 243 | Isolate comptime function/global declaration reconstruction. | Complete: `FunctionArtifact.toDecl()` and `GlobalArtifact.toDecl()` were deleted; comptime compatibility now uses the named `comptimeFnDeclFromArtifact(...)` adapter and a private global-declaration adapter, while backend emission/metadata paths stay field-normalized and inventory gates keep the old generic `.toDecl()` methods at zero. |
| 244 | Rename inspection surfaces at the CLI boundary. | Complete: `inspect-hir`, `verify-inspect-hir`, and `inspect-ir` are now the documented inspection commands; legacy `lower-hir`, `verify-hir`, and `lower-ir` aliases remain accepted for existing tests/scripts, and help/docs now state that inspection artifacts are not backend pipeline inputs. |
| 245 | Rename HIR/IR implementation files as inspection modules. | Complete: `src/hir.zig` and `src/ir.zig` were renamed to `src/hir_inspection.zig` and `src/ir_inspection.zig`; imports, docs, dev-gate rules, and inventory anchors now use inspection names, with module headers explicitly stating they are diagnostics/debug surfaces rather than backend inputs. |
| 246 | Add per-file source ownership beside textual inclusion. | Complete: `LoadedProject` now carries a `SourceDatabase` with one independent source buffer per `FileId` while preserving the legacy combined source; loader expansion records stripped per-file sources during graph construction, parser tests assert each graph file has a source entry and that raw root imports are distinct from blanked combined text, and architecture inventory anchors the boundary. |
| 247 | Add parser-ready source buffers per module file. | Complete: each `SourceFile` now carries both raw `source` and import-blanked `parser_source`, loader records the parser-ready bytes before appending them to the legacy combined source, parser tests parse a root file directly from `SourceDatabase.parserSourceForFile(...)`, and architecture inventory anchors this per-file syntax boundary for the future module parser. |
| 248 | Add parsed source database for per-file AST ownership. | Complete: `module_parser.zig` owns `ParsedSourceDatabase`, parses every `ModuleGraph` file from `SourceDatabase.parser_source`, and parser/module tests verify the resulting per-file AST modules can be queried by `FileId`; the main pipeline still uses the legacy combined module, but the next module-model migration has an explicit syntax authority. |
| 249 | Add resolved source database for per-file name resolution. | Complete: `module_parser.zig` now owns `ResolvedSourceDatabase` and `resolveParsedSourceDatabase(...)`, parser tests run per-file name resolution over loaded module sources, and module-parser tests prove qualified references can be rewritten without first constructing the legacy combined AST. |
| 250 | Attach per-file resolved modules to `CompilationSession`. | Complete: `CompilationSession` now carries a request-scoped `ResolvedSourceDatabase` pointer, `attachLoadedProjectSyntax(...)` builds parsed/resolved per-file modules from `LoadedProject.source_db`, `main` attaches it after import loading, session tests verify the per-file syntax authority is populated, and the compilation-session inventory anchors the new boundary. |
| 251 | Drive `mcc symbols` from per-file resolved modules. | Complete: `symbols.emitJsonFromResolvedSources(...)` merges `ResolvedSourceDatabase` modules with `ModuleGraph` source offsets, `runSymbols` requires `session.resolved_sources`, and the symbols gate checks an imported function keeps its imported-file span instead of relying on the legacy combined AST. |
| 252 | Drive inspection artifacts from per-file resolved modules. | Complete: `ir_inspection.appendFactsFromResolvedSources(...)` and `appendLowerIrFromResolvedSources(...)` aggregate `ResolvedSourceDatabase` modules, `runFacts`/`runLowerIr` use the session-owned per-file syntax boundary when available, and `mcc-inspection-modules-test` proves imported modules contribute facts and inspect-IR rows without relying on the legacy combined AST parse path. |
| 253 | Drive `mcc list-tests` from per-file resolved modules. | Complete: `runListTests` now enumerates `#[test]` declarations from `CompilationSession.resolved_sources`, and `mcc-list-tests-modules-test` proves imported per-file modules contribute discovered tests. The old combined-module parse fallback was later removed and exact-gated. |
| 254 | Preserve imported-file diagnostics during per-file syntax attach. | Complete: `module_parser.parseSourceDatabase(...)` now switches the reporter to each `ModuleFile` path/source while parsing per-file modules and preserves that context on parse failure, `main` renders/serializes attach failures through the normal CLI diagnostic path, `E_MONOMORPHIZATION_LIMIT` is classified as an expected CLI failure to avoid stack traces, and `diagnostics-test` covers imported-file parse JSON locations. |
| 255 | Add a resolved declaration stream for per-file modules. | Complete: `ResolvedSourceDatabase.collectDecls(...)` now exposes a file-id-tagged declaration stream, symbols/facts/inspect-IR/list-tests consume that stream instead of open-coding `sources.files` traversals, and module-parser tests anchor declaration order plus `FileId` ownership for later check/sema migration. |
| 256 | Ratchet the per-file declaration stream boundary. | Complete: architecture-boundary inventory now anchors `ResolvedDecl`/`collectDecls(...)` and fails if symbols, inspect-IR/facts, or list-tests regress to direct `ResolvedSourceDatabase.files` traversal, keeping new module-aware consumers on the shared declaration stream before check/sema migration. |
| 257 | Add declaration artifact collection from the resolved declaration stream. | Complete: `EarlyDeclarationArtifacts.collectFromResolvedDecls(...)` accepts `ResolvedDecl` streams and is equivalence-tested against the syntax-backed collector, while inventory gates keep the compatibility adapter named and prevent accidental growth of driver `module.decls` collection. Production codegen still waits for file-id-aware source-map artifacts before switching from the legacy combined-source span path. |
| 258 | Route driver declaration artifacts through the resolved declaration stream. | Complete: `driver_codegen_inputs.zig` now always builds declaration artifacts through `collectFromResolvedDecls(...)`; attached module syntax uses `CompilationSession.resolved_sources.collectDecls(...)`, and missing resolved-source state fails closed instead of wrapping `module.decls` into a fallback `ResolvedDecl` stream. Inventory gates ratchet the direct driver syntax collection count from two to zero. |
| 259 | Remove the public syntax declaration artifact collector. | Complete: `SyntaxDeclarationSlice` was deleted, `collectFromSyntaxDecls(...)` is now a private implementation helper, and C/LLVM/spec tests use the named `collectFromModuleDeclsForTests(...)` compatibility wrapper that funnels syntax-shaped modules through the same `ResolvedDecl` artifact path as driver code. Inventory gates keep public syntax artifact collection at zero. |
...
| 268 | Delete unused module-shaped lower-ir text wrapper. | Complete: the public `ir_inspection.appendLowerIr(allocator, ast.Module, ...)` wrapper was removed after CLI `lower-ir` moved to `appendLowerIrFromResolvedSources(...)`; inventory gates keep the module-shaped text-IR wrapper at zero. |
| 269 | Move direct IR inspection tests to resolved-source inputs. | Complete: `ir_tests.zig` now drives facts, lower-IR artifact assertions, and lower-IR OOM checks through `ResolvedSourceDatabase`, `appendFactsFromResolvedSources(...)`, and `buildModuleIrFromDecls(...)`. |
| 270 | Delete the nested facts collector wrapper. | Complete: the public `Collector` wrapper in `ir_inspection.zig` was removed; top-level inspection functions now instantiate `ModuleFactCollector` directly, while the remaining `appendFacts(...)` compatibility path is narrowed to a declaration slice for spec harness callers until that harness can migrate through monomorphized resolved-source fixtures. |
...
| 276 | Narrow public MIR APIs to declaration slices. | Complete: public `mir.build(...)`, `buildOpt(...)`, `appendDump(...)`, `appendDumpOpt(...)`, `appendVerificationFacts(...)`, `verify(...)`, and `verifyOpt(...)` module-shaped entrypoints were replaced by `*FromDecls(...)`; CLI/session/spec/backend/MIR tests now call the decl-slice API, and inventory gates keep the old public MIR module API at zero. |
| 277 | Stop passing AST modules through driver codegen input assembly. | Complete: `driver_codegen_inputs.buildBackendInputs(...)` and `buildCArtifactInputs(...)` now accept declaration slices, call `CompilationSession.buildVerifiedProgramFromDecls(...)`, and no longer accept `module: ast.Module`; inventory gates keep the old driver codegen module-shaped boundary at zero. |
| 278 | Remove MIR-internal module-shaped fact collectors. | Complete: MIR drop-glue, type-ownership, aggregate-return, and direct-global pointer-return collectors now accept declaration slices directly; `buildOptFromDecls(...)` no longer traverses `module.decls`, and inventory gates keep MIR helper `module: ast.Module` parameters at zero while the remaining eval compatibility wrapper is explicitly counted. |
...
| 282 | Wrap parsed frontend output before inspection handoff. | Complete: public `parseModuleOrReport(...)` now returns `ParsedModule`; HIR, run-trap, and lower-C inspection commands use `parsed.decls()`, `lower_c.appendInspection(...)` was replaced by `appendInspectionFromDecls(...)`, and the temporary `ParsedModule.moduleForInspection()` escape was deleted. |
