...
| 258 | Route driver declaration artifacts through the resolved declaration stream. | Complete: `driver_codegen_inputs.zig` now prefers `CompilationSession.resolved_sources.collectDecls(...)` plus `collectFromResolvedDecls(...)` when module graph syntax is attached, leaving `module.decls` collection as a single fallback for non-file/stdin-style callers; inventory gates ratchet the direct driver syntax collection count from two to one. |
