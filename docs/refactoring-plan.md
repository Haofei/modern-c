...
CompilationSession owns file-boundary, module-graph, visibility, IO, parse/check, MIR build, VerifiedProgram construction, artifact output, and metadata sidecar preflight.
...
| 8 | Prototype the next `VerifiedBundle` closure step behind the kernel profile. | Complete for rollback admission: `rollback_install_verified_candidate` now rejects metadata-only tokens, and the production-ops fixture proves only exact-byte `bundle_verify_and_admit_image` output can install a boot candidate. |
...
| 28 | Narrow legacy declaration syntax access behind a typed view. | Complete: C/LLVM backend entrypoints now use `VerifiedProgram.declarationMetadata()` instead of the old raw `syntaxForLegacyDeclarationMetadata()` escape; the legacy raw accessor is exact-zero-gated while the remaining source-map mechanics accessor stays separate. |
...
| 33 | Narrow the remaining source-map syntax mechanics escape. | Complete: source-map generation now consumes `SourceMapMechanicsView`; `VerifiedProgram` exposes `sourceMapMechanics()` instead of raw `syntaxForSourceMapMechanics()`, and row enumeration is isolated behind a mechanics view. |
| 34 | Keep declaration metadata as an explicit view across backend entrypoints. | Complete: C and LLVM verified backend entrypoints now pass `DeclarationMetadataView` into their internal lowering entrypoints; backend-specific early metadata accessors are opened only at the remaining legacy mechanics boundary, while raw-module compatibility wrappers construct explicit declaration metadata views. |
...
| 54 | Split the legacy declaration syntax accessor by backend early-metadata use. | Complete: `DeclarationMetadataView` no longer exposes generic `syntaxForLegacyLowering()`; C and LLVM use explicit `syntaxForCEarlyDeclarationMetadata()` / `syntaxForLlvmEarlyDeclarationMetadata()` accessors and the old generic accessor is exact-zero-gated. |
| 55 | Wrap C/LLVM early declaration metadata syntax access in dedicated views. | Complete: `DeclarationMetadataView` now hands out `CEarlyDeclarationMetadataView` / `LlvmEarlyDeclarationMetadataView`; the old direct backend-specific syntax accessors are exact-zero-gated. |
...
| 59 | Split early declaration metadata view access by comptime-module and declaration-list use. | Complete: C/LLVM early metadata views split the remaining comptime and declaration-list dependencies instead of using a generic syntax scan accessor; the old `syntaxForEarlyDeclarationScan()` methods are exact-zero-gated. |
...
| 61 | Store declaration metadata and source-map subviews as declaration slices. | Complete: `CEarlyDeclarationMetadataView`, `LlvmEarlyDeclarationMetadataView`, and `SourceMapMechanicsView` now store `[]const ast.Decl` instead of `ast.Module`; remaining parent-view module storage was isolated for the next slice. |
| 62 | Store parent declaration metadata view as a declaration slice. | Complete: `DeclarationMetadataView` now stores `[]const ast.Decl` instead of `ast.Module`, so all declaration/source-map mechanics views are decl-slice views; remaining `VerifiedProgram` syntax storage was isolated for the next slice. |
| 63 | Replace legacy module-shaped view constructors with declaration-slice constructors. | Complete: `DeclarationMetadataView` and `SourceMapMechanicsView` now expose `forDecls(...)`; the old `forLegacySyntax(...)` constructors and all call sites are exact-zero-gated. |
| 64 | Store `VerifiedProgram` declaration metadata as a declaration slice. | Complete: `VerifiedProgram` now stores `decls: []const ast.Decl` instead of `syntax_module: ast.Module`; declaration metadata and source-map mechanics are built from that slice. |
| 65 | Construct `VerifiedProgram` from declaration slices. | Complete: `VerifiedProgram.initFromDecls(...)` replaces the old module-shaped initializer; `CompilationSession.buildVerifiedProgram` and direct tests pass `module.decls`, and `backend.VerifiedProgram.init(...)` is exact-zero-gated. |
...
| 69 | Store `VerifiedProgram` syntax escapes as named views. | Complete: `VerifiedProgram` no longer stores a generic declaration slice directly; it stores `DeclarationMetadataView` and `SourceMapMechanicsView`, so remaining syntax-shaped access must go through named mechanics views and the old raw declaration field/count is exact-gated. |
