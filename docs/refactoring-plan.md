...
| 270 | Delete the nested facts collector wrapper. | Complete: the public `Collector` wrapper in `ir_inspection.zig` was removed; top-level inspection functions now instantiate `ModuleFactCollector` directly, while the remaining module-shaped `appendFacts(...)` compatibility path is limited to spec harness callers until that harness can migrate through monomorphized resolved-source fixtures. |
| 271 | Name the remaining module-shaped lower-IR API as spec-harness compatibility. | Complete: `ir_inspection.buildModuleIr(...)` was renamed to `buildModuleIrFromModuleForSpecHarness(...)`; inventory gates keep the generic module-shaped name at zero and make the remaining spec harness dependency explicit. |
