...
| 251 | Drive `mcc symbols` from per-file resolved modules. | Complete: `symbols.emitJsonFromResolvedSources(...)` merges `ResolvedSourceDatabase` modules with `ModuleGraph` source offsets, `runSymbols` uses `session.resolved_sources` when available, and the symbols gate now checks an imported function keeps its imported-file span instead of relying on the legacy combined AST. |
...
| 253 | Drive `mcc list-tests` from per-file resolved modules. | Complete: `runListTests` now enumerates `#[test]` declarations from `CompilationSession.resolved_sources` when available, retains the legacy parse fallback for stdin/single-buffer callers, and `mcc-list-tests-modules-test` proves imported per-file modules contribute discovered tests. |
