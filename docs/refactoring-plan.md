...
| 214 | Rename declaration metadata mechanics by current role. | Complete: `EarlyDeclarationMetadataView` moved from the obsolete `legacy_backend_syntax.zig` container to `early_declaration_metadata.zig`; the backend-facing request now imports `declaration_artifacts.zig`, while CLI/tests still collect the transitional artifacts at the syntax compatibility edge. |
