...
| 8 | Prototype the next `VerifiedBundle` closure step behind the kernel profile. | Complete for rollback admission: metadata-only validation no longer creates `VerifiedBundle`; only exact-byte `bundle_verify_and_admit_image` output can produce the token consumed by rollback or the ELF loader. |
