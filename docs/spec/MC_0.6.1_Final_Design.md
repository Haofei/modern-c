...
checked-arithmetic trap paths, inline asm, related runtime helper calls,
loop/break/continue branch terminators, and switch/if-let dispatches for the
covered backend subset.
...
(`nuw`/`nsw`/`nonnull`/`noalias`/`noundef`/`poison`) in the swept IR. The
...
LLVM `undef`, `poison`, `noundef`, `nonnull`, `noalias`, `nuw`, and `nsw` are not allowed to become hidden semantic assumptions outside the MC verifier rules.
