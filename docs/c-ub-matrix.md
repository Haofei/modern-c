...
| 7 | **Uninitialized read** | Forbidden (must init); explicit `uninit` is unspecified-not-UB; read-before-whole-assign is a compile error | Ordinary `var x: i32;` with no initializer is a compile error (spec §12). `var buf = uninit;` is allowed; its bytes are *unspecified, not UB* (no trap representations / poison). **S0.1 definite-init:** a `var x: T = uninit;` read before it is definitely assigned as a whole value on every control-flow path is `E_USE_BEFORE_INIT` — flow-sensitive (assigned only if assigned on both `if`/`switch` arms; loops conservative; taking `&x` does not count as assignment). Aggregate member/index writes are partial storage uses and do not prove later value reads initialized; use whole assignment or `MaybeUninit<T>` for explicit staged initialization. | `ub_uninit.mc` |
| 8 | **Evaluation order / sequence points** (C-only) | Defined | Evaluation order is part of the language (args left-to-right; binary ops left-then-right; assignment RHS→LHS→store; `&&`/`||` short-circuit). The C backend lowers each subexpression to its own sequenced MIR temporary (`mc_tmp0`, `mc_tmp1`, …), so the emitted C has no unsequenced reads/writes. One construct cannot satisfy both rules at once: an MMIO register read in a short-circuiting `&&`/`||` operand must render *inline* (so it is not executed unconditionally) yet a sequenced temp cannot be hoisted across the short-circuit; a single read per operand is sequencing-safe inline, but **two or more MMIO reads in one `&&`/`||` operand are rejected** (`UnsupportedCEmission`) rather than emitted with unspecified order — refactor to nested `if`s or read each register into a `let` first. | `ub_eval_order.mc` |
