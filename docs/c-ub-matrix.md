...
| 7 | **Uninitialized read** | Forbidden (must init); explicit `uninit` is unspecified-not-UB; **scalar read-before-assign is a compile error** | Ordinary `var x: i32;` with no initializer is a compile error (spec §12). `var buf = uninit;` is allowed; its bytes are *unspecified, not UB* (no trap representations / poison). **S0.1 definite-init:** a *scalar* `var x: T = uninit;` read before it is definitely assigned on every control-flow path is `E_USE_BEFORE_INIT` — flow-sensitive (assigned only if assigned on both `if`/`switch` arms; loops conservative; taking `&x` does not count as assignment). Aggregate member/index storage uses may clear the root to keep field-fill idioms accepted; aggregates filled element/field-at-a-time stay the programmer's obligation (write before read). | `ub_uninit.mc` |
