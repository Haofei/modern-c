// UDP-over-virtio-net runtime: same platform layer as runtime.c (device discovery,
// DMA, vring), but transmits a real UDP datagram built by the MC udp_send demo.
...
int udp_transmit(volatile VirtioMmio *regs, Virtq *txq);
...
__attribute__((used)) void test_main(void) {
    ...
    if (!udp_transmit(regs, &txq)) { puts_("TX-FAIL\n"); goto done; }
    puts_("UDP-TX-OK\n");
