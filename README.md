...
- Debug mapping: `emit-c` writes `#line` source hints for generated C, and
  ...
  terminators plus switch/if-let dispatches. DWARF-quality native debug mapping
  with richer statement/expression coverage is still pending.
...
subprograms, and call/return/loop-branch/switch/if-let dispatch line locations
