//! C backend type-alias resolution helpers.
...
pub fn aliasTargetType(type_aliases: *const std.StringHashMap(ast.TypeExpr), ty: ast.TypeExpr) ?ast.TypeExpr {
    return switch (ty.kind) {
        .name => |name| {
            const target = type_aliases.get(name.text) orelse return null;
            if (typeName(target)) |target_name| {
                if (std.mem.eql(u8, target_name, name.text)) return null;
            }
            return target;
        },
        .qualified => |node| aliasTargetType(type_aliases, node.child.*),
        else => null,
    };
}
