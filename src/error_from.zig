...
pub fn resolve(functions: anytype, signature_types: mir.SignatureTypeTable, from: []const u8, to: []const u8) ?[]const u8 {
    ...
    while (it.next()) |entry| {
        ...
        const p = signature_type_mechanics.nominalName(signature_types, info.params[0].type_id) catch continue orelse continue;
        const rn = signature_type_mechanics.nominalName(signature_types, info.return_type_id) catch continue orelse continue;
        if (std.mem.eql(u8, p, from) and std.mem.eql(u8, rn, to)) return entry.key_ptr.*;
    }
    return null;
}

/// Frontend-only compatibility resolver.  Semantic checking still owns AST
/// function declarations; backend registries must use `resolve` above.
pub fn resolveAst(functions: anytype, from: []const u8, to: []const u8) ?[]const u8 {
    if (std.mem.eql(u8, from, to)) return null;
    var it = functions.iterator();
    while (it.next()) |entry| {
        const info = entry.value_ptr.*;
        if (!info.error_from or info.params.len != 1) continue;
        const p = ast_query.typeName(info.params[0].ty) orelse continue;
        const ret = info.return_ty orelse continue;
...
pub fn resolveTypes(functions: anytype, signature_types: mir.SignatureTypeTable, from_ty: ast.TypeExpr, to_ty: ast.TypeExpr) ?[]const u8 {
    const from = ast_query.typeName(from_ty) orelse return null;
    const to = ast_query.typeName(to_ty) orelse return null;
    return resolve(functions, signature_types, from, to);
}
