...
test "thread spawn boundaries require unsafe checked resource handoff" {
    const source =
        ...
        \\}
        \\fn reject_attr_thread_move_transfer() -> void {
        \\    handoff_send(make_send());
        \\}
        \\fn reject_thread_move_aggregate_transfer() -> void {
    ...
    try std.testing.expectEqual(@as(usize, 7), countDiagnosticCode(&reporter, "E_THREAD_MOVE_RESOURCE"));
...
test "move dynamic-place policy keeps dynamic indexes as overlap-only facts" {
    ...
    try std.testing.expect(wildcard.conflicts(constant_one));
}

test "move dynamic-place policy keeps dynamic overlap behind field boundaries" {
    const root: sema_model.MovePlace = .{ .root = "arr" };
    ...
    try std.testing.expectEqual(
        ...
        sema_model.movePlaceProjectionRelation(.wildcard_index, .{ .constant_index = 0 }),
    );
