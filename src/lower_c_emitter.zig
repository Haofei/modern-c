...
pub fn appendLayoutAsserts(allocator: std.mem.Allocator, module: ast.Module, out: *std.ArrayList(u8), struct_names: []const []const u8) anyerror!void {
    ...
    try emitter.collectModule(backend_mod.DeclarationMetadataView.forDecls(module.decls).cEarlyDeclarationMetadata());
...
pub fn appendModule(
    ...
    try appendModuleMir(allocator, backend_mod.DeclarationMetadataView.forDecls(module.decls).cEarlyDeclarationMetadata(), &typed_mir, out, source_path, ksan, msan, csan, stub_asm, reporter);
}

pub fn appendModuleMir(
    allocator: std.mem.Allocator,
    declarations: backend_mod.CEarlyDeclarationMetadataView,
...
pub const CEmitter = struct {
    ...
    fn emitCleanupEdge(self: *CEmitter, start: usize, kind: backend_cleanup.CleanupEdgeKind, locals: *std.StringHashMap(LocalInfo), return_ty: ?ast.TypeExpr) anyerror!void {
        const function = self.currentMirFunction() orelse return error.UnsupportedCEmission;
        var plan = (try backend_cleanup.buildCleanupEdgePlan(self.allocator, self.mir_module, function.*, self.currentOwnershipCleanupPlan(), self.currentDeferCleanupEdges(), self.currentOwnershipCleanupEdges(), self.defer_stack.items, start, kind)) orelse return error.UnsupportedCEmission;
