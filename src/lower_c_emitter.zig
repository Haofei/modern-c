...
pub const CEmitter = struct {
    ...
    fn callReturnTypeForCall(self: *CEmitter, call: anytype, locals: ?*std.StringHashMap(LocalInfo)) ?ast_bridge.TypeExpr {
        const call_span = call.callee.*.span;
        const call_kind = self.mirCallTargetKindAt(call_span);
        if (self.mirResultReturnTypeForCall(call, .reflection_result)) |ty| return ty;
        if (self.mirResultReturnTypeForCall(call, .byte_view_result)) |ty| return ty;
        if (self.mirResultReturnTypeForCall(call, .bitcast_target)) |ty| return ty;
        if (self.mirResultReturnTypeForCall(call, .conversion_target)) |ty| return ty;
        if (self.mirResultReturnTypeForCall(call, .phys_result)) |ty| return ty;
        if (call_kind) |kind| {
