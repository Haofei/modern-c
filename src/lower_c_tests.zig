...
test "lower-c aggregate-return loop prefix fails closed" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendCheckedCTest("c_loop_prefix_aggregate_return_fail_closed.mc", source, &output);
...
test "lower-c aggregate-return if-let control fails closed" {
    const source =
        \\global shared_counter: u32 = 0;
        \\struct Holder { ptr: *mut u32, tag: u32 }
        \\
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendCheckedCTest("c_if_let_control_aggregate_return_fail_closed.mc", source, &output);
    try expectNotContains(output.items, "/* mir aggregate_return_pointer consumed caller=use_returned_holder callee=returned_holder");
    try expectContains(output.items, "mc_race_load_u32");
}
...
test "lower-c aggregate-return unsafe-block prefix fails closed" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendCheckedCTest("c_unsafe_block_prefix_aggregate_return_fail_closed.mc", source, &output);
    try expectNotContains(output.items, "/* mir aggregate_return_pointer consumed caller=use_returned_holder callee=returned_holder");
    try expectContains(output.items, "mc_race_load_u32");
}

test "lower-c aggregate-return comptime-block prefix fails closed" {
    const source =
        \\global shared_counter: u32 = 0;
        \\struct Holder { ptr: *mut u32, tag: u32 }
        \\
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendCheckedCTest("c_comptime_block_prefix_aggregate_return_fail_closed.mc", source, &output);
    try expectNotContains(output.items, "/* mir aggregate_return_pointer consumed caller=use_returned_holder callee=returned_holder");
    try expectContains(output.items, "mc_race_load_u32");
}

test "lower-c aggregate-return contract-block prefix fails closed" {
    const source =
        \\global shared_counter: u32 = 0;
        \\struct Holder { ptr: *mut u32, tag: u32 }
        \\
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendCheckedCTest("c_contract_block_prefix_aggregate_return_fail_closed.mc", source, &output);
    try expectNotContains(output.items, "/* mir aggregate_return_pointer consumed caller=use_returned_holder callee=returned_holder");
    try expectContains(output.items, "mc_race_load_u32");
}
...
test "lower-c aggregate-return defer prefix fails closed" {
    const source =
        \\global shared_counter: u32 = 0;
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendCheckedCTest("c_defer_prefix_aggregate_return_fail_closed.mc", source, &output);
    try expectNotContains(output.items, "/* mir aggregate_return_pointer consumed caller=use_returned_holder callee=returned_holder");
    try expectContains(output.items, "mc_race_load_u32");
}
