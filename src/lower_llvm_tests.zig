...
test "LLVM aggregate-return loop prefix fails closed" {
    const source =
        \\global shared_counter: u32 = 0;
        \\struct Holder { ptr: *mut u32, tag: u32 }
        \\
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmTest("llvm_loop_prefix_aggregate_return_fail_closed.mc", source, &output);
    const body = try llvmFunctionBody(output.items, "define internal i32 @use_returned_holder");
    try expectNotContains(body, "; mir aggregate_return_pointer consumed caller=use_returned_holder callee=returned_holder");
    try expectContains(body, "load atomic i32, ptr %");
}

test "LLVM aggregate-return nested loop control fails closed" {
    const source =
        \\global shared_counter: u32 = 0;
        \\struct Holder { ptr: *mut u32, tag: u32 }
        \\
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmTest("llvm_nested_loop_control_aggregate_return_fail_closed.mc", source, &output);
    const body = try llvmFunctionBody(output.items, "define internal i32 @use_returned_holder");
    try expectNotContains(body, "; mir aggregate_return_pointer consumed caller=use_returned_holder callee=returned_holder");
    try expectContains(body, "load atomic i32, ptr %");
}
...
test "LLVM aggregate-return if-let control fails closed" {
    const source =
        \\global shared_counter: u32 = 0;
        \\struct Holder { ptr: *mut u32, tag: u32 }
        \\
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmTest("llvm_if_let_control_aggregate_return_fail_closed.mc", source, &output);
    const body = try llvmFunctionBody(output.items, "define internal i32 @use_returned_holder");
    try expectNotContains(body, "; mir aggregate_return_pointer consumed caller=use_returned_holder callee=returned_holder");
    try expectContains(body, "load atomic i32, ptr %");
}
...
test "LLVM aggregate-return unsafe-block prefix fails closed" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmTest("llvm_unsafe_block_prefix_aggregate_return_fail_closed.mc", source, &output);
    const body = try llvmFunctionBody(output.items, "define internal i32 @use_returned_holder");
    try expectNotContains(body, "; mir aggregate_return_pointer consumed caller=use_returned_holder callee=returned_holder");
    try expectContains(body, "load atomic i32, ptr %");
}
...
test "LLVM aggregate-return contract-block prefix fails closed" {
    const source =
        \\global shared_counter: u32 = 0;
        \\struct Holder { ptr: *mut u32, tag: u32 }
        \\
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmTest("llvm_contract_block_prefix_aggregate_return_fail_closed.mc", source, &output);
    const body = try llvmFunctionBody(output.items, "define internal i32 @use_returned_holder");
    try expectNotContains(body, "; mir aggregate_return_pointer consumed caller=use_returned_holder callee=returned_holder");
    try expectContains(body, "load atomic i32, ptr %");
}
...
test "LLVM aggregate-return defer prefix fails closed" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmTest("llvm_defer_prefix_aggregate_return_fail_closed.mc", source, &output);
    const body = try llvmFunctionBody(output.items, "define internal i32 @use_returned_holder");
    try expectNotContains(body, "; mir aggregate_return_pointer consumed caller=use_returned_holder callee=returned_holder");
    try expectContains(body, "load atomic i32, ptr %");
}

test "LLVM aggregate-return for prefix fails closed" {
    const source =
        \\global shared_counter: u32 = 0;
        \\struct Holder { ptr: *mut u32, tag: u32 }
        \\
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmTest("llvm_for_prefix_aggregate_return_fail_closed.mc", source, &output);
    const body = try llvmFunctionBody(output.items, "define internal i32 @use_returned_holder");
    try expectNotContains(body, "; mir aggregate_return_pointer consumed caller=use_returned_holder callee=returned_holder");
    try expectContains(body, "load atomic i32, ptr %");
}
