...
test "LLVM target-typed char literals require MIR facts" {
...

    {
        ...
        var output: std.ArrayList(u8) = .empty;
        defer output.deinit(std.testing.allocator);
        try lower_llvm.appendLlvmCheckedMir(std.testing.allocator, parsed.module, &module_mir, &output, "llvm_char_literal_facts.mc", .{}, false, .riscv64, null);
    ...
    }
    {
        ...
        var output: std.ArrayList(u8) = .empty;
        defer output.deinit(std.testing.allocator);
        try std.testing.expectError(error.InvalidMirTargetTypeFacts, lower_llvm.appendLlvmCheckedMir(std.testing.allocator, parsed.module, &module_mir, &output, "llvm_char_literal_facts.mc", .{}, false, .riscv64, null));
    }
    {
        var module_mir = try mir.buildOpt(std.testing.allocator, parsed.module, .{});
        defer module_mir.deinit();
        ...
        var output: std.ArrayList(u8) = .empty;
        defer output.deinit(std.testing.allocator);
        try std.testing.expectError(error.UnsupportedLlvmEmission, lower_llvm.appendLlvmCheckedMir(std.testing.allocator, parsed.module, &module_mir, &output, "llvm_char_literal_facts.mc", .{}, false, .riscv64, null));
    }
}
...
test "LLVM rejects prebuilt MIR with missing Result try payload representation facts" {
    ...
    try std.testing.expectError(
        error.InvalidMirRepresentationFacts,
        lower_llvm.appendLlvmCheckedMir(std.testing.allocator, parsed.module, &module_mir, &output, "llvm_missing_result_try_payload_representation_facts.mc", .{}, false, .riscv64, null),
    );
}
...
test "LLVM rejects prebuilt MIR with stale Result try payload representation facts" {
    ...
    try std.testing.expectError(
        error.InvalidMirRepresentationFacts,
        lower_llvm.appendLlvmCheckedMir(std.testing.allocator, parsed.module, &module_mir, &output, "llvm_stale_result_try_payload_representation_facts.mc", .{}, false, .riscv64, null),
    );
}
...
test "LLVM consumes MIR drop glue facts and fails closed when absent or stale" {
    ...
    try std.testing.expectError(error.UnsupportedLlvmEmission, lower_llvm.appendLlvmCheckedMir(std.testing.allocator, parsed.module, &module_mir, &missing_auto_drop_event_output, "llvm_drop_glue_mir_facts.mc", .{}, false, .riscv64, null));
    for (module_mir.functions) |*function| {
        if (std.mem.eql(u8, function.name, "auto_drop_from_mir_fact")) {
...
test "LLVM move auto-drop cancellation requires source-matched MIR move-out event" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try std.testing.expectError(error.UnsupportedLlvmEmission, lower_llvm.appendLlvmCheckedMir(std.testing.allocator, parsed.module, &module_mir, &output, "llvm_drop_attr_transfer_move_out_source.mc", .{}, false, .riscv64, null));
}
...
test "LLVM ordinary direct defer with arguments requires MIR argument facts" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try std.testing.expectError(error.UnsupportedLlvmEmission, lower_llvm.appendLlvmCheckedMir(std.testing.allocator, parsed.module, &module_mir, &output, "llvm_ordinary_defer_arg_requires_fact.mc", .{}, false, .riscv64, null));
}
...
test "LLVM ordinary direct defer with discarded result requires MIR result fact" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try std.testing.expectError(error.UnsupportedLlvmEmission, lower_llvm.appendLlvmCheckedMir(std.testing.allocator, parsed.module, &module_mir, &output, "llvm_ordinary_defer_result_requires_fact.mc", .{}, false, .riscv64, null));
}
...
test "LLVM ordinary direct defer may discard call result" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try lower_llvm.appendLlvmChecked(std.testing.allocator, parsed.module, &output, "llvm_ordinary_defer_discard_result.mc", .{}, false, .riscv64);
...
test "LLVM ordinary raw-store defer requires MIR target facts" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try std.testing.expectError(error.UnsupportedLlvmEmission, lower_llvm.appendLlvmCheckedMir(std.testing.allocator, parsed.module, &module_mir, &output, "llvm_ordinary_defer_raw_store_requires_fact.mc", .{}, false, .riscv64, null));
}
...
test "LLVM ordinary raw-store defer emits typed cleanup" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try lower_llvm.appendLlvmChecked(std.testing.allocator, parsed.module, &output, "llvm_ordinary_defer_raw_store_cleanup.mc", .{}, false, .riscv64);
...
test "LLVM ordinary MMIO read defer requires MIR call-target facts" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try std.testing.expectError(error.UnsupportedLlvmEmission, lower_llvm.appendLlvmCheckedMir(std.testing.allocator, parsed.module, &module_mir, &output, "llvm_ordinary_defer_mmio_read_requires_fact.mc", .{}, false, .riscv64, null));
}
...
test "LLVM ordinary MMIO read defer emits typed cleanup" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try lower_llvm.appendLlvmChecked(std.testing.allocator, parsed.module, &output, "llvm_ordinary_defer_mmio_read_cleanup.mc", .{}, false, .riscv64);
...
test "LLVM ordinary DMA cache defer requires MIR call-target facts" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try std.testing.expectError(error.UnsupportedLlvmEmission, lower_llvm.appendLlvmCheckedMir(std.testing.allocator, parsed.module, &module_mir, &output, "llvm_ordinary_defer_dma_cache_requires_fact.mc", .{}, false, .riscv64, null));
}
...
test "LLVM ordinary DMA cache defer emits typed cleanup" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try lower_llvm.appendLlvmChecked(std.testing.allocator, parsed.module, &output, "llvm_ordinary_defer_dma_cache_cleanup.mc", .{}, false, .riscv64);
...
test "LLVM ordinary MaybeUninit write defer requires MIR call-target facts" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try std.testing.expectError(error.UnsupportedLlvmEmission, lower_llvm.appendLlvmCheckedMir(std.testing.allocator, parsed.module, &module_mir, &output, "llvm_ordinary_defer_maybe_uninit_write_requires_fact.mc", .{}, false, .riscv64, null));
}
...
test "LLVM ordinary MaybeUninit write defer emits typed cleanup" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try lower_llvm.appendLlvmChecked(std.testing.allocator, parsed.module, &output, "llvm_ordinary_defer_maybe_uninit_write_cleanup.mc", .{}, false, .riscv64);
...
test "LLVM ordinary atomic store defer requires MIR call-target facts" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try std.testing.expectError(error.UnsupportedLlvmEmission, lower_llvm.appendLlvmCheckedMir(std.testing.allocator, parsed.module, &module_mir, &output, "llvm_ordinary_defer_atomic_store_requires_fact.mc", .{}, false, .riscv64, null));
}
...
test "LLVM ordinary atomic store defer emits typed cleanup" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try lower_llvm.appendLlvmChecked(std.testing.allocator, parsed.module, &output, "llvm_ordinary_defer_atomic_store_cleanup.mc", .{}, false, .riscv64);
...
test "LLVM ordinary va.end defer requires MIR call-target facts" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try std.testing.expectError(error.UnsupportedLlvmEmission, lower_llvm.appendLlvmCheckedMir(std.testing.allocator, parsed.module, &module_mir, &output, "llvm_ordinary_defer_va_end_requires_fact.mc", .{}, false, .riscv64, null));
}
...
test "LLVM ordinary va.end defer emits typed cleanup" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try lower_llvm.appendLlvmChecked(std.testing.allocator, parsed.module, &output, "llvm_ordinary_defer_va_end_cleanup.mc", .{}, false, .riscv64);
...
test "LLVM explicit drop release cancellation requires MIR explicit-drop event" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try std.testing.expectError(error.UnsupportedLlvmEmission, lower_llvm.appendLlvmCheckedMir(std.testing.allocator, parsed.module, &module_mir, &output, "llvm_drop_attr_release_requires_mir_event.mc", .{}, false, .riscv64, null));
}
...
test "LLVM inferred local direct addresses require MIR types" {
    ...
    try std.testing.expectError(error.UnsupportedLlvmEmission, lower_llvm.appendLlvmCheckedMir(std.testing.allocator, parsed.module, &stale_address_local_operand, &stale_address_local_operand_output, "llvm_inferred_local_address.mc", .{}, false, .riscv64, null));
    ...
    try std.testing.expectError(error.UnsupportedLlvmEmission, lower_llvm.appendLlvmCheckedMir(std.testing.allocator, parsed.module, &stale_pointee_source, &stale_pointee_source_output, "llvm_inferred_local_address.mc", .{}, false, .riscv64, null));
}
...
test "LLVM grouped expressions consume their own MIR result facts" {
    ...
    var complete_output: std.ArrayList(u8) = .empty;
    defer complete_output.deinit(std.testing.allocator);
    try lower_llvm.appendLlvmCheckedMir(std.testing.allocator, parsed.module, &complete, &complete_output, "llvm_grouped_expression_result.mc", .{}, false, .riscv64, null);
    ...
    var missing_output: std.ArrayList(u8) = .empty;
    defer missing_output.deinit(std.testing.allocator);
    try std.testing.expectError(error.InvalidMirTargetTypeFacts, lower_llvm.appendLlvmCheckedMir(std.testing.allocator, parsed.module, &missing, &missing_output, "llvm_grouped_expression_result.mc", .{}, false, .riscv64, null));
    ...
    var stale_output: std.ArrayList(u8) = .empty;
    defer stale_output.deinit(std.testing.allocator);
    try std.testing.expectError(error.UnsupportedLlvmEmission, lower_llvm.appendLlvmCheckedMir(std.testing.allocator, parsed.module, &stale, &stale_output, "llvm_grouped_expression_result.mc", .{}, false, .riscv64, null));
}
...
test "LLVM grouped direct calls consume the outer MIR result fact" {
    ...
    var complete_output: std.ArrayList(u8) = .empty;
    defer complete_output.deinit(std.testing.allocator);
    try lower_llvm.appendLlvmCheckedMir(std.testing.allocator, parsed.module, &complete, &complete_output, "llvm_grouped_call_result.mc", .{}, false, .riscv64, null);
    ...
    var missing_output: std.ArrayList(u8) = .empty;
    defer missing_output.deinit(std.testing.allocator);
    try std.testing.expectError(error.InvalidMirTargetTypeFacts, lower_llvm.appendLlvmCheckedMir(std.testing.allocator, parsed.module, &missing, &missing_output, "llvm_grouped_call_result.mc", .{}, false, .riscv64, null));
...
test "LLVM switches require MIR subject types" {
    ...
    try std.testing.expectError(error.InvalidMirTargetTypeFacts, lower_llvm.appendLlvmCheckedMir(std.testing.allocator, parsed.module, &stale_nullable_repr, &stale_nullable_repr_output, "llvm_switch_subject_type_facts.mc", .{}, false, .riscv64, null));
    ...
    try std.testing.expectError(error.InvalidMirTargetTypeFacts, lower_llvm.appendLlvmCheckedMir(std.testing.allocator, parsed.module, &unknown_subject_repr, &unknown_subject_repr_output, "llvm_switch_subject_type_facts.mc", .{}, false, .riscv64, null));
}
...
test "LLVM if-let statements require MIR subject types" {
    ...
    var stale_nullable_repr_output: std.ArrayList(u8) = .empty;
    defer stale_nullable_repr_output.deinit(std.testing.allocator);
    try std.testing.expectError(error.InvalidMirTargetTypeFacts, lower_llvm.appendLlvmCheckedMir(std.testing.allocator, parsed.module, &stale_nullable_repr, &stale_nullable_repr_output, "llvm_if_let_subject_type_facts.mc", .{}, false, .riscv64, null));
}
...
test "LLVM inferred local literals require MIR types" {
    ...
    try std.testing.expectError(error.InvalidMirTargetTypeFacts, lower_llvm.appendLlvmCheckedMir(std.testing.allocator, parsed.module, &missing_literal_result, &missing_literal_result_output, "llvm_inferred_local_literal_types.mc", .{}, false, .riscv64, null));
    ...
    try std.testing.expectError(error.UnsupportedLlvmEmission, lower_llvm.appendLlvmCheckedMir(std.testing.allocator, parsed.module, &stale_literal_result, &stale_literal_result_output, "llvm_inferred_local_literal_types.mc", .{}, false, .riscv64, null));
}
...
test "LLVM sequenced comparison literals require MIR result types" {
    ...
    var complete_output: std.ArrayList(u8) = .empty;
    defer complete_output.deinit(std.testing.allocator);
    try lower_llvm.appendLlvmCheckedMir(std.testing.allocator, parsed.module, &complete, &complete_output, "llvm_condition_literal_result.mc", .{}, false, .riscv64, null);
    ...
    var missing_output: std.ArrayList(u8) = .empty;
    defer missing_output.deinit(std.testing.allocator);
    try std.testing.expectError(error.InvalidMirTargetTypeFacts, lower_llvm.appendLlvmCheckedMir(std.testing.allocator, parsed.module, &missing, &missing_output, "llvm_condition_literal_result.mc", .{}, false, .riscv64, null));
...
test "LLVM sequenced comparison member operands require MIR result types" {
    ...
    var complete_output: std.ArrayList(u8) = .empty;
    defer complete_output.deinit(std.testing.allocator);
    try lower_llvm.appendLlvmCheckedMir(std.testing.allocator, parsed.module, &complete, &complete_output, "llvm_condition_member_result.mc", .{}, false, .riscv64, null);
    ...
    var missing_output: std.ArrayList(u8) = .empty;
    defer missing_output.deinit(std.testing.allocator);
    try std.testing.expectError(error.InvalidMirTargetTypeFacts, lower_llvm.appendLlvmCheckedMir(std.testing.allocator, parsed.module, &missing, &missing_output, "llvm_condition_member_result.mc", .{}, false, .riscv64, null));
    ...
    var stale_output: std.ArrayList(u8) = .empty;
    defer stale_output.deinit(std.testing.allocator);
    try std.testing.expectError(error.UnsupportedLlvmEmission, lower_llvm.appendLlvmCheckedMir(std.testing.allocator, parsed.module, &stale, &stale_output, "llvm_condition_member_result.mc", .{}, false, .riscv64, null));
}
...
test "LLVM inferred local direct calls require MIR types" {
    ...
    try std.testing.expectError(error.InvalidMirTargetTypeFacts, lower_llvm.appendLlvmCheckedMir(std.testing.allocator, parsed.module, &missing_call_result, &missing_call_result_output, "llvm_inferred_local_call_types.mc", .{}, false, .riscv64, null));
    ...
    try std.testing.expectError(error.UnsupportedLlvmEmission, lower_llvm.appendLlvmCheckedMir(std.testing.allocator, parsed.module, &stale_call_result, &stale_call_result_output, "llvm_inferred_local_call_types.mc", .{}, false, .riscv64, null));
}
...
test "LLVM unsupported diagnostics use nearest source span for generated nodes" {
    ...
    var early_metadata = try early_declaration_metadata.EarlyDeclarationArtifacts.collectFromDecls(std.testing.allocator, module.decls);
