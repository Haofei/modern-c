...
test "LLVM derives enums from checked module facts" {
    const source =
        \\enum Status: i8 { negative = -1, ready = 'R' }
        \\fn status() -> Status { return .negative; }
    ;
    var parsed = try test_support.parseCheckedModule("llvm_enum_facts.mc", source);
    defer parsed.deinit();
    var module_mir = try mir.buildOptFromDecls(std.testing.allocator, parsed.decls(), .{});
    defer module_mir.deinit();
    ...
    try expectContains(output.items, "define internal i8 @status(");
    try expectContains(output.items, "ret i8 -1");
}

test "LLVM signature facts prioritize nominal Error over library scalar spelling" {
    const source =
        \\enum Error: u16 { denied = 1 }
        \\extern fn last_error() -> Error;
    ;
    var parsed = try test_support.parseCheckedModule("llvm_nominal_error_signature.mc", source);
    defer parsed.deinit();
    var module_mir = try mir.buildOptFromDecls(std.testing.allocator, parsed.decls(), .{});
    defer module_mir.deinit();
    ...
    try lower_llvm.appendLlvmCheckedMirArtifacts(
        ...
        &module_mir,
        &output,
        "llvm_nominal_error_signature.mc",
        .{},
        false,
        .riscv64,
        false,
    ...
    try expectContains(output.items, "declare i16 @last_error()");
}

test "LLVM derives packed bits from checked module facts" {
    const source =
        \\packed bits Flags: u8 { ready: bool, busy: bool }
        \\fn flags(value: Flags) -> bool { return value.ready; }
    ;
    var parsed = try test_support.parseCheckedModule("llvm_packed_bits_facts.mc", source);
    defer parsed.deinit();
    var module_mir = try mir.buildOptFromDecls(std.testing.allocator, parsed.decls(), .{});
    defer module_mir.deinit();
...
test "LLVM derives overlay unions from checked module facts" {
    const source =
        \\overlay union Overlay { bytes: [4]u8, word: u32 }
        \\fn read_word(value: Overlay) -> u32 { return value.word; }
    ;
    var parsed = try test_support.parseCheckedModule("llvm_overlay_union_facts.mc", source);
    defer parsed.deinit();
    var module_mir = try mir.buildOptFromDecls(std.testing.allocator, parsed.decls(), .{});
    defer module_mir.deinit();
...
test "LLVM derives structs from checked module facts" {
    const source =
        \\struct Pair { left: u32, right: u16 }
        \\fn identity(value: Pair) -> Pair { return value; }
    ;
    var parsed = try test_support.parseCheckedModule("llvm_struct_facts.mc", source);
    defer parsed.deinit();
    var module_mir = try mir.buildOptFromDecls(std.testing.allocator, parsed.decls(), .{});
    defer module_mir.deinit();
    ...
    try expectContains(output.items, "define internal { i32, i16 } @identity");
}
...
test "LLVM canonical MIR renders scalar closure capture through a thunk" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmExecutableMirTest("llvm_mir_scalar_closure.mc", source, &output);
...
test "LLVM canonical MIR renders variadic cursor operations" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmExecutableMirTest("llvm_mir_varargs.mc", source, &output);
...
test "LLVM canonical MIR maps propagated Result errors" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmExecutableMirTest("llvm_mir_try_map_error.mc", source, &output);
...
test "LLVM canonical MIR renders packed field read-modify-write" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmExecutableMirTest("llvm_mir_packed_field_store.mc", source, &output);
...
test "LLVM canonical executable MIR preserves function render attributes" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmExecutableMirTest("llvm_mir_function_attrs.mc", source, &output);
...
test "LLVM canonical executable MIR renders projected structs with fixed-array fields" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmExecutableMirTest("llvm_mir_nested_array_field_layout.mc", source, &output);
...
test "LLVM lexical unsafe and contract call bodies use canonical executable MIR" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmExecutableMirTest("llvm_mir_lexical_contract_calls.mc", source, &output);
...
test "LLVM fixed-array signatures and direct calls use canonical executable MIR" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmExecutableMirTest("llvm_mir_fixed_array_calls.mc", source, &output);
...
test "LLVM fixed-array element addresses use canonical executable MIR" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmExecutableMirTest("llvm_mir_fixed_array_element_address.mc", source, &output);
...
test "LLVM callable parameters forward through canonical executable MIR" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmExecutableMirTest("llvm_mir_callable_parameter.mc", source, &output);
...
test "LLVM callable field stores use verified signatures" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmExecutableMirTest("llvm_mir_callable_field_store.mc", source, &output);
...
test "LLVM valid slice representation check uses canonical executable MIR" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmExecutableMirTest("llvm_mir_valid_slice.mc", source, &output);
...
test "LLVM value optional construction needs no function body fallback" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmExecutableMirTest("llvm_mir_value_optional.mc", source, &output);
...
test "LLVM atomic loads use canonical executable MIR" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmExecutableMirTest("llvm_mir_atomic_load.mc", source, &output);
...
test "LLVM atomic updates use canonical executable MIR" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmExecutableMirTest("llvm_mir_atomic_update.mc", source, &output);
...
test "LLVM MMIO scalar accesses use canonical executable MIR" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmExecutableMirTest("llvm_mir_mmio_scalar.mc", source, &output);
...
test "LLVM shared structural body plans cover nested, aggregate, workflow, and hoisted-loop families" {
    ...
    for (cases) |case| {
        ...
        var output: std.ArrayList(u8) = .empty;
        defer output.deinit(std.testing.allocator);
        try appendLlvmExecutableMirTest(case.name, source, &output);
...
test "LLVM emits assertion expression trees from MIR without body fallback" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmExecutableMirTest("llvm_mir_assert_expression_tree.mc", source, &output);
...
test "LLVM struct literal fields evaluate in lexical source order" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmExecutableMirTest("llvm_mir_struct_literal_order.mc", source, &output);
...
test "LLVM struct literal call fields lower from MIR without body fallback" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmExecutableMirTest("llvm_mir_struct_literal_call_fields.mc", source, &output);
...
test "LLVM array literal call elements lower from MIR without body fallback" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmExecutableMirTest("llvm_mir_array_literal_call_elements.mc", source, &output);
...
test "LLVM local uninit aggregate assignment returns lower from MIR without body fallback" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmExecutableMirTest("llvm_mir_local_uninit_aggregate_assignment.mc", source, &output);
...
test "LLVM local aggregate place updates return from MIR without body fallback" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmExecutableMirTest("llvm_mir_local_aggregate_place_update.mc", source, &output);
...
test "LLVM direct-call aggregate projections return from MIR without body fallback" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmExecutableMirTest("llvm_mir_direct_call_aggregate_projections.mc", source, &output);
...
test "LLVM returns first fixed-array element from MIR CFG without body fallback" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmExecutableMirTest("llvm_mir_canonical_foreach_return.mc", source, &output);
...
test "LLVM emits break and continue while CFG from MIR without body fallback" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmExecutableMirTest("llvm_mir_while_control.mc", source, &output);
...
test "LLVM function symbol returns lower from MIR without body fallback" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmExecutableMirTest("llvm_mir_identity_return.mc", source, &output);
...
test "LLVM emits slice length returns from shared MIR plan without body fallback" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmExecutableMirTest("llvm_mir_slice_length_return.mc", source, &output);
...
test "LLVM emits slice foreach local updates from MIR without body fallback" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmExecutableMirTest("llvm_mir_canonical_foreach_update.mc", source, &output);
...
test "LLVM literal unary components lower from MIR without body fallback" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmExecutableMirTest("llvm_mir_literal_unary_components.mc", source, &output);
...
test "LLVM literal compare components lower from MIR without body fallback" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmExecutableMirTest("llvm_mir_literal_compare_components.mc", source, &output);
...
test "LLVM literal checked arithmetic components lower from MIR without body fallback" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmExecutableMirTest("llvm_mir_literal_checked_arithmetic_components.mc", source, &output);
...
test "LLVM literal checked unary components lower from MIR without body fallback" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmExecutableMirTest("llvm_mir_literal_checked_unary_components.mc", source, &output);
...
test "LLVM local literal checked components return from MIR without body fallback" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmExecutableMirTest("llvm_mir_local_literal_checked_components.mc", source, &output);
...
test "LLVM assigned literal checked components return from MIR without body fallback" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmExecutableMirTest("llvm_mir_assigned_literal_checked_components.mc", source, &output);
...
test "LLVM local and assigned literal call components return from MIR without body fallback" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmExecutableMirTest("llvm_mir_local_assigned_literal_call_components.mc", source, &output);
...
test "LLVM local and assigned aggregate direct calls return from MIR without body fallback" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmExecutableMirTest("llvm_mir_local_assigned_aggregate_direct_calls.mc", source, &output);
...
test "LLVM grouped scalar expressions return from MIR without body fallback" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmExecutableMirTest("llvm_mir_grouped_scalar_returns.mc", source, &output);
...
test "LLVM void calls before grouped scalar returns lower from MIR without body fallback" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmExecutableMirTest("llvm_mir_void_calls_before_grouped_scalar_returns.mc", source, &output);
...
test "LLVM conditional grouped scalar returns lower from MIR without body fallback" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmExecutableMirTest("llvm_mir_conditional_grouped_scalar_returns.mc", source, &output);
...
test "LLVM conditional global and call returns lower from MIR without body fallback" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmExecutableMirTest("llvm_mir_conditional_global_call_returns.mc", source, &output);
...
test "LLVM loop grouped scalar returns lower from MIR without body fallback" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmExecutableMirTest("llvm_mir_loop_grouped_scalar_returns.mc", source, &output);
...
test "LLVM loop derived scalar returns lower from MIR without body fallback" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmExecutableMirTest("llvm_mir_loop_derived_scalar_returns.mc", source, &output);
...
test "LLVM loop checked scalar returns lower from MIR without body fallback" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmExecutableMirTest("llvm_mir_loop_checked_scalar_returns.mc", source, &output);
...
test "LLVM loop call and global returns lower from MIR without body fallback" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmExecutableMirTest("llvm_mir_loop_call_global_returns.mc", source, &output);
...
test "LLVM MIR conditional fast path uses only the switch subject expression" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmExecutableMirTest("llvm_mir_conditional_subject.mc", source, &output);
...
test "LLVM emits simple void conditional direct calls from MIR" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmExecutableMirTest("llvm_mir_void_conditional_calls.mc", source, &output);
...
test "LLVM emits simple sequential void direct calls from MIR" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmExecutableMirTest("llvm_mir_void_call_sequence.mc", source, &output);
...
test "LLVM emits pure local-only void functions from MIR" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmExecutableMirTest("llvm_mir_void_local_only.mc", source, &output);
...
test "LLVM preserves MIR void calls before simple returns" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmExecutableMirTest("llvm_mir_void_calls_before_return.mc", source, &output);
...
test "LLVM emits direct struct parameter field returns from MIR" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmExecutableMirTest("llvm_mir_param_field_return.mc", source, &output);
...
test "LLVM emits nested parameter and global field places from MIR without body fallback" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmExecutableMirTest("llvm_mir_nested_place_return.mc", source, &output);
...
test "LLVM emits fixed-array constant-index places from MIR without body fallback" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmExecutableMirTest("llvm_mir_array_place_return.mc", source, &output);
...
test "LLVM checked dynamic fixed-array stores use canonical executable MIR" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmExecutableMirTest("llvm_mir_dynamic_array_store.mc", source, &output);
...
test "LLVM emits conditional struct parameter field returns from MIR" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmExecutableMirTest("llvm_mir_conditional_param_field_return.mc", source, &output);
...
test "LLVM emits conditional boolean struct field conditions from MIR" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmExecutableMirTest("llvm_mir_conditional_param_bool_field.mc", source, &output);
...
test "LLVM emits struct parameter field call arguments from MIR" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmExecutableMirTest("llvm_mir_param_field_call_args.mc", source, &output);
...
test "LLVM emits struct parameter field checked operands from MIR" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmExecutableMirTest("llvm_mir_param_field_checked_operands.mc", source, &output);
...
test "LLVM emits struct parameter field comparisons from MIR" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmExecutableMirTest("llvm_mir_param_field_compare_operands.mc", source, &output);
...
test "LLVM emits simple struct literal returns from MIR" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmExecutableMirTest("llvm_mir_struct_literal_returns.mc", source, &output);
...
test "LLVM canonical executable MIR emits nested by-value struct member reads" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmExecutableMirTest("llvm_executable_struct_member.mc", source, &output);
...
test "LLVM canonical executable MIR emits nested parameter array indexes" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmExecutableMirTest("llvm_mir_nested_parameter_array_index.mc", source, &output);
...
test "LLVM canonical executable MIR emits guarded pointer member reads" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmExecutableMirTest("llvm_executable_pointer_member.mc", source, &output);
...
test "LLVM canonical executable local pointer deref owns its representation edge" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmExecutableMirTest("llvm_executable_local_pointer_deref.mc", source, &output);
...
test "LLVM canonical executable MIR keeps ordinary len fields distinct from slice length" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmExecutableMirTest("llvm_mir_struct_len_field.mc", source, &output);
...
test "LLVM emits simple array literal returns from MIR" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmExecutableMirTest("llvm_mir_array_literal_returns.mc", source, &output);
...
test "LLVM emits array control-flow returns from MIR" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmExecutableMirTest("llvm_mir_array_control_returns.mc", source, &output);
...
test "LLVM emits scalar comparison returns from MIR" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmExecutableMirTest("llvm_mir_scalar_comparison_returns.mc", source, &output);
...
test "LLVM emits typed unary call-target returns from MIR without body fallback" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmExecutableMirTest("llvm_mir_typed_unary_call_target_returns.mc", source, &output);
...
test "LLVM emits typed binary domain calls from MIR without body fallback" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmExecutableMirTest("llvm_mir_typed_binary_domain_calls.mc", source, &output);
...
test "LLVM emits checked arithmetic returns from MIR without body fallback" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmExecutableMirTest("llvm_mir_checked_arithmetic_returns.mc", source, &output);
...
test "LLVM emits checked division returns from MIR without body fallback" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmExecutableMirTest("llvm_mir_checked_division_returns.mc", source, &output);
...
test "LLVM emits checked mod and shift returns from MIR without body fallback" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmExecutableMirTest("llvm_mir_checked_mod_shift_returns.mc", source, &output);
...
test "LLVM emits checked unary returns from MIR without body fallback" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmExecutableMirTest("llvm_mir_checked_unary_returns.mc", source, &output);
...
test "LLVM target-types negated integer literals in canonical MIR" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmExecutableMirTest("llvm_mir_target_typed_negated_literals.mc", source, &output);
...
test "LLVM emits logical-not returns from MIR without body fallback" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmExecutableMirTest("llvm_mir_logical_not_returns.mc", source, &output);
...
test "LLVM emits basic scalar returns from MIR without body fallback" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmExecutableMirTest("llvm_mir_basic_scalar_returns.mc", source, &output);
...
test "LLVM emits local and assigned scalar returns from MIR without body fallback" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmExecutableMirTest("llvm_mir_local_assigned_scalar_returns.mc", source, &output);
...
test "LLVM preserves nullable pointer promotion locals from MIR without body fallback" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmExecutableMirTest("llvm_mir_nullable_pointer_promotions.mc", source, &output);
...
test "LLVM emits nullable pointer try from MIR without body fallback" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmExecutableMirTest("llvm_mir_nullable_pointer_try.mc", source, &output);
...
test "LLVM emits value optional and Result try from MIR without body fallback" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmExecutableMirTest("llvm_mir_aggregate_try.mc", source, &output);
...
test "LLVM emits nullable none returns from MIR without body fallback" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmExecutableMirTest("llvm_mir_nullable_none_returns.mc", source, &output);
...
test "LLVM emits conditional nullable none returns from MIR without body fallback" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmExecutableMirTest("llvm_mir_conditional_nullable_none_returns.mc", source, &output);
...
test "LLVM preserves MIR void calls before nullable none returns" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmExecutableMirTest("llvm_mir_void_calls_before_nullable_none_return.mc", source, &output);
...
test "LLVM emits loop nullable none returns from MIR without body fallback" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmExecutableMirTest("llvm_mir_loop_nullable_none_return.mc", source, &output);
...
test "LLVM emits enum literal returns from MIR without body fallback" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmExecutableMirTest("llvm_mir_enum_literal_return.mc", source, &output);
...
test "LLVM emits enum variant raw values from MIR without body fallback" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmExecutableMirTest("llvm_mir_enum_variant_raw.mc", source, &output);
...
test "LLVM emits nominal scalar resource flow from MIR without body fallback" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmExecutableMirTest("llvm_mir_nominal_scalar_resource.mc", source, &output);
...
test "LLVM emits nested fixed-array aggregates from MIR without body fallback" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmExecutableMirTest("llvm_mir_nested_array_aggregate.mc", source, &output);
...
test "LLVM compares value optionals with null from MIR without body fallback" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmExecutableMirTest("llvm_mir_optional_null_compare.mc", source, &output);
...
test "LLVM emits local and loop enum returns from MIR without body fallback" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmExecutableMirTest("llvm_mir_local_loop_enum_returns.mc", source, &output);
...
test "LLVM preserves MIR void calls before local enum returns" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmExecutableMirTest("llvm_mir_void_calls_before_local_enum_return.mc", source, &output);
...
test "LLVM emits conditional enum literal returns from MIR without body fallback" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmExecutableMirTest("llvm_mir_conditional_enum_literal_return.mc", source, &output);
...
test "LLVM preserves MIR void calls before direct-call returns" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmExecutableMirTest("llvm_mir_void_calls_before_direct_call_return.mc", source, &output);
...
test "LLVM emits enum literal direct-call arguments from MIR without body fallback" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmExecutableMirTest("llvm_mir_enum_direct_call_argument.mc", source, &output);
...
test "LLVM emits enum literal compare operands from MIR without body fallback" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmExecutableMirTest("llvm_mir_enum_literal_compare_operands.mc", source, &output);
...
test "LLVM emits enum literal explicit casts from MIR without body fallback" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmExecutableMirTest("llvm_mir_enum_literal_explicit_cast.mc", source, &output);
...
test "LLVM emits enum, pointer-address, and signedness casts from executable MIR" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmExecutableMirTest("llvm_mir_representation_casts.mc", source, &output);
...
test "LLVM emits transparent integer domain casts from executable MIR" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmExecutableMirTest("llvm_mir_domain_casts.mc", source, &output);
...
test "LLVM scalar switch returns lower from MIR without body fallback" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmExecutableMirTest("llvm_mir_scalar_switch.mc", source, &output);
...
test "LLVM emits local global returns from MIR" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmExecutableMirTest("llvm_mir_local_global_return.mc", source, &output);
...
test "LLVM inferred local global return lowers without function body fallback" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmExecutableMirTest("llvm_mir_inferred_local_global_return.mc", source, &output);
...
test "LLVM preserves MIR void calls before global returns" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmExecutableMirTest("llvm_mir_void_calls_before_global_return.mc", source, &output);
...
test "LLVM preserves MIR void calls before conditional returns" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmExecutableMirTest("llvm_mir_void_calls_before_conditional_return.mc", source, &output);
...
test "LLVM discarded direct-call results lower from MIR without body fallback" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmExecutableMirTest("llvm_mir_discarded_direct_call_result.mc", source, &output);
...
test "LLVM zero-argument function-pointer calls lower from MIR without body fallback" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmExecutableMirTest("llvm_mir_zero_arg_function_pointer_calls.mc", source, &output);
...
test "LLVM typed indirect call returns lower from MIR without body fallback" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmExecutableMirTest("llvm_mir_typed_indirect_call_returns.mc", source, &output);
...
fn appendLlvmCheckedMirProfileDeclsTest(allocator: std.mem.Allocator, _decls: []ast.Decl, module_mir: *const mir.Module, output: *std.ArrayList(u8), source_path: []const u8, checks: backend_mod.Checks, stub_asm: bool, target: backend_mod.TargetArch, linux_kernel: bool, reporter: ?*diagnostics.Reporter) !void {
    // Retain the parsed declarations in this test-helper signature so existing
    // malformed-MIR fixtures stay concise. LLVM lowering receives only the
    // already-verified module; it no longer constructs declaration artifacts.
    _ = _decls;
...
test "LLVM renders no-init scalar and array globals from verified zero plans" {
    const source =
        \\global COUNT: u32;
        \\global VALUES: [2]u32;
        \\struct State { count: u32, ready: bool }
        \\type StateAlias = State;
        \\global STATE: State;
        \\global STATE_ALIAS: StateAlias;
    ;
    ...
    try expectContains(output.items, "@STATE = internal global { i32, i1 } zeroinitializer");
    try expectContains(output.items, "@STATE_ALIAS = internal global { i32, i1 } zeroinitializer");
}
...
test "LLVM emits direct global-address plans without AST initializer artifacts" {
    const source =
        \\global shared: u32 = 7;
        \\global shared_ptr: *mut u32 = &shared;
    ;
    var parsed = try test_support.parseCheckedModule("llvm_global_address_initializer_plan.mc", source);
    defer parsed.deinit();
    var module_mir = try mir.buildFromDecls(std.testing.allocator, parsed.decls());
    defer module_mir.deinit();
...
test "LLVM emits named struct global literals from syntax-free plans" {
    const source =
        \\open enum Mode: u32 { ready = 7 }
    ...
    ;
    var parsed = try test_support.parseCheckedModule("llvm_named_struct_global_plan.mc", source);
    defer parsed.deinit();
    var module_mir = try mir.buildFromDecls(std.testing.allocator, parsed.decls());
    defer module_mir.deinit();
...
test "LLVM generated locals and blocks avoid source parameter names" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmExecutableMirTest("llvm_mir_local_name_collisions.mc", source, &output);
...
test "LLVM nominal declarations shadow same-named library scalars" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmExecutableMirTest("llvm_mir_shadowed_library_scalar.mc", source, &output);
...
test "LLVM target-typed char literals require MIR facts" {
...

    {
        var output: std.ArrayList(u8) = .empty;
        defer output.deinit(std.testing.allocator);
        try appendLlvmExecutableMirTest("llvm_mir_char_literal_facts.mc", source, &output);
...
test "LLVM local and assigned char literal returns lower without body fallback" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmExecutableMirTest("llvm_mir_local_assigned_char_literal_return.mc", source, &output);
...
test "LLVM float literal returns lower without body fallback" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmExecutableMirTest("llvm_mir_float_literal_return.mc", source, &output);
...
test "LLVM rejects prebuilt MIR with missing target type facts" {
    ...
    try appendLlvmExecutableMirTest("llvm_mir_result_constructor_target_type_facts.mc", source, &complete_output);
...
test "LLVM consumes f32 and f64 literal target type facts" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmExecutableMirTest("llvm_mir_float_target_type_facts.mc", source, &output);
...
test "LLVM conversion literal source type comes from MIR" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmExecutableMirTest("llvm_mir_conversion_literal_source_type.mc", source, &output);
...
test "LLVM negative integer literal uses the MIR unary result type" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmExecutableMirTest("llvm_mir_negative_integer_literal_return.mc", source, &output);
...
test "LLVM explicit casts require MIR source and target type facts" {
    ...
    var complete_output: std.ArrayList(u8) = .empty;
    defer complete_output.deinit(std.testing.allocator);
    try appendLlvmExecutableMirTest("llvm_mir_explicit_cast_type_facts.mc", source, &complete_output);
...
test "LLVM local and assigned explicit casts lower from MIR without body fallback" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmExecutableMirTest("llvm_mir_local_assigned_explicit_cast_return.mc", source, &output);
...
test "LLVM local and assigned conversion calls lower from MIR without body fallback" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmExecutableMirTest("llvm_mir_local_assigned_conversion_return.mc", source, &output);
...
test "LLVM rejects prebuilt MIR with missing integer facts" {
    ...
    var complete_output: std.ArrayList(u8) = .empty;
    defer complete_output.deinit(std.testing.allocator);
    try appendLlvmExecutableMirTest("llvm_mir_integer_fact_gate.mc", source, &complete_output);
...
test "LLVM executable MIR forget evaluates its operand once without a release call" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmExecutableMirTest("llvm_mir_discard_value.mc", source, &output);
...
test "LLVM emits wrapping arithmetic call from MIR without body fallback" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmExecutableMirTest("llvm_mir_wrapping_call.mc", source, &output);
...
test "LLVM emits local wrapping arithmetic from MIR without body fallback" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmExecutableMirTest("llvm_mir_local_wrapping_call.mc", source, &output);
...
test "LLVM emits unchecked arithmetic call from MIR without body fallback" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmExecutableMirTest("llvm_mir_unchecked_call.mc", source, &output);
...
test "LLVM emits local unchecked arithmetic from MIR without body fallback" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmExecutableMirTest("llvm_mir_local_unchecked_call.mc", source, &output);
...
test "LLVM emits unchecked sub and mul returns from MIR without body fallback" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmExecutableMirTest("llvm_mir_unchecked_sub_mul_calls.mc", source, &output);
...
test "LLVM atomic init requires MIR identity and complete types" {
    ...
    for ([_][]const u8{ "boot_counter", "local_init" }) |name| {
        ...
        try std.testing.expectError(error.InvalidMirTargetTypeFacts, appendLlvmCheckedMirDeclsTest(std.testing.allocator, parsed.decls(), &stale_payload, &stale_payload_output, "llvm_atomic_init_facts.mc", .{}, false, .riscv64, null));
...
test "LLVM inferred local try payloads require MIR types" {
    ...
    var stale_output: std.ArrayList(u8) = .empty;
    defer stale_output.deinit(std.testing.allocator);
    try std.testing.expectError(error.InvalidMirTargetTypeFacts, appendLlvmCheckedMirDeclsTest(std.testing.allocator, parsed.decls(), &stale, &stale_output, "llvm_inferred_local_try_payloads.mc", .{}, false, .riscv64, null));
}
...
test "LLVM grouped expressions consume their own MIR result facts" {
    ...
    var complete_output: std.ArrayList(u8) = .empty;
    defer complete_output.deinit(std.testing.allocator);
    try appendLlvmExecutableMirTest("llvm_mir_grouped_expression_result_fact_gate.mc", source, &complete_output);
...
test "LLVM grouped direct calls consume the outer MIR result fact" {
    ...
    var complete_output: std.ArrayList(u8) = .empty;
    defer complete_output.deinit(std.testing.allocator);
    try appendLlvmExecutableMirTest("llvm_mir_grouped_call_result_fact_gate.mc", source, &complete_output);
...
test "LLVM direct-call inferred local lowers without function body fallback" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmExecutableMirTest("llvm_mir_inferred_local_direct_call_return.mc", source, &output);
...
test "LLVM literal inferred local lowers without function body fallback" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmExecutableMirTest("llvm_mir_inferred_local_literal_return.mc", source, &output);
...
test "LLVM bool-literal inferred local lowers without function body fallback" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmExecutableMirTest("llvm_mir_inferred_local_bool_literal_return.mc", source, &output);
...
test "LLVM checked-unary inferred local lowers without function body fallback" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmExecutableMirTest("llvm_mir_inferred_local_checked_unary_return.mc", source, &output);
...
test "LLVM checked-binary inferred local lowers without function body fallback" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmExecutableMirTest("llvm_mir_inferred_local_checked_binary_return.mc", source, &output);
...
test "LLVM logical-not inferred local lowers without function body fallback" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmExecutableMirTest("llvm_mir_inferred_local_logical_not_return.mc", source, &output);
...
test "LLVM logical return tree lowers without function body fallback" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmExecutableMirTest("llvm_mir_logical_return_tree.mc", source, &output);
...
test "LLVM compare inferred local lowers without function body fallback" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmExecutableMirTest("llvm_mir_inferred_local_compare_return.mc", source, &output);
...
test "LLVM copied inferred local lowers without function body fallback" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmExecutableMirTest("llvm_mir_inferred_local_copy_return.mc", source, &output);
...
test "LLVM param-field copied inferred local lowers without function body fallback" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmExecutableMirTest("llvm_mir_inferred_local_param_field_copy_return.mc", source, &output);
...
test "LLVM null inferred local lowers without function body fallback" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmExecutableMirTest("llvm_mir_inferred_local_null_return.mc", source, &output);
...
test "LLVM emits explicit traps from MIR without body fallback" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmExecutableMirTest("llvm_mir_explicit_traps.mc", source, &output);
...
test "LLVM runtime asserts require MIR bool condition types" {
    ...
    var stale_output: std.ArrayList(u8) = .empty;
    defer stale_output.deinit(std.testing.allocator);
    try std.testing.expectError(error.InvalidMirTargetTypeFacts, appendLlvmCheckedMirDeclsTest(std.testing.allocator, parsed.decls(), &stale, &stale_output, "llvm_assert_condition_type_facts.mc", .{}, false, .riscv64, null));
}
...
test "LLVM emits runtime assert from MIR without body fallback" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmExecutableMirTest("llvm_mir_runtime_assert.mc", source, &output);
...
test "LLVM while loops require MIR bool condition types" {
    ...
    var stale_output: std.ArrayList(u8) = .empty;
    defer stale_output.deinit(std.testing.allocator);
    try std.testing.expectError(error.InvalidMirTargetTypeFacts, appendLlvmCheckedMirDeclsTest(std.testing.allocator, parsed.decls(), &stale, &stale_output, "llvm_loop_condition_type_facts.mc", .{}, false, .riscv64, null));
}
...
test "LLVM emits void-returning while loop from MIR without body fallback" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmExecutableMirTest("llvm_mir_void_returning_while_loop.mc", source, &output);
...
test "LLVM emits enum switch returns from MIR without body fallback" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmExecutableMirTest("llvm_mir_enum_switch_returns.mc", source, &output);
    try expectContains(output.items, "; canonical executable MIR");
...
test "LLVM emits multi-arm enum switch returns from MIR without body fallback" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmExecutableMirTest("llvm_mir_enum_switch_multi_arm_returns.mc", source, &output);
    try expectContains(output.items, "; canonical executable MIR");
    try expectContains(output.items, "switch ");
    try expectContains(output.items, "label %mc_block_");
    try expectContains(output.items, "ret i32 1");
...
test "LLVM try expressions require MIR operand and result types" {
    ...
    for ([_][]const u8{ "result_try", "nullable_try" }) |name| {
        ...
        var stale_result_output: std.ArrayList(u8) = .empty;
        defer stale_result_output.deinit(std.testing.allocator);
        try std.testing.expectError(error.InvalidMirTargetTypeFacts, appendLlvmCheckedMirDeclsTest(std.testing.allocator, parsed.decls(), &stale_result, &stale_result_output, "llvm_try_operand_type_facts.mc", .{}, false, .riscv64, null));
    }
}
...
test "LLVM for loops require MIR iterable and element types" {
    ...
    for ([_]mir.TargetTypeKind{ .for_iterable, .for_element }) |kind| {
        ...
        var stale_output: std.ArrayList(u8) = .empty;
        defer stale_output.deinit(std.testing.allocator);
        try std.testing.expectError(error.InvalidMirTargetTypeFacts, appendLlvmCheckedMirDeclsTest(std.testing.allocator, parsed.decls(), &stale, &stale_output, "llvm_for_loop_type_facts.mc", .{}, false, .riscv64, null));
    }
}
...
test "LLVM inferred local copies require MIR types" {
    ...
    var stale_output: std.ArrayList(u8) = .empty;
    defer stale_output.deinit(std.testing.allocator);
    try std.testing.expectError(error.InvalidMirTargetTypeFacts, appendLlvmCheckedMirDeclsTest(std.testing.allocator, parsed.decls(), &stale, &stale_output, "llvm_inferred_local_copy_types.mc", .{}, false, .riscv64, null));
}
...
test "LLVM inferred local literals require MIR types" {
    ...
    var stale_output: std.ArrayList(u8) = .empty;
    defer stale_output.deinit(std.testing.allocator);
    try std.testing.expectError(error.InvalidMirTargetTypeFacts, appendLlvmCheckedMirDeclsTest(std.testing.allocator, parsed.decls(), &stale, &stale_output, "llvm_inferred_local_literal_types.mc", .{}, false, .riscv64, null));
    ...
    try std.testing.expectError(error.InvalidMirTargetTypeFacts, appendLlvmCheckedMirDeclsTest(std.testing.allocator, parsed.decls(), &stale_literal_result, &stale_literal_result_output, "llvm_inferred_local_literal_types.mc", .{}, false, .riscv64, null));
}
...
test "LLVM inferred local unary expressions require MIR types" {
    ...
    var stale_output: std.ArrayList(u8) = .empty;
    defer stale_output.deinit(std.testing.allocator);
    try std.testing.expectError(error.InvalidMirTargetTypeFacts, appendLlvmCheckedMirDeclsTest(std.testing.allocator, parsed.decls(), &stale, &stale_output, "llvm_inferred_local_unary_types.mc", .{}, false, .riscv64, null));
}
...
test "LLVM inferred local direct calls require MIR types" {
    ...
    var complete_output: std.ArrayList(u8) = .empty;
    defer complete_output.deinit(std.testing.allocator);
    try appendLlvmExecutableMirTest("llvm_mir_inferred_local_call_types.mc", source, &complete_output);
...
test "LLVM inferred local Result direct calls require MIR types" {
    ...
    var stale_output: std.ArrayList(u8) = .empty;
    defer stale_output.deinit(std.testing.allocator);
    try std.testing.expectError(error.InvalidMirTargetTypeFacts, appendLlvmCheckedMirDeclsTest(std.testing.allocator, parsed.decls(), &stale, &stale_output, "llvm_inferred_local_result_call_types.mc", .{}, false, .riscv64, null));
}
...
test "LLVM inferred local indirect calls require MIR types" {
    ...
    var stale_output: std.ArrayList(u8) = .empty;
    defer stale_output.deinit(std.testing.allocator);
    try std.testing.expectError(error.InvalidMirTargetTypeFacts, appendLlvmCheckedMirDeclsTest(std.testing.allocator, parsed.decls(), &stale, &stale_output, "llvm_inferred_local_indirect_call_types.mc", .{}, false, .riscv64, null));
}
...
test "LLVM inferred local atomic and MaybeUninit calls require MIR types" {
    ...
    for ([_][]const u8{ "atomic_inferred_locals", "maybe_uninit_inferred_local" }) |name| {
        ...
        var stale_output: std.ArrayList(u8) = .empty;
        defer stale_output.deinit(std.testing.allocator);
        try std.testing.expectError(error.InvalidMirTargetTypeFacts, appendLlvmCheckedMirDeclsTest(std.testing.allocator, parsed.decls(), &stale, &stale_output, "llvm_builtin_inferred_local_types.mc", .{}, false, .riscv64, null));
    }
}
...
test "LLVM inferred local phys calls require MIR types" {
    ...
    var stale_output: std.ArrayList(u8) = .empty;
    defer stale_output.deinit(std.testing.allocator);
    try std.testing.expectError(error.InvalidMirTargetTypeFacts, appendLlvmCheckedMirDeclsTest(std.testing.allocator, parsed.decls(), &stale, &stale_output, "llvm_inferred_phys_local_type.mc", .{}, false, .riscv64, null));
}
...
test "LLVM inferred local bitcast calls require MIR types" {
    ...
    var stale_output: std.ArrayList(u8) = .empty;
    defer stale_output.deinit(std.testing.allocator);
    try std.testing.expectError(error.InvalidMirTargetTypeFacts, appendLlvmCheckedMirDeclsTest(std.testing.allocator, parsed.decls(), &stale, &stale_output, "llvm_inferred_bitcast_local_type.mc", .{}, false, .riscv64, null));
}
...
test "LLVM inferred local byte-view calls require MIR types" {
    ...
    for ([_][]const u8{ "inferred_byte_view", "inferred_byte_equal" }) |name| {
        ...
        var stale_output: std.ArrayList(u8) = .empty;
        defer stale_output.deinit(std.testing.allocator);
        try std.testing.expectError(error.InvalidMirTargetTypeFacts, appendLlvmCheckedMirDeclsTest(std.testing.allocator, parsed.decls(), &stale, &stale_output, "llvm_inferred_byte_view_local_types.mc", .{}, false, .riscv64, null));
    }
}
...
test "LLVM inferred local enum raw calls require MIR types" {
    ...
    var stale_output: std.ArrayList(u8) = .empty;
    defer stale_output.deinit(std.testing.allocator);
    try std.testing.expectError(error.InvalidMirTargetTypeFacts, appendLlvmCheckedMirDeclsTest(std.testing.allocator, parsed.decls(), &stale, &stale_output, "llvm_inferred_enum_raw_local_type.mc", .{}, false, .riscv64, null));
}
...
test "LLVM inferred local reflection calls require MIR types" {
    ...
    var stale_output: std.ArrayList(u8) = .empty;
    defer stale_output.deinit(std.testing.allocator);
    try std.testing.expectError(error.InvalidMirTargetTypeFacts, appendLlvmCheckedMirDeclsTest(std.testing.allocator, parsed.decls(), &stale, &stale_output, "llvm_inferred_reflection_local_type.mc", .{}, false, .riscv64, null));
}
...
test "LLVM inferred local semantic escape calls require MIR types" {
    ...
    var stale_output: std.ArrayList(u8) = .empty;
    defer stale_output.deinit(std.testing.allocator);
    try std.testing.expectError(error.InvalidMirTargetTypeFacts, appendLlvmCheckedMirDeclsTest(std.testing.allocator, parsed.decls(), &stale, &stale_output, "llvm_inferred_semantic_escape_local_type.mc", .{}, false, .riscv64, null));
}
...
test "LLVM inferred local raw result calls require MIR types" {
    ...
    for ([_][]const u8{ "inferred_raw_load", "inferred_raw_ptr" }) |name| {
        ...
        var stale_output: std.ArrayList(u8) = .empty;
        defer stale_output.deinit(std.testing.allocator);
        try std.testing.expectError(error.InvalidMirTargetTypeFacts, appendLlvmCheckedMirDeclsTest(std.testing.allocator, parsed.decls(), &stale, &stale_output, "llvm_inferred_raw_local_types.mc", .{}, false, .riscv64, null));
    }
}
...
test "LLVM rejects experimental dynamic trait dispatch at codegen admission" {
    ...
    var stale_output: std.ArrayList(u8) = .empty;
    defer stale_output.deinit(std.testing.allocator);
    try std.testing.expectError(error.InvalidMirTargetTypeFacts, appendLlvmCheckedMirDeclsTest(std.testing.allocator, parsed.decls(), &stale, &stale_output, "llvm_inferred_local_dyn_dispatch_call_types.mc", .{}, false, .riscv64, null));
    ...
    try std.testing.expectError(error.InvalidMirTargetTypeFacts, appendLlvmCheckedMirDeclsTest(std.testing.allocator, parsed.decls(), &stale_argument, &stale_argument_output, "llvm_inferred_local_dyn_dispatch_call_types.mc", .{}, false, .riscv64, null));
}
...
test "LLVM ordinary direct calls require MIR result and argument types" {
    ...
    var complete_output: std.ArrayList(u8) = .empty;
    defer complete_output.deinit(std.testing.allocator);
    try appendLlvmExecutableMirTest("llvm_mir_direct_call_type_facts.mc", source, &complete_output);
    ...
    var stale_output: std.ArrayList(u8) = .empty;
    defer stale_output.deinit(std.testing.allocator);
    try std.testing.expectError(error.InvalidMirTargetTypeFacts, appendLlvmCheckedMirDeclsTest(std.testing.allocator, parsed.decls(), &stale, &stale_output, "llvm_direct_call_type_facts.mc", .{}, false, .riscv64, null));
...
test "LLVM direct global closure calls use canonical fat-value loads" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmExecutableMirTest("llvm_direct_global_closure.mc", source, &output);
...
test "LLVM emits opaque asm from canonical executable MIR" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmExecutableMirTest("llvm_canonical_opaque_asm.mc", source, &output);
...
test "LLVM emits precise asm from canonical executable MIR" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmExecutableMirTest("llvm_canonical_precise_asm.mc", source, &output);
    try expectContains(output.items, "; canonical executable MIR");
...
test "LLVM backend emits a backend_name alias for the override symbol" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmExecutableMirTest("llvm_mir_backend_name_alias.mc", source, &output);
...
test "LLVM backend emits checked integer add from MIR-gated source" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmExecutableMirTest("llvm_mir_smoke_checked_add.mc", source, &output);
...
test "LLVM aggregate-return pointer facts are MIR-owned and fail closed when absent" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmExecutableMirTest("llvm_mir_aggregate_return_pointer_fact.mc", source, &output);
...
test "LLVM lowers pointer parameter field stores after specialized plan retirement" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmExecutableMirTest("llvm_mir_pointer_param_field_store.mc", source, &output);
...
test "LLVM admits plain unary returns from MIR (bitwise not, wrapping negate)" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmExecutableMirTest("llvm_mir_plain_unary.mc", source, &output);
...
test "LLVM emits global address direct-call args from MIR without body fallback" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmExecutableMirTest("llvm_mir_global_address_call_arg.mc", source, &output);
...
test "LLVM emits global address returns from MIR without body fallback" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmExecutableMirTest("llvm_mir_global_address_return.mc", source, &output);
...
test "LLVM emits local global address returns from MIR without body fallback" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmExecutableMirTest("llvm_mir_local_global_address_return.mc", source, &output);
...
test "LLVM emits conditional global address returns from MIR without body fallback" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmExecutableMirTest("llvm_mir_conditional_global_address_return.mc", source, &output);
...
test "LLVM checked pointer-root field store does not use function body fallback" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmExecutableMirTest("llvm_mir_pointer_root_store.mc", source, &output);
...
test "LLVM checked pointer-to-integer cast does not use function body fallback" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmExecutableMirTest("llvm_mir_pointer_to_integer.mc", source, &output);
...
test "LLVM address representation cast does not use function body fallback" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmExecutableMirTest("llvm_mir_address_representation_cast.mc", source, &output);
...
test "LLVM checked scalar local return does not use function body fallback" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmExecutableMirTest("llvm_mir_scalar_local_checked_return.mc", source, &output);
...
test "LLVM canonical executable MIR preserves typed high-word local and flag-set order" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmExecutableMirTest("llvm_mir_canonical_scalar_expressions.mc", source, &output);
...
test "LLVM scalar control plans preserve checked local CFGs without body fallback" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmExecutableMirTest("llvm_canonical_scalar_control.mc", source, &output);
...
test "LLVM canonical slice bucket lowers without function body fallback" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmExecutableMirTest("llvm_access_plan_slice_bucket.mc", source, &output);
...
test "LLVM canonical access lowering materializes locals and addresses without body fallback" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmExecutableMirTest("llvm_structural_access_plan.mc", source, &output);
...
test "LLVM canonical access lowering handles store-return and range terminals without body fallback" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmExecutableMirTest("llvm_structural_access_terminals.mc", source, &output);
...
test "LLVM canonical executable MIR lowers broad local bodies without AST fallback" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmExecutableMirTest("llvm_executable_body_broad.mc", source, &output);
...
test "LLVM canonical executable MIR models explicit uninit as storage without a value" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmExecutableMirTest("llvm_executable_uninit_local.mc", source, &output);
...
test "LLVM canonical executable MIR owns scalar integer conversions" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmExecutableMirTest("llvm_mir_trap_conversion.mc", source, &output);
...
test "LLVM canonical executable MIR owns serial compare Result" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmExecutableMirTest("llvm_mir_serial_compare.mc", source, &output);
...
test "LLVM canonical executable MIR owns bounded counter Result" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmExecutableMirTest("llvm_mir_counter_elapsed_bounded.mc", source, &output);
...
test "LLVM canonical executable MIR precedes legacy specialized plans" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmExecutableMirTest("llvm_executable_body_preferred.mc", source, &output);
...
test "LLVM canonical executable MIR lowers pure scalar bitcasts without body fallback" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmExecutableMirTest("llvm_executable_scalar_bitcast.mc", source, &output);
...
test "LLVM canonical executable MIR emits raw scalar load and store without AST fallback" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmExecutableMirTest("llvm_executable_raw_scalar.mc", source, &output);
...
test "LLVM canonical executable MIR owns reflection constants without AST fallback" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmExecutableMirTest("llvm_executable_reflection_constants.mc", source, &output);
...
test "LLVM canonical executable MIR guards parameter deref load and address identity" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmExecutableMirTest("llvm_executable_parameter_deref.mc", source, &output);
...
test "LLVM local-address access tag lowers checked update without function body fallback" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmExecutableMirTest("llvm_local_address_tag.mc", source, &output);
...
test "LLVM ordinary bool global accesses use byte-sized atomics" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmExecutableMirTest("llvm_mir_ordinary_bool_global.mc", source, &output);
...
test "LLVM immutable scalar global value reads avoid atomic traffic" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmExecutableMirTest("llvm_mir_immutable_scalar_global.mc", source, &output);
...
test "LLVM scalar global reads lower from MIR without body fallback" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmExecutableMirTest("llvm_mir_scalar_global_reads.mc", source, &output);
...
test "LLVM simple functions and race-safe globals lower from MIR without body fallback" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmExecutableMirTest("llvm_mir_simple_functions_race_safe_globals.mc", source, &output);
...
test "LLVM omits checked comptime blocks from canonical runtime bodies" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmExecutableMirTest("llvm_mir_comptime_block.mc", source, &output);
...
test "LLVM renders canonical string bytes without body fallback" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmExecutableMirTest("llvm_mir_string_bytes.mc", source, &output);
