...
fn appendLlvmTest(source_name: []const u8, source: []const u8, output: *std.ArrayList(u8)) !void {
    ...
    try appendLlvmModuleTest(std.testing.allocator, parsed.module, output);
}

fn appendLlvmModuleTest(allocator: std.mem.Allocator, module: ast.Module, output: *std.ArrayList(u8)) !void {
    try appendLlvmWithSourcePathTest(allocator, module, output, "input.mc", false);
}

fn appendLlvmWithSourcePathTest(allocator: std.mem.Allocator, module: ast.Module, output: *std.ArrayList(u8), source_path: []const u8, optimize: bool) !void {
    try appendLlvmCheckedTest(allocator, module, output, source_path, .{ .optimize = optimize }, false, .riscv64);
}

fn appendLlvmCheckedTest(allocator: std.mem.Allocator, module: ast.Module, output: *std.ArrayList(u8), source_path: []const u8, checks: backend_mod.Checks, stub_asm: bool, target: backend_mod.TargetArch) !void {
    var module_mir = try mir.buildOpt(allocator, module, .{ .optimize = checks.optimize });
    defer module_mir.deinit();
    try appendLlvmCheckedMirTest(allocator, module, &module_mir, output, source_path, checks, stub_asm, target, null);
}

fn appendLlvmCheckedMirTest(allocator: std.mem.Allocator, module: ast.Module, module_mir: *const mir.Module, output: *std.ArrayList(u8), source_path: []const u8, checks: backend_mod.Checks, stub_asm: bool, target: backend_mod.TargetArch, reporter: ?*diagnostics.Reporter) !void {
    try appendLlvmCheckedMirProfileTest(allocator, module, module_mir, output, source_path, checks, stub_asm, target, false, reporter);
}
...
fn appendLlvmTargetTest(source_name: []const u8, source: []const u8, target: @import("backend.zig").TargetArch, output: *std.ArrayList(u8)) !void {
    ...
    defer module_mir.deinit();
    try appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &module_mir, output, source_name, .{}, false, target, null);
}
...
fn appendLlvmLinuxKernelTest(source_name: []const u8, source: []const u8, target: @import("backend.zig").TargetArch, output: *std.ArrayList(u8)) !void {
    ...
    var module_mir = try mir.buildOpt(std.testing.allocator, parsed.module, .{});
    defer module_mir.deinit();
    try appendLlvmCheckedMirProfileTest(std.testing.allocator, parsed.module, &module_mir, output, source_name, .{}, false, target, true, null);
}
...
test "LLVM target-typed char literals require MIR facts" {
...

    {
        ...
        var output: std.ArrayList(u8) = .empty;
        defer output.deinit(std.testing.allocator);
        try appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &module_mir, &output, "llvm_char_literal_facts.mc", .{}, false, .riscv64, null);
    ...
    }
    {
        ...
        var output: std.ArrayList(u8) = .empty;
        defer output.deinit(std.testing.allocator);
        try std.testing.expectError(error.InvalidMirTargetTypeFacts, appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &module_mir, &output, "llvm_char_literal_facts.mc", .{}, false, .riscv64, null));
    }
    {
        var module_mir = try mir.buildOpt(std.testing.allocator, parsed.module, .{});
        defer module_mir.deinit();
        ...
        var output: std.ArrayList(u8) = .empty;
        defer output.deinit(std.testing.allocator);
        try std.testing.expectError(error.UnsupportedLlvmEmission, appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &module_mir, &output, "llvm_char_literal_facts.mc", .{}, false, .riscv64, null));
    }
}
...
test "LLVM rejects prebuilt MIR with missing target type facts" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try std.testing.expectError(error.InvalidMirTargetTypeFacts, appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &module_mir, &output, "llvm_missing_target_type_facts.mc", .{}, false, .riscv64, null));
}
...
test "LLVM Result constructors require MIR call target facts" {
    ...
    for ([_][]const u8{ "make", "forward" }) |name| {
        ...
        var output: std.ArrayList(u8) = .empty;
        defer output.deinit(std.testing.allocator);
        try std.testing.expectError(error.InvalidMirCallTargetFacts, appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &module_mir, &output, "llvm_result_constructor_call_facts.mc", .{}, false, .riscv64, null));
    }
}
...
test "LLVM bind closures require MIR call target facts" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try std.testing.expectError(error.InvalidMirCallTargetFacts, appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &module_mir, &output, "llvm_bind_call_facts.mc", .{}, false, .riscv64, null));
}
...
test "LLVM rejects missing tagged-union target type facts" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try std.testing.expectError(error.InvalidMirTargetTypeFacts, appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &module_mir, &output, "llvm_missing_union_target_type_facts.mc", .{}, false, .riscv64, null));
}
...
test "LLVM rejects missing enum-literal target type facts" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try std.testing.expectError(error.InvalidMirTargetTypeFacts, appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &module_mir, &output, "llvm_missing_enum_target_type_facts.mc", .{}, false, .riscv64, null));
}
...
test "LLVM rejects missing string-literal target type facts" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try std.testing.expectError(error.InvalidMirTargetTypeFacts, appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &module_mir, &output, "llvm_missing_string_target_type_facts.mc", .{}, false, .riscv64, null));
}
...
test "LLVM rejects missing aggregate-literal target type facts" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try std.testing.expectError(error.InvalidMirTargetTypeFacts, appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &module_mir, &output, "llvm_missing_aggregate_target_type_facts.mc", .{}, false, .riscv64, null));
}
...
test "LLVM struct literal construction class is MIR-owned" {
    ...
    try appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &valid_mir, &valid_output, "llvm_aggregate_construction_fact.mc", .{}, false, .riscv64, null);
    ...
    for ([_]?mir.AggregateConstructionKind{ null, .packed_bits }) |stale| {
        ...
        var output: std.ArrayList(u8) = .empty;
        defer output.deinit(std.testing.allocator);
        try std.testing.expectError(error.UnsupportedLlvmEmission, appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &stale_mir, &output, "llvm_aggregate_construction_fact.mc", .{}, false, .riscv64, null));
    }
}
...
test "LLVM rejects missing float-literal target type facts" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try std.testing.expectError(error.InvalidMirTargetTypeFacts, appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &module_mir, &output, "llvm_missing_float_target_type_facts.mc", .{}, false, .riscv64, null));
}
...
test "LLVM rejects missing null and value-optional target type facts" {
    ...
    for ([_][]const u8{ "present", "absent" }) |name| {
        ...
        var output: std.ArrayList(u8) = .empty;
        defer output.deinit(std.testing.allocator);
        try std.testing.expectError(error.InvalidMirTargetTypeFacts, appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &module_mir, &output, "llvm_missing_optional_target_type_facts.mc", .{}, false, .riscv64, null));
    }
}
...
test "LLVM rejects missing dyn-coercion target type facts" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try std.testing.expectError(error.InvalidMirTargetTypeFacts, appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &module_mir, &output, "llvm_missing_dyn_target_type_facts.mc", .{}, false, .riscv64, null));
}
...
test "LLVM conversion builtins require exact MIR call-target facts" {
    ...
    try std.testing.expectError(error.InvalidMirCallTargetFacts, appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &missing_mir, &missing_output, "llvm_conversion_call_target_facts.mc", .{}, false, .riscv64, null));
    ...
    try std.testing.expectError(error.InvalidMirCallTargetFacts, appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &stale_mir, &stale_output, "llvm_conversion_call_target_facts.mc", .{}, false, .riscv64, null));
    ...
    try std.testing.expectError(error.InvalidMirTargetTypeFacts, appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &missing_types_mir, &missing_types_output, "llvm_conversion_call_target_facts.mc", .{}, false, .riscv64, null));
}
...
test "LLVM explicit casts require MIR source and target type facts" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try std.testing.expectError(error.InvalidMirTargetTypeFacts, appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &module_mir, &output, "llvm_explicit_cast_type_facts.mc", .{}, false, .riscv64, null));
    ...
    try std.testing.expectError(error.InvalidMirTargetTypeFacts, appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &missing_result, &missing_result_output, "llvm_explicit_cast_type_facts.mc", .{}, false, .riscv64, null));
    ...
    try std.testing.expectError(error.UnsupportedLlvmEmission, appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &stale_result, &stale_result_output, "llvm_explicit_cast_type_facts.mc", .{}, false, .riscv64, null));
}
...
test "LLVM implicit view const narrowing requires MIR source and target type facts" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try std.testing.expectError(error.InvalidMirTargetTypeFacts, appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &module_mir, &output, "llvm_view_const_narrow_type_facts.mc", .{}, false, .riscv64, null));
}
...
test "LLVM self-typed union and enum paths require MIR result type facts" {
    ...
    for ([_][]const u8{ "make", "variant" }) |name| {
        ...
        var output: std.ArrayList(u8) = .empty;
        defer output.deinit(std.testing.allocator);
        try std.testing.expectError(error.InvalidMirTargetTypeFacts, appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &module_mir, &output, "llvm_self_typed_expression_facts.mc", .{}, false, .riscv64, null));
    }
}
...
fn appendLlvmTestWithoutPointerProvenanceFacts(source_name: []const u8, source: []const u8, function_names: []const []const u8, output: *std.ArrayList(u8)) !void {
    ...
    try appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &module_mir, output, source_name, .{}, false, .riscv64, null);
}
...
test "LLVM rejects prebuilt MIR with missing bounds facts" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try std.testing.expectError(error.UnsupportedLlvmEmission, appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &module_mir, &output, "llvm_missing_bounds_facts.mc", .{}, false, .riscv64, null));
}
...
test "LLVM rejects prebuilt MIR with missing representation facts" {
    ...
    try std.testing.expectError(
        ...
        appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &module_mir, &output, "llvm_missing_representation_facts.mc", .{}, false, .riscv64, null),
    );
}
...
test "LLVM rejects prebuilt MIR with stale representation facts" {
    ...
    try std.testing.expectError(
        error.InvalidMirRepresentationFacts,
        appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &module_mir, &output, "llvm_stale_representation_facts.mc", .{}, false, .riscv64, null),
    );
}
...
test "LLVM rejects prebuilt MIR with extra stale representation facts" {
    ...
    try std.testing.expectError(
        error.InvalidMirRepresentationFacts,
        appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &module_mir, &output, "llvm_extra_stale_representation_facts.mc", .{}, false, .riscv64, null),
    );
}
...
test "LLVM rejects prebuilt MIR with missing Result try payload representation facts" {
    ...
    try std.testing.expectError(
        error.InvalidMirRepresentationFacts,
        appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &module_mir, &output, "llvm_missing_result_try_payload_representation_facts.mc", .{}, false, .riscv64, null),
    );
}
...
test "LLVM rejects prebuilt MIR with stale Result try payload representation facts" {
    ...
    try std.testing.expectError(
        error.InvalidMirRepresentationFacts,
        appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &module_mir, &output, "llvm_stale_result_try_payload_representation_facts.mc", .{}, false, .riscv64, null),
    );
}
...
test "LLVM rejects prebuilt MIR with missing integer facts" {
    ...
    try std.testing.expectError(
        ...
        appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &module_mir, &output, "llvm_missing_integer_facts.mc", .{}, false, .riscv64, null),
    );
}
...
test "LLVM rejects prebuilt MIR with missing call target facts" {
    ...
    try std.testing.expectError(
        ...
        appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &module_mir, &output, "llvm_missing_call_target_facts.mc", .{}, false, .riscv64, null),
    );
}
...
test "LLVM rejects prebuilt MIR with missing reflection call target facts" {
    ...
    try std.testing.expectError(
        error.InvalidMirCallTargetFacts,
        appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &module_mir, &output, "llvm_missing_reflection_call_target_facts.mc", .{}, false, .riscv64, null),
    );
}
...
test "LLVM rejects prebuilt MIR with missing byte-view call target facts" {
    ...
    try std.testing.expectError(
        error.InvalidMirCallTargetFacts,
        appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &module_mir, &output, "llvm_missing_byte_view_call_target_facts.mc", .{}, false, .riscv64, null),
    );
}
...
test "LLVM reflection and complete byte-view types require MIR target facts" {
    ...
    for ([_][]const u8{ "reflected_size", "reflected_alignment", "reflected_field_offset", "reflected_bit_offset", "reflected_repr", "view", "equal" }) |name| {
        ...
        var output: std.ArrayList(u8) = .empty;
        defer output.deinit(std.testing.allocator);
        try std.testing.expectError(error.InvalidMirTargetTypeFacts, appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &module_mir, &output, "llvm_reflection_byte_view_type_facts.mc", .{}, false, .riscv64, null));
    }
}
...
test "LLVM rejects prebuilt MIR with missing semantic escape call target facts" {
    ...
    try std.testing.expectError(
        error.InvalidMirCallTargetFacts,
        appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &reveal_mir, &reveal_output, "llvm_missing_semantic_escape_call_target_facts.mc", .{}, false, .riscv64, null),
    );
    ...
    try std.testing.expectError(
        error.InvalidMirCallTargetFacts,
        appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &noalias_mir, &noalias_output, "llvm_missing_semantic_escape_call_target_facts.mc", .{}, false, .riscv64, null),
    );
}
...
test "LLVM semantic escape types require MIR target facts" {
    ...
    for ([_][]const u8{ "reveal_type_gate", "noalias_type_gate" }) |name| {
        ...
        var output: std.ArrayList(u8) = .empty;
        defer output.deinit(std.testing.allocator);
        try std.testing.expectError(error.InvalidMirTargetTypeFacts, appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &module_mir, &output, "llvm_semantic_escape_target_type_facts.mc", .{}, false, .riscv64, null));
    }
}
...
test "LLVM discard calls require MIR identity and argument type facts" {
    ...
    try appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &complete, &complete_output, "llvm_discard_call_facts.mc", .{}, false, .riscv64, null);
    ...
    try std.testing.expectError(error.InvalidMirCallTargetFacts, appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &missing_identity, &identity_output, "llvm_discard_call_facts.mc", .{}, false, .riscv64, null));
    ...
    try std.testing.expectError(error.InvalidMirTargetTypeFacts, appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &missing_type, &type_output, "llvm_discard_call_facts.mc", .{}, false, .riscv64, null));
}
...
test "LLVM consumes MIR drop glue facts and fails closed when absent or stale" {
    ...
    var valid_output: std.ArrayList(u8) = .empty;
    defer valid_output.deinit(std.testing.allocator);
    try appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &module_mir, &valid_output, "llvm_drop_glue_mir_facts.mc", .{}, false, .riscv64, null);
    ...
    try std.testing.expectError(error.UnsupportedLlvmEmission, appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &module_mir, &missing_auto_drop_event_output, "llvm_drop_glue_mir_facts.mc", .{}, false, .riscv64, null));
    for (module_mir.functions) |*function| {
        if (std.mem.eql(u8, function.name, "auto_drop_from_mir_fact")) {
    ...
    try std.testing.expectError(error.InvalidMirDropGlueFacts, appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &module_mir, &missing_ownership_output, "llvm_drop_glue_mir_facts.mc", .{}, false, .riscv64, null));
    ...
    try std.testing.expectError(error.InvalidMirDropGlueFacts, appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &module_mir, &stale_ownership_output, "llvm_drop_glue_mir_facts.mc", .{}, false, .riscv64, null));
    ...
    try std.testing.expectError(error.InvalidMirDropGlueFacts, appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &module_mir, &stale_ownership_symbol_output, "llvm_drop_glue_mir_facts.mc", .{}, false, .riscv64, null));
    ...
    var missing_output: std.ArrayList(u8) = .empty;
    defer missing_output.deinit(std.testing.allocator);
    try std.testing.expectError(error.InvalidMirTypeOwnershipFacts, appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &module_mir, &missing_output, "llvm_drop_glue_mir_facts.mc", .{}, false, .riscv64, null));
    ...
    var stale_output: std.ArrayList(u8) = .empty;
    defer stale_output.deinit(std.testing.allocator);
    try std.testing.expectError(error.InvalidMirDropGlueFacts, appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &module_mir, &stale_output, "llvm_drop_glue_mir_facts.mc", .{}, false, .riscv64, null));
}
...
test "LLVM rejects auto-drop transfer authorization with stale MIR resource type" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try std.testing.expectError(error.UnsupportedLlvmEmission, appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &module_mir, &output, "llvm_drop_attr_transfer_stale_resource.mc", .{}, false, .riscv64, null));
}
...
test "LLVM move auto-drop cancellation requires MIR move-out event" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try std.testing.expectError(error.UnsupportedLlvmEmission, appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &module_mir, &output, "llvm_drop_attr_transfer_requires_move_out.mc", .{}, false, .riscv64, null));
}
...
test "LLVM move auto-drop cancellation requires source-matched MIR move-out event" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try std.testing.expectError(error.UnsupportedLlvmEmission, appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &module_mir, &output, "llvm_drop_attr_transfer_move_out_source.mc", .{}, false, .riscv64, null));
}
...
test "LLVM deferred drop release requires source-matched MIR explicit-drop event" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try std.testing.expectError(error.UnsupportedLlvmEmission, appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &module_mir, &output, "llvm_drop_attr_defer_source_requires_event.mc", .{}, false, .riscv64, null));
}
...
test "LLVM ordinary defer requires source-matched MIR cleanup marker" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try std.testing.expectError(error.UnsupportedLlvmEmission, appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &module_mir, &output, "llvm_ordinary_defer_requires_marker.mc", .{}, false, .riscv64, null));
}
...
test "LLVM ordinary defer rejects unsupported expression fallback" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try std.testing.expectError(error.UnsupportedLlvmEmission, appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &module_mir, &output, "llvm_ordinary_defer_expression_fallback.mc", .{}, false, .riscv64, null));
}
...
test "LLVM ordinary direct defer requires MIR call marker" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try std.testing.expectError(error.UnsupportedLlvmEmission, appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &module_mir, &output, "llvm_ordinary_defer_requires_call_marker.mc", .{}, false, .riscv64, null));
}
...
test "LLVM ordinary direct defer with arguments requires MIR call marker" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try std.testing.expectError(error.UnsupportedLlvmEmission, appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &module_mir, &output, "llvm_ordinary_defer_arg_requires_call_marker.mc", .{}, false, .riscv64, null));
}
...
test "LLVM ordinary direct defer with arguments requires MIR argument facts" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try std.testing.expectError(error.UnsupportedLlvmEmission, appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &module_mir, &output, "llvm_ordinary_defer_arg_requires_fact.mc", .{}, false, .riscv64, null));
}
...
test "LLVM ordinary direct defer with discarded result requires MIR result fact" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try std.testing.expectError(error.UnsupportedLlvmEmission, appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &module_mir, &output, "llvm_ordinary_defer_result_requires_fact.mc", .{}, false, .riscv64, null));
}
...
test "LLVM ordinary direct defer may discard call result" {
    const source =
        ...
        \\}
    ;
    var parsed = try test_support.parseModule("llvm_ordinary_defer_discard_result.mc", source);
    defer parsed.deinit();

    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmCheckedTest(std.testing.allocator, parsed.module, &output, "llvm_ordinary_defer_discard_result.mc", .{}, false, .riscv64);
...
test "LLVM ordinary call-target defer requires MIR call-target fact" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try std.testing.expectError(error.UnsupportedLlvmEmission, appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &module_mir, &output, "llvm_ordinary_defer_call_target_requires_fact.mc", .{}, false, .riscv64, null));
}
...
test "LLVM ordinary call-target defer emits typed cleanup" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmCheckedTest(std.testing.allocator, parsed.module, &output, "llvm_ordinary_defer_call_target_cleanup.mc", .{}, false, .riscv64);
...
test "LLVM ordinary raw-store defer requires MIR target facts" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try std.testing.expectError(error.UnsupportedLlvmEmission, appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &module_mir, &output, "llvm_ordinary_defer_raw_store_requires_fact.mc", .{}, false, .riscv64, null));
}

test "LLVM ordinary raw-store defer emits typed cleanup" {
    const source =
        \\fn ordinary_defer_raw_store_cleanup(addr: PAddr, value: u32) -> void {
        \\    unsafe {
        \\        defer raw.store<u32>(addr, value);
        \\    }
        \\    return;
        \\}
    ;
    var parsed = try test_support.parseModule("llvm_ordinary_defer_raw_store_cleanup.mc", source);
    defer parsed.deinit();

    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmCheckedTest(std.testing.allocator, parsed.module, &output, "llvm_ordinary_defer_raw_store_cleanup.mc", .{}, false, .riscv64);
...
test "LLVM ordinary MMIO write defer requires MIR call-target facts" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try std.testing.expectError(error.UnsupportedLlvmEmission, appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &module_mir, &output, "llvm_ordinary_defer_mmio_write_requires_fact.mc", .{}, false, .riscv64, null));
}
...
test "LLVM ordinary MMIO write defer emits typed cleanup" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmCheckedTest(std.testing.allocator, parsed.module, &output, "llvm_ordinary_defer_mmio_write_cleanup.mc", .{}, false, .riscv64);
...
test "LLVM ordinary MMIO read defer requires MIR call-target facts" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try std.testing.expectError(error.UnsupportedLlvmEmission, appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &module_mir, &output, "llvm_ordinary_defer_mmio_read_requires_fact.mc", .{}, false, .riscv64, null));
}
...
test "LLVM ordinary MMIO read defer emits typed cleanup" {
    const source =
        ...
        \\}
    ;
    var parsed = try test_support.parseModule("llvm_ordinary_defer_mmio_read_cleanup.mc", source);
    defer parsed.deinit();

    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmCheckedTest(std.testing.allocator, parsed.module, &output, "llvm_ordinary_defer_mmio_read_cleanup.mc", .{}, false, .riscv64);
...
test "LLVM ordinary DMA cache defer requires MIR call-target facts" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try std.testing.expectError(error.UnsupportedLlvmEmission, appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &module_mir, &output, "llvm_ordinary_defer_dma_cache_requires_fact.mc", .{}, false, .riscv64, null));
}
...
test "LLVM ordinary DMA cache defer emits typed cleanup" {
    const source =
        \\extern struct Packet { len: u32 }
        \\type Buffer = DmaBuf<Packet, .noncoherent>;
        \\fn ordinary_defer_dma_cache_cleanup(buf: Buffer) -> void {
        \\    defer cache.clean(buf);
        \\    defer cache.invalidate(buf);
        \\    return;
        \\}
    ;
    var parsed = try test_support.parseModule("llvm_ordinary_defer_dma_cache_cleanup.mc", source);
    defer parsed.deinit();

    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmCheckedTest(std.testing.allocator, parsed.module, &output, "llvm_ordinary_defer_dma_cache_cleanup.mc", .{}, false, .riscv64);
...
test "LLVM ordinary MaybeUninit write defer requires MIR call-target facts" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try std.testing.expectError(error.UnsupportedLlvmEmission, appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &module_mir, &output, "llvm_ordinary_defer_maybe_uninit_write_requires_fact.mc", .{}, false, .riscv64, null));
}
...
test "LLVM ordinary MaybeUninit write defer emits typed cleanup" {
    const source =
        ...
        \\}
    ;
    var parsed = try test_support.parseModule("llvm_ordinary_defer_maybe_uninit_write_cleanup.mc", source);
    defer parsed.deinit();

    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmCheckedTest(std.testing.allocator, parsed.module, &output, "llvm_ordinary_defer_maybe_uninit_write_cleanup.mc", .{}, false, .riscv64);
...
test "LLVM ordinary atomic store defer requires MIR call-target facts" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try std.testing.expectError(error.UnsupportedLlvmEmission, appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &module_mir, &output, "llvm_ordinary_defer_atomic_store_requires_fact.mc", .{}, false, .riscv64, null));
}
...
test "LLVM ordinary atomic store defer emits typed cleanup" {
    const source =
        ...
        \\}
    ;
    var parsed = try test_support.parseModule("llvm_ordinary_defer_atomic_store_cleanup.mc", source);
    defer parsed.deinit();

    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmCheckedTest(std.testing.allocator, parsed.module, &output, "llvm_ordinary_defer_atomic_store_cleanup.mc", .{}, false, .riscv64);
...
test "LLVM ordinary va.end defer requires MIR call-target facts" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try std.testing.expectError(error.UnsupportedLlvmEmission, appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &module_mir, &output, "llvm_ordinary_defer_va_end_requires_fact.mc", .{}, false, .riscv64, null));
}

test "LLVM ordinary va.end defer emits typed cleanup" {
    const source =
        \\export fn ordinary_defer_va_end_cleanup(count: i32, ...) -> i32 {
        \\    var ap: va_list = va.start();
        \\    defer va.end(&ap);
        \\    return count;
        \\}
    ;
    var parsed = try test_support.parseModule("llvm_ordinary_defer_va_end_cleanup.mc", source);
    defer parsed.deinit();

    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmCheckedTest(std.testing.allocator, parsed.module, &output, "llvm_ordinary_defer_va_end_cleanup.mc", .{}, false, .riscv64);
...
test "LLVM explicit drop release cancellation requires MIR explicit-drop event" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try std.testing.expectError(error.UnsupportedLlvmEmission, appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &module_mir, &output, "llvm_drop_attr_release_requires_mir_event.mc", .{}, false, .riscv64, null));
}
...
test "LLVM explicit drop release cancellation requires source-matched MIR explicit-drop event" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try std.testing.expectError(error.UnsupportedLlvmEmission, appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &module_mir, &output, "llvm_drop_attr_release_source_requires_event.mc", .{}, false, .riscv64, null));
}
...
test "LLVM loop bodies restore auto-drop cleanup state" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmTest("llvm_loop_restore_auto_drop_stack.mc", source, &output);
    const body = try llvmFunctionBody(output.items, "@transfer_in_loop");
    try std.testing.expectEqual(@as(usize, 1), std.mem.count(u8, body, "call void @close_guard(ptr %g.addr"));
}
...
test "LLVM wrapping arithmetic requires MIR identity and operand/result type facts" {
    ...
    var complete_output: std.ArrayList(u8) = .empty;
    defer complete_output.deinit(std.testing.allocator);
    try appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &complete, &complete_output, "llvm_wrapping_call_facts.mc", .{}, false, .riscv64, null);
    ...
    var identity_output: std.ArrayList(u8) = .empty;
    defer identity_output.deinit(std.testing.allocator);
    try std.testing.expectError(error.InvalidMirCallTargetFacts, appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &missing_identity, &identity_output, "llvm_wrapping_call_facts.mc", .{}, false, .riscv64, null));
    ...
    inline for ([_]mir.TargetTypeKind{ .wrapping_left, .wrapping_right, .wrapping_result }) |kind| {
        ...
        var type_output: std.ArrayList(u8) = .empty;
        defer type_output.deinit(std.testing.allocator);
        try std.testing.expectError(error.InvalidMirTargetTypeFacts, appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &missing_type, &type_output, "llvm_wrapping_call_facts.mc", .{}, false, .riscv64, null));
    }
}
...
test "LLVM unchecked arithmetic requires MIR identity and operand/result type facts" {
    ...
    var complete_output: std.ArrayList(u8) = .empty;
    defer complete_output.deinit(std.testing.allocator);
    try appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &complete, &complete_output, "llvm_unchecked_call_facts.mc", .{}, false, .riscv64, null);
    try expectContains(complete_output.items, " = add i32 %a, 1");
    ...
    var identity_output: std.ArrayList(u8) = .empty;
    defer identity_output.deinit(std.testing.allocator);
    try std.testing.expectError(error.InvalidMirCallTargetFacts, appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &missing_identity, &identity_output, "llvm_unchecked_call_facts.mc", .{}, false, .riscv64, null));
    ...
    inline for ([_]mir.TargetTypeKind{ .unchecked_left, .unchecked_right, .unchecked_result }) |kind| {
        ...
        var type_output: std.ArrayList(u8) = .empty;
        defer type_output.deinit(std.testing.allocator);
        try std.testing.expectError(error.InvalidMirTargetTypeFacts, appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &missing_type, &type_output, "llvm_unchecked_call_facts.mc", .{}, false, .riscv64, null));
    }
}
...
test "LLVM rejects prebuilt MIR with missing atomic call target facts" {
    ...
    try std.testing.expectError(
        error.InvalidMirCallTargetFacts,
        appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &module_mir, &output, "llvm_missing_atomic_call_target_facts.mc", .{}, false, .riscv64, null),
    );
}
...
test "LLVM atomic and MaybeUninit payloads require MIR target facts" {
    ...
    for ([_][]const u8{ "atomic_payload_fact_gate", "maybe_uninit_payload_fact_gate" }) |name| {
        ...
        var output: std.ArrayList(u8) = .empty;
        defer output.deinit(std.testing.allocator);
        try std.testing.expectError(error.InvalidMirTargetTypeFacts, appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &module_mir, &output, "llvm_atomic_maybe_uninit_payload_facts.mc", .{}, false, .riscv64, null));
    }
}
...
test "LLVM atomic init requires MIR identity and complete types" {
    ...
    var complete_output: std.ArrayList(u8) = .empty;
    defer complete_output.deinit(std.testing.allocator);
    try appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &complete, &complete_output, "llvm_atomic_init_facts.mc", .{}, false, .riscv64, null);
    ...
    for ([_][]const u8{ "boot_counter", "local_init" }) |name| {
        ...
        try std.testing.expectError(error.InvalidMirCallTargetFacts, appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &missing_identity, &missing_identity_output, "llvm_atomic_init_facts.mc", .{}, false, .riscv64, null));
        ...
        inline for ([_]mir.TargetTypeKind{ .atomic_init_payload, .atomic_init_result }) |kind| {
            ...
            try std.testing.expectError(error.InvalidMirTargetTypeFacts, appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &missing_type, &missing_type_output, "llvm_atomic_init_facts.mc", .{}, false, .riscv64, null));
        }
        ...
        try std.testing.expectError(error.UnsupportedLlvmEmission, appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &stale_payload, &stale_payload_output, "llvm_atomic_init_facts.mc", .{}, false, .riscv64, null));
        ...
        var stale_result_output: std.ArrayList(u8) = .empty;
        defer stale_result_output.deinit(std.testing.allocator);
        try std.testing.expectError(error.UnsupportedLlvmEmission, appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &stale_result, &stale_result_output, "llvm_atomic_init_facts.mc", .{}, false, .riscv64, null));
    }
}
...
test "LLVM rejects prebuilt MIR with missing bitcast call target facts" {
    ...
    try std.testing.expectError(
        error.InvalidMirCallTargetFacts,
        appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &module_mir, &output, "llvm_missing_bitcast_call_target_facts.mc", .{}, false, .riscv64, null),
    );
}
...
test "LLVM rejects prebuilt MIR with missing bitcast target type facts" {
    ...
    try std.testing.expectError(
        ...
        appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &module_mir, &output, "llvm_missing_bitcast_target_type_facts.mc", .{}, false, .riscv64, null),
    );
}
...
test "LLVM rejects prebuilt MIR with missing const_get call target facts" {
    ...
    try std.testing.expectError(
        error.InvalidMirCallTargetFacts,
        appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &module_mir, &output, "llvm_missing_const_get_call_target_facts.mc", .{}, false, .riscv64, null),
    );
}
...
test "LLVM const_get consumes MIR base result and index facts" {
    ...
    defer parsed.deinit();
    {
        ...
        var output: std.ArrayList(u8) = .empty;
        defer output.deinit(std.testing.allocator);
        try appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &module_mir, &output, "llvm_const_get_facts.mc", .{}, false, .riscv64, null);
    ...
    }
    {
        ...
        var output: std.ArrayList(u8) = .empty;
        defer output.deinit(std.testing.allocator);
        try std.testing.expectError(error.InvalidMirTargetTypeFacts, appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &module_mir, &output, "llvm_const_get_facts.mc", .{}, false, .riscv64, null));
    }
    {
        var module_mir = try mir.buildOpt(std.testing.allocator, parsed.module, .{});
        defer module_mir.deinit();
        ...
        var output: std.ArrayList(u8) = .empty;
        defer output.deinit(std.testing.allocator);
        try std.testing.expectError(error.InvalidMirConstGetFacts, appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &module_mir, &output, "llvm_const_get_facts.mc", .{}, false, .riscv64, null));
    }
    {
        var module_mir = try mir.buildOpt(std.testing.allocator, parsed.module, .{});
        defer module_mir.deinit();
...
test "LLVM DMA calls consume MIR identities and complete types" {
    ...
    defer parsed.deinit();
    {
        ...
        var output: std.ArrayList(u8) = .empty;
        defer output.deinit(std.testing.allocator);
        try appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &module_mir, &output, "llvm_dma_facts.mc", .{}, false, .riscv64, null);
        try std.testing.expect(std.mem.indexOf(u8, output.items, "fence release") != null);
        try std.testing.expect(std.mem.indexOf(u8, output.items, "fence acquire") != null);
    ...
    }
    {
        ...
        var output: std.ArrayList(u8) = .empty;
        defer output.deinit(std.testing.allocator);
        try std.testing.expectError(error.InvalidMirCallTargetFacts, appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &module_mir, &output, "llvm_dma_facts.mc", .{}, false, .riscv64, null));
    }
    {
        var module_mir = try mir.buildOpt(std.testing.allocator, parsed.module, .{});
        defer module_mir.deinit();
    ...
    }
    {
        ...
        var output: std.ArrayList(u8) = .empty;
        defer output.deinit(std.testing.allocator);
        try std.testing.expectError(error.InvalidMirTargetTypeFacts, appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &module_mir, &output, "llvm_dma_facts.mc", .{}, false, .riscv64, null));
    }
}
...
test "LLVM raw-many offset consumes MIR identity and complete types" {
    ...
    defer parsed.deinit();
    {
        ...
        var output: std.ArrayList(u8) = .empty;
        defer output.deinit(std.testing.allocator);
        try appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &module_mir, &output, "llvm_raw_many_offset_facts.mc", .{}, false, .riscv64, null);
    ...
    }
    {
        ...
        var output: std.ArrayList(u8) = .empty;
        defer output.deinit(std.testing.allocator);
        try std.testing.expectError(error.InvalidMirCallTargetFacts, appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &module_mir, &output, "llvm_raw_many_offset_facts.mc", .{}, false, .riscv64, null));
    }
    {
        var module_mir = try mir.buildOpt(std.testing.allocator, parsed.module, .{});
        defer module_mir.deinit();
    ...
    }
    {
        ...
        var output: std.ArrayList(u8) = .empty;
        defer output.deinit(std.testing.allocator);
        try std.testing.expectError(error.InvalidMirTargetTypeFacts, appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &module_mir, &output, "llvm_raw_many_offset_facts.mc", .{}, false, .riscv64, null));
    }
    {
        var module_mir = try mir.buildOpt(std.testing.allocator, parsed.module, .{});
        defer module_mir.deinit();
    ...
    }
    {
        ...
        var output: std.ArrayList(u8) = .empty;
        defer output.deinit(std.testing.allocator);
        try std.testing.expectError(error.UnsupportedLlvmEmission, appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &module_mir, &output, "llvm_raw_many_offset_facts.mc", .{}, false, .riscv64, null));
    }
    {
        var module_mir = try mir.buildOpt(std.testing.allocator, parsed.module, .{});
        defer module_mir.deinit();
...
test "LLVM direct storage-read inferred locals require MIR facts" {
    ...
    var complete_output: std.ArrayList(u8) = .empty;
    defer complete_output.deinit(std.testing.allocator);
    try appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &complete, &complete_output, "llvm_inferred_storage_reads.mc", .{}, false, .riscv64, null);
    ...
    var missing_output: std.ArrayList(u8) = .empty;
    defer missing_output.deinit(std.testing.allocator);
    try std.testing.expectError(error.InvalidMirTargetTypeFacts, appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &missing, &missing_output, "llvm_inferred_storage_reads.mc", .{}, false, .riscv64, null));
    ...
    var stale_output: std.ArrayList(u8) = .empty;
    defer stale_output.deinit(std.testing.allocator);
    try std.testing.expectError(error.UnsupportedLlvmEmission, appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &stale, &stale_output, "llvm_inferred_storage_reads.mc", .{}, false, .riscv64, null));
}
...
test "LLVM inferred local try payloads require MIR types" {
    ...
    var complete_output: std.ArrayList(u8) = .empty;
    defer complete_output.deinit(std.testing.allocator);
    try appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &complete, &complete_output, "llvm_inferred_local_try_payloads.mc", .{}, false, .riscv64, null);
    ...
    var missing_output: std.ArrayList(u8) = .empty;
    defer missing_output.deinit(std.testing.allocator);
    try std.testing.expectError(error.InvalidMirTargetTypeFacts, appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &missing, &missing_output, "llvm_inferred_local_try_payloads.mc", .{}, false, .riscv64, null));
    ...
    var stale_output: std.ArrayList(u8) = .empty;
    defer stale_output.deinit(std.testing.allocator);
    try std.testing.expectError(error.UnsupportedLlvmEmission, appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &stale, &stale_output, "llvm_inferred_local_try_payloads.mc", .{}, false, .riscv64, null));
}
...
test "LLVM inferred local direct addresses require MIR types" {
    ...
    var complete_output: std.ArrayList(u8) = .empty;
    defer complete_output.deinit(std.testing.allocator);
    try appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &complete, &complete_output, "llvm_inferred_local_address.mc", .{}, false, .riscv64, null);
    ...
    var missing_output: std.ArrayList(u8) = .empty;
    defer missing_output.deinit(std.testing.allocator);
    try std.testing.expectError(error.InvalidMirTargetTypeFacts, appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &missing, &missing_output, "llvm_inferred_local_address.mc", .{}, false, .riscv64, null));
    ...
    try std.testing.expectError(error.InvalidMirTargetTypeFacts, appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &missing_const_global, &missing_const_global_output, "llvm_inferred_local_address.mc", .{}, false, .riscv64, null));
    ...
    try std.testing.expectError(error.UnsupportedLlvmEmission, appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &stale_const_global, &stale_const_global_output, "llvm_inferred_local_address.mc", .{}, false, .riscv64, null));

    var stale_address_local_operand = try mir.build(std.testing.allocator, parsed.module);
    ...
    try std.testing.expectError(error.UnsupportedLlvmEmission, appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &stale_address_local_operand, &stale_address_local_operand_output, "llvm_inferred_local_address.mc", .{}, false, .riscv64, null));
    ...
    var stale_output: std.ArrayList(u8) = .empty;
    defer stale_output.deinit(std.testing.allocator);
    try std.testing.expectError(error.UnsupportedLlvmEmission, appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &stale, &stale_output, "llvm_inferred_local_address.mc", .{}, false, .riscv64, null));
    ...
    try std.testing.expectError(error.UnsupportedLlvmEmission, appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &stale_pointee_source, &stale_pointee_source_output, "llvm_inferred_local_address.mc", .{}, false, .riscv64, null));
}
...
test "LLVM compound expressions require complete MIR result facts" {
    ...
    defer parsed.deinit();
    {
        ...
        var output: std.ArrayList(u8) = .empty;
        defer output.deinit(std.testing.allocator);
        try appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &module_mir, &output, "llvm_expression_result_facts.mc", .{}, false, .riscv64, null);
    }
    {
        var module_mir = try mir.buildOpt(std.testing.allocator, parsed.module, .{});
        defer module_mir.deinit();
        ...
        var output: std.ArrayList(u8) = .empty;
        defer output.deinit(std.testing.allocator);
        try std.testing.expectError(error.InvalidMirTargetTypeFacts, appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &module_mir, &output, "llvm_expression_result_facts.mc", .{}, false, .riscv64, null));
    }
    {
        var module_mir = try mir.buildOpt(std.testing.allocator, parsed.module, .{});
        defer module_mir.deinit();
        ...
        var output: std.ArrayList(u8) = .empty;
        defer output.deinit(std.testing.allocator);
        try std.testing.expectError(error.UnsupportedLlvmEmission, appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &module_mir, &output, "llvm_expression_result_facts.mc", .{}, false, .riscv64, null));
    }
    {
        var module_mir = try mir.buildOpt(std.testing.allocator, parsed.module, .{});
        defer module_mir.deinit();
...
test "LLVM grouped expressions consume their own MIR result facts" {
    ...
    var complete_output: std.ArrayList(u8) = .empty;
    defer complete_output.deinit(std.testing.allocator);
    try appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &complete, &complete_output, "llvm_grouped_expression_result.mc", .{}, false, .riscv64, null);
    ...
    var missing_output: std.ArrayList(u8) = .empty;
    defer missing_output.deinit(std.testing.allocator);
    try std.testing.expectError(error.InvalidMirTargetTypeFacts, appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &missing, &missing_output, "llvm_grouped_expression_result.mc", .{}, false, .riscv64, null));
    ...
    var stale_output: std.ArrayList(u8) = .empty;
    defer stale_output.deinit(std.testing.allocator);
    try std.testing.expectError(error.UnsupportedLlvmEmission, appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &stale, &stale_output, "llvm_grouped_expression_result.mc", .{}, false, .riscv64, null));
}
...
test "LLVM grouped direct calls consume the outer MIR result fact" {
    ...
    var complete_output: std.ArrayList(u8) = .empty;
    defer complete_output.deinit(std.testing.allocator);
    try appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &complete, &complete_output, "llvm_grouped_call_result.mc", .{}, false, .riscv64, null);
    ...
    var missing_output: std.ArrayList(u8) = .empty;
    defer missing_output.deinit(std.testing.allocator);
    try std.testing.expectError(error.InvalidMirTargetTypeFacts, appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &missing, &missing_output, "llvm_grouped_call_result.mc", .{}, false, .riscv64, null));
    ...
    var stale_output: std.ArrayList(u8) = .empty;
    defer stale_output.deinit(std.testing.allocator);
    try std.testing.expectError(error.UnsupportedLlvmEmission, appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &stale, &stale_output, "llvm_grouped_call_result.mc", .{}, false, .riscv64, null));
}
...
test "LLVM block expressions consume MIR result facts" {
    ...
    var complete_output: std.ArrayList(u8) = .empty;
    defer complete_output.deinit(std.testing.allocator);
    try appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &complete, &complete_output, "llvm_block_expression_policy.mc", .{}, false, .riscv64, null);
    ...
    var missing_output: std.ArrayList(u8) = .empty;
    defer missing_output.deinit(std.testing.allocator);
    try std.testing.expectError(error.InvalidMirTargetTypeFacts, appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &missing, &missing_output, "llvm_block_expression_policy.mc", .{}, false, .riscv64, null));
    ...
    var stale_output: std.ArrayList(u8) = .empty;
    defer stale_output.deinit(std.testing.allocator);
    try std.testing.expectError(error.UnsupportedLlvmEmission, appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &stale, &stale_output, "llvm_block_expression_policy.mc", .{}, false, .riscv64, null));
}
...
test "LLVM nested array member and index results require MIR expression facts" {
    ...
    defer parsed.deinit();
    {
        ...
        var output: std.ArrayList(u8) = .empty;
        defer output.deinit(std.testing.allocator);
        try appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &module_mir, &output, "llvm_array_member_expression_result_facts.mc", .{}, false, .riscv64, null);
    ...
    }
    {
        ...
        var output: std.ArrayList(u8) = .empty;
        defer output.deinit(std.testing.allocator);
        try std.testing.expectError(error.InvalidMirTargetTypeFacts, appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &module_mir, &output, "llvm_array_member_expression_result_facts.mc", .{}, false, .riscv64, null));
    }
    {
        var module_mir = try mir.buildOpt(std.testing.allocator, parsed.module, .{});
        defer module_mir.deinit();
        ...
        var output: std.ArrayList(u8) = .empty;
        defer output.deinit(std.testing.allocator);
        try std.testing.expectError(error.UnsupportedLlvmEmission, appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &module_mir, &output, "llvm_array_member_expression_result_facts.mc", .{}, false, .riscv64, null));
    }
}
...
test "LLVM nested pointer members require MIR expression facts" {
    ...
    defer parsed.deinit();
    {
        ...
        var output: std.ArrayList(u8) = .empty;
        defer output.deinit(std.testing.allocator);
        try appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &module_mir, &output, "llvm_pointer_member_expression_result_facts.mc", .{}, false, .riscv64, null);
        try expectContains(output.items, "getelementptr");
    }
    {
        var module_mir = try mir.buildOpt(std.testing.allocator, parsed.module, .{});
        ...
        var output: std.ArrayList(u8) = .empty;
        defer output.deinit(std.testing.allocator);
        try std.testing.expectError(error.InvalidMirTargetTypeFacts, appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &module_mir, &output, "llvm_pointer_member_expression_result_facts.mc", .{}, false, .riscv64, null));
    }
    {
        var module_mir = try mir.buildOpt(std.testing.allocator, parsed.module, .{});
        defer module_mir.deinit();
        ...
        var output: std.ArrayList(u8) = .empty;
        defer output.deinit(std.testing.allocator);
        try std.testing.expectError(error.UnsupportedLlvmEmission, appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &module_mir, &output, "llvm_pointer_member_expression_result_facts.mc", .{}, false, .riscv64, null));
    }
}
...
test "LLVM nested struct members require MIR expression facts" {
    ...
    defer parsed.deinit();
    {
        ...
        var output: std.ArrayList(u8) = .empty;
        defer output.deinit(std.testing.allocator);
        try appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &module_mir, &output, "llvm_struct_member_expression_result_facts.mc", .{}, false, .riscv64, null);
        try expectContains(output.items, "getelementptr");
    }
    {
        var module_mir = try mir.buildOpt(std.testing.allocator, parsed.module, .{});
        ...
        var output: std.ArrayList(u8) = .empty;
        defer output.deinit(std.testing.allocator);
        try std.testing.expectError(error.InvalidMirTargetTypeFacts, appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &module_mir, &output, "llvm_struct_member_expression_result_facts.mc", .{}, false, .riscv64, null));
    }
    {
        var module_mir = try mir.buildOpt(std.testing.allocator, parsed.module, .{});
        defer module_mir.deinit();
        ...
        var output: std.ArrayList(u8) = .empty;
        defer output.deinit(std.testing.allocator);
        try std.testing.expectError(error.UnsupportedLlvmEmission, appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &module_mir, &output, "llvm_struct_member_expression_result_facts.mc", .{}, false, .riscv64, null));
    }
}
...
test "LLVM MMIO calls consume MIR identities and complete types" {
    ...
    defer parsed.deinit();
    {
        ...
        var output: std.ArrayList(u8) = .empty;
        defer output.deinit(std.testing.allocator);
        try appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &module_mir, &output, "llvm_mmio_facts.mc", .{}, false, .riscv64, null);
        try std.testing.expect(std.mem.indexOf(u8, output.items, "store volatile i32") != null);
        try std.testing.expect(std.mem.indexOf(u8, output.items, "load volatile i32") != null);
    ...
    }
    {
        ...
        var output: std.ArrayList(u8) = .empty;
        defer output.deinit(std.testing.allocator);
        try std.testing.expectError(error.InvalidMirCallTargetFacts, appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &module_mir, &output, "llvm_mmio_facts.mc", .{}, false, .riscv64, null));
    }
    {
        var module_mir = try mir.buildOpt(std.testing.allocator, parsed.module, .{});
        defer module_mir.deinit();
    ...
    }
    {
        ...
        var output: std.ArrayList(u8) = .empty;
        defer output.deinit(std.testing.allocator);
        try std.testing.expectError(error.InvalidMirTargetTypeFacts, appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &module_mir, &output, "llvm_mmio_facts.mc", .{}, false, .riscv64, null));
    }
    {
        var module_mir = try mir.buildOpt(std.testing.allocator, parsed.module, .{});
        defer module_mir.deinit();
    ...
    }
    {
        ...
        var output: std.ArrayList(u8) = .empty;
        defer output.deinit(std.testing.allocator);
        try std.testing.expectError(error.UnsupportedLlvmEmission, appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &module_mir, &output, "llvm_mmio_facts.mc", .{}, false, .riscv64, null));
    }
}
...
test "LLVM MMIO map consumes MIR identity and complete types" {
    ...
    defer parsed.deinit();
    {
        ...
        var output: std.ArrayList(u8) = .empty;
        defer output.deinit(std.testing.allocator);
        try appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &module_mir, &output, "llvm_mmio_map_facts.mc", .{}, false, .riscv64, null);
    ...
    }
    {
        ...
        var output: std.ArrayList(u8) = .empty;
        defer output.deinit(std.testing.allocator);
        try std.testing.expectError(error.InvalidMirCallTargetFacts, appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &module_mir, &output, "llvm_mmio_map_facts.mc", .{}, false, .riscv64, null));
    }
    {
        var module_mir = try mir.buildOpt(std.testing.allocator, parsed.module, .{});
        defer module_mir.deinit();
    ...
    }
    {
        ...
        var output: std.ArrayList(u8) = .empty;
        defer output.deinit(std.testing.allocator);
        try std.testing.expectError(error.InvalidMirTargetTypeFacts, appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &module_mir, &output, "llvm_mmio_map_facts.mc", .{}, false, .riscv64, null));
    }
}
...
test "LLVM reductions require MIR source and element type facts" {
    ...
    for ([_]mir.TargetTypeKind{ .reduce_source, .reduce_element }) |kind| {
        ...
        try std.testing.expectError(
            error.InvalidMirTargetTypeFacts,
            appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &module_mir, &output, "llvm_missing_reduce_element_facts.mc", .{}, false, .riscv64, null),
        );
    }
}
...
test "LLVM enum raw requires MIR call and target type facts" {
    ...
    defer parsed.deinit();
    {
        ...
        var output: std.ArrayList(u8) = .empty;
        defer output.deinit(std.testing.allocator);
        try appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &module_mir, &output, "llvm_enum_raw_facts.mc", .{}, false, .riscv64, null);
    }
    {
        var module_mir = try mir.buildOpt(std.testing.allocator, parsed.module, .{});
        defer module_mir.deinit();
        ...
        try std.testing.expectError(
            error.InvalidMirCallTargetFacts,
            appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &module_mir, &output, "llvm_enum_raw_facts.mc", .{}, false, .riscv64, null),
        );
    }
    {
        var module_mir = try mir.buildOpt(std.testing.allocator, parsed.module, .{});
...
test "LLVM arithmetic domain calls require MIR identities and complete types" {
    ...
    defer parsed.deinit();
    {
        ...
        var output: std.ArrayList(u8) = .empty;
        defer output.deinit(std.testing.allocator);
        try appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &module_mir, &output, "llvm_domain_call_facts.mc", .{}, false, .riscv64, null);
    }
    {
        var module_mir = try mir.buildOpt(std.testing.allocator, parsed.module, .{});
        defer module_mir.deinit();
        ...
        try std.testing.expectError(
            error.InvalidMirCallTargetFacts,
            appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &module_mir, &output, "llvm_domain_call_facts.mc", .{}, false, .riscv64, null),
        );
    }
    {
        var module_mir = try mir.buildOpt(std.testing.allocator, parsed.module, .{});
...
test "LLVM rejects prebuilt MIR with missing phys call target facts" {
    ...
    try std.testing.expectError(
        error.InvalidMirCallTargetFacts,
        appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &module_mir, &output, "llvm_missing_phys_call_target_facts.mc", .{}, false, .riscv64, null),
    );
}
...
test "LLVM rejects prebuilt MIR with missing phys result type facts" {
    ...
    try std.testing.expectError(
        error.InvalidMirTargetTypeFacts,
        appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &module_mir, &output, "llvm_missing_phys_result_type_facts.mc", .{}, false, .riscv64, null),
    );
}
...
test "LLVM rejects prebuilt MIR with missing MaybeUninit call target facts" {
    ...
    try std.testing.expectError(
        error.InvalidMirCallTargetFacts,
        appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &module_mir, &output, "llvm_missing_maybe_uninit_call_target_facts.mc", .{}, false, .riscv64, null),
    );
}
...
test "LLVM rejects prebuilt MIR with missing raw store call target facts" {
    ...
    try std.testing.expectError(
        error.InvalidMirCallTargetFacts,
        appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &module_mir, &output, "llvm_missing_raw_store_call_target_facts.mc", .{}, false, .riscv64, null),
    );
}
...
test "LLVM rejects prebuilt MIR with missing raw load call target facts" {
    ...
    try std.testing.expectError(
        error.InvalidMirCallTargetFacts,
        appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &module_mir, &output, "llvm_missing_raw_load_call_target_facts.mc", .{}, false, .riscv64, null),
    );
}
...
test "LLVM rejects prebuilt MIR with missing raw ptr call target facts" {
    ...
    try std.testing.expectError(
        error.InvalidMirCallTargetFacts,
        appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &module_mir, &output, "llvm_missing_raw_ptr_call_target_facts.mc", .{}, false, .riscv64, null),
    );
}
...
test "LLVM raw memory calls require complete MIR target type facts" {
    ...
    for ([_][]const u8{ "read", "pointer", "write" }) |name| {
        ...
        var output: std.ArrayList(u8) = .empty;
        defer output.deinit(std.testing.allocator);
        try std.testing.expectError(error.InvalidMirTargetTypeFacts, appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &module_mir, &output, "llvm_raw_memory_type_facts.mc", .{}, false, .riscv64, null));
    }
}
...
test "LLVM varargs calls require complete MIR cursor payload and result facts" {
    ...
    var complete_output: std.ArrayList(u8) = .empty;
    defer complete_output.deinit(std.testing.allocator);
    try appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &complete, &complete_output, "llvm_varargs_call_type_facts.mc", .{}, false, .riscv64, null);
    ...
    try std.testing.expectError(error.InvalidMirCallTargetFacts, appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &missing_calls, &call_output, "llvm_varargs_call_type_facts.mc", .{}, false, .riscv64, null));
    ...
    for ([_]mir.TargetTypeKind{ .va_cursor, .va_payload, .va_result }) |kind| {
        ...
        var type_output: std.ArrayList(u8) = .empty;
        defer type_output.deinit(std.testing.allocator);
        try std.testing.expectError(error.InvalidMirTargetTypeFacts, appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &missing_type, &type_output, "llvm_varargs_call_type_facts.mc", .{}, false, .riscv64, null));
    }
}
...
test "LLVM explicit traps require exact MIR reason identities" {
    ...
    var complete_output: std.ArrayList(u8) = .empty;
    defer complete_output.deinit(std.testing.allocator);
    try appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &complete, &complete_output, "llvm_explicit_trap_target_facts.mc", .{}, false, .riscv64, null);
    ...
    var missing_output: std.ArrayList(u8) = .empty;
    defer missing_output.deinit(std.testing.allocator);
    try std.testing.expectError(error.InvalidMirCallTargetFacts, appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &missing, &missing_output, "llvm_explicit_trap_target_facts.mc", .{}, false, .riscv64, null));
    ...
    var stale_output: std.ArrayList(u8) = .empty;
    defer stale_output.deinit(std.testing.allocator);
    try std.testing.expectError(error.InvalidMirCallTargetFacts, appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &stale, &stale_output, "llvm_explicit_trap_target_facts.mc", .{}, false, .riscv64, null));
}
...
test "LLVM runtime asserts require MIR bool condition types" {
    ...
    var complete_output: std.ArrayList(u8) = .empty;
    defer complete_output.deinit(std.testing.allocator);
    try appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &complete, &complete_output, "llvm_assert_condition_type_facts.mc", .{}, false, .riscv64, null);
    ...
    var missing_output: std.ArrayList(u8) = .empty;
    defer missing_output.deinit(std.testing.allocator);
    try std.testing.expectError(error.InvalidMirTargetTypeFacts, appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &missing, &missing_output, "llvm_assert_condition_type_facts.mc", .{}, false, .riscv64, null));
    ...
    var stale_output: std.ArrayList(u8) = .empty;
    defer stale_output.deinit(std.testing.allocator);
    try std.testing.expectError(error.UnsupportedLlvmEmission, appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &stale, &stale_output, "llvm_assert_condition_type_facts.mc", .{}, false, .riscv64, null));
}
...
test "LLVM while loops require MIR bool condition types" {
    ...
    var complete_output: std.ArrayList(u8) = .empty;
    defer complete_output.deinit(std.testing.allocator);
    try appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &complete, &complete_output, "llvm_loop_condition_type_facts.mc", .{}, false, .riscv64, null);
    try std.testing.expect(std.mem.indexOf(u8, complete_output.items, "br i1 %") != null);
    ...
    var missing_output: std.ArrayList(u8) = .empty;
    defer missing_output.deinit(std.testing.allocator);
    try std.testing.expectError(error.InvalidMirTargetTypeFacts, appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &missing, &missing_output, "llvm_loop_condition_type_facts.mc", .{}, false, .riscv64, null));
    ...
    var stale_output: std.ArrayList(u8) = .empty;
    defer stale_output.deinit(std.testing.allocator);
    try std.testing.expectError(error.UnsupportedLlvmEmission, appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &stale, &stale_output, "llvm_loop_condition_type_facts.mc", .{}, false, .riscv64, null));
}
...
test "LLVM switches require MIR subject types" {
    ...
    var complete_output: std.ArrayList(u8) = .empty;
    defer complete_output.deinit(std.testing.allocator);
    try appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &complete, &complete_output, "llvm_switch_subject_type_facts.mc", .{}, false, .riscv64, null);
    ...
    var missing_output: std.ArrayList(u8) = .empty;
    defer missing_output.deinit(std.testing.allocator);
    try std.testing.expectError(error.InvalidMirTargetTypeFacts, appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &missing, &missing_output, "llvm_switch_subject_type_facts.mc", .{}, false, .riscv64, null));
    ...
    for ([_][]const u8{ "result_subject", "nullable_subject", "union_subject", "enum_subject", "bool_subject" }) |name| {
        ...
        var stale_output: std.ArrayList(u8) = .empty;
        defer stale_output.deinit(std.testing.allocator);
        try std.testing.expectError(error.UnsupportedLlvmEmission, appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &stale, &stale_output, "llvm_switch_subject_type_facts.mc", .{}, false, .riscv64, null));
    }
    ...
    try std.testing.expectError(error.InvalidMirTargetTypeFacts, appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &stale_nullable_repr, &stale_nullable_repr_output, "llvm_switch_subject_type_facts.mc", .{}, false, .riscv64, null));
    ...
    try std.testing.expectError(error.InvalidMirTargetTypeFacts, appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &unknown_subject_repr, &unknown_subject_repr_output, "llvm_switch_subject_type_facts.mc", .{}, false, .riscv64, null));
}
...
test "LLVM if-let statements require MIR subject types" {
    ...
    var complete_output: std.ArrayList(u8) = .empty;
    defer complete_output.deinit(std.testing.allocator);
    try appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &complete, &complete_output, "llvm_if_let_subject_type_facts.mc", .{}, false, .riscv64, null);
    ...
    var missing_output: std.ArrayList(u8) = .empty;
    defer missing_output.deinit(std.testing.allocator);
    try std.testing.expectError(error.InvalidMirTargetTypeFacts, appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &missing, &missing_output, "llvm_if_let_subject_type_facts.mc", .{}, false, .riscv64, null));
    ...
    for ([_][]const u8{ "result_subject", "nullable_subject" }) |name| {
        ...
        var stale_output: std.ArrayList(u8) = .empty;
        defer stale_output.deinit(std.testing.allocator);
        try std.testing.expectError(error.UnsupportedLlvmEmission, appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &stale, &stale_output, "llvm_if_let_subject_type_facts.mc", .{}, false, .riscv64, null));
    }
    ...
    var stale_nullable_repr_output: std.ArrayList(u8) = .empty;
    defer stale_nullable_repr_output.deinit(std.testing.allocator);
    try std.testing.expectError(error.InvalidMirTargetTypeFacts, appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &stale_nullable_repr, &stale_nullable_repr_output, "llvm_if_let_subject_type_facts.mc", .{}, false, .riscv64, null));
}
...
test "LLVM try expressions require MIR operand and result types" {
    ...
    var complete_output: std.ArrayList(u8) = .empty;
    defer complete_output.deinit(std.testing.allocator);
    try appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &complete, &complete_output, "llvm_try_operand_type_facts.mc", .{}, false, .riscv64, null);
    ...
    var missing_output: std.ArrayList(u8) = .empty;
    defer missing_output.deinit(std.testing.allocator);
    try std.testing.expectError(error.InvalidMirTargetTypeFacts, appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &missing, &missing_output, "llvm_try_operand_type_facts.mc", .{}, false, .riscv64, null));
    ...
    var missing_result_output: std.ArrayList(u8) = .empty;
    defer missing_result_output.deinit(std.testing.allocator);
    try std.testing.expectError(error.InvalidMirTargetTypeFacts, appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &missing_result, &missing_result_output, "llvm_try_operand_type_facts.mc", .{}, false, .riscv64, null));
    ...
    for ([_][]const u8{ "result_try", "nullable_try" }) |name| {
        ...
        var stale_output: std.ArrayList(u8) = .empty;
        defer stale_output.deinit(std.testing.allocator);
        try std.testing.expectError(error.UnsupportedLlvmEmission, appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &stale, &stale_output, "llvm_try_operand_type_facts.mc", .{}, false, .riscv64, null));
    }
    ...
    for ([_][]const u8{ "result_try", "nullable_try" }) |name| {
        ...
        var stale_result_output: std.ArrayList(u8) = .empty;
        defer stale_result_output.deinit(std.testing.allocator);
        try std.testing.expectError(error.UnsupportedLlvmEmission, appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &stale_result, &stale_result_output, "llvm_try_operand_type_facts.mc", .{}, false, .riscv64, null));
    }
}
...
test "LLVM for loops require MIR iterable and element types" {
    ...
    var complete_output: std.ArrayList(u8) = .empty;
    defer complete_output.deinit(std.testing.allocator);
    try appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &complete, &complete_output, "llvm_for_loop_type_facts.mc", .{}, false, .riscv64, null);
    ...
    for ([_]mir.TargetTypeKind{ .for_iterable, .for_element }) |kind| {
        ...
        var missing_output: std.ArrayList(u8) = .empty;
        defer missing_output.deinit(std.testing.allocator);
        try std.testing.expectError(error.InvalidMirTargetTypeFacts, appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &missing, &missing_output, "llvm_for_loop_type_facts.mc", .{}, false, .riscv64, null));
        ...
        var stale_output: std.ArrayList(u8) = .empty;
        defer stale_output.deinit(std.testing.allocator);
        try std.testing.expectError(error.UnsupportedLlvmEmission, appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &stale, &stale_output, "llvm_for_loop_type_facts.mc", .{}, false, .riscv64, null));
    }
}
...
test "LLVM inferred local copies require MIR types" {
    ...
    var complete_output: std.ArrayList(u8) = .empty;
    defer complete_output.deinit(std.testing.allocator);
    try appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &complete, &complete_output, "llvm_inferred_local_copy_types.mc", .{}, false, .riscv64, null);
    ...
    var missing_output: std.ArrayList(u8) = .empty;
    defer missing_output.deinit(std.testing.allocator);
    try std.testing.expectError(error.InvalidMirTargetTypeFacts, appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &missing, &missing_output, "llvm_inferred_local_copy_types.mc", .{}, false, .riscv64, null));
    ...
    var stale_output: std.ArrayList(u8) = .empty;
    defer stale_output.deinit(std.testing.allocator);
    try std.testing.expectError(error.UnsupportedLlvmEmission, appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &stale, &stale_output, "llvm_inferred_local_copy_types.mc", .{}, false, .riscv64, null));
}
...
test "LLVM inferred local casts require MIR types" {
    ...
    var complete_output: std.ArrayList(u8) = .empty;
    defer complete_output.deinit(std.testing.allocator);
    try appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &complete, &complete_output, "llvm_inferred_local_cast_types.mc", .{}, false, .riscv64, null);
    ...
    var missing_output: std.ArrayList(u8) = .empty;
    defer missing_output.deinit(std.testing.allocator);
    try std.testing.expectError(error.InvalidMirTargetTypeFacts, appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &missing, &missing_output, "llvm_inferred_local_cast_types.mc", .{}, false, .riscv64, null));
    ...
    var stale_output: std.ArrayList(u8) = .empty;
    defer stale_output.deinit(std.testing.allocator);
    try std.testing.expectError(error.UnsupportedLlvmEmission, appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &stale, &stale_output, "llvm_inferred_local_cast_types.mc", .{}, false, .riscv64, null));
}
...
test "LLVM inferred local binary expressions require MIR types" {
    ...
    var complete_output: std.ArrayList(u8) = .empty;
    defer complete_output.deinit(std.testing.allocator);
    try appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &complete, &complete_output, "llvm_inferred_local_binary_types.mc", .{}, false, .riscv64, null);
    ...
    var missing_output: std.ArrayList(u8) = .empty;
    defer missing_output.deinit(std.testing.allocator);
    try std.testing.expectError(error.InvalidMirTargetTypeFacts, appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &missing, &missing_output, "llvm_inferred_local_binary_types.mc", .{}, false, .riscv64, null));
    ...
    var stale_output: std.ArrayList(u8) = .empty;
    defer stale_output.deinit(std.testing.allocator);
    try std.testing.expectError(error.UnsupportedLlvmEmission, appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &stale, &stale_output, "llvm_inferred_local_binary_types.mc", .{}, false, .riscv64, null));
    ...
    try std.testing.expectError(error.InvalidMirTargetTypeFacts, appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &missing_bitwise, &missing_bitwise_output, "llvm_inferred_local_binary_types.mc", .{}, false, .riscv64, null));
    ...
    try std.testing.expectError(error.UnsupportedLlvmEmission, appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &stale_bitwise, &stale_bitwise_output, "llvm_inferred_local_binary_types.mc", .{}, false, .riscv64, null));
}
...
test "LLVM inferred local literals require MIR types" {
    ...
    var complete_output: std.ArrayList(u8) = .empty;
    defer complete_output.deinit(std.testing.allocator);
    try appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &complete, &complete_output, "llvm_inferred_local_literal_types.mc", .{}, false, .riscv64, null);
    ...
    var missing_output: std.ArrayList(u8) = .empty;
    defer missing_output.deinit(std.testing.allocator);
    try std.testing.expectError(error.InvalidMirTargetTypeFacts, appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &missing, &missing_output, "llvm_inferred_local_literal_types.mc", .{}, false, .riscv64, null));
    ...
    try std.testing.expectError(error.InvalidMirTargetTypeFacts, appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &missing_literal_result, &missing_literal_result_output, "llvm_inferred_local_literal_types.mc", .{}, false, .riscv64, null));
    ...
    var stale_output: std.ArrayList(u8) = .empty;
    defer stale_output.deinit(std.testing.allocator);
    try std.testing.expectError(error.UnsupportedLlvmEmission, appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &stale, &stale_output, "llvm_inferred_local_literal_types.mc", .{}, false, .riscv64, null));
    ...
    try std.testing.expectError(error.UnsupportedLlvmEmission, appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &stale_literal_result, &stale_literal_result_output, "llvm_inferred_local_literal_types.mc", .{}, false, .riscv64, null));
}
...
test "LLVM sequenced comparison literals require MIR result types" {
    ...
    var complete_output: std.ArrayList(u8) = .empty;
    defer complete_output.deinit(std.testing.allocator);
    try appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &complete, &complete_output, "llvm_condition_literal_result.mc", .{}, false, .riscv64, null);
    ...
    var missing_output: std.ArrayList(u8) = .empty;
    defer missing_output.deinit(std.testing.allocator);
    try std.testing.expectError(error.InvalidMirTargetTypeFacts, appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &missing, &missing_output, "llvm_condition_literal_result.mc", .{}, false, .riscv64, null));
    ...
    var stale_output: std.ArrayList(u8) = .empty;
    defer stale_output.deinit(std.testing.allocator);
    try std.testing.expectError(error.UnsupportedLlvmEmission, appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &stale, &stale_output, "llvm_condition_literal_result.mc", .{}, false, .riscv64, null));
}
...
test "LLVM sequenced comparison member operands require MIR result types" {
    ...
    var complete_output: std.ArrayList(u8) = .empty;
    defer complete_output.deinit(std.testing.allocator);
    try appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &complete, &complete_output, "llvm_condition_member_result.mc", .{}, false, .riscv64, null);
    ...
    var missing_output: std.ArrayList(u8) = .empty;
    defer missing_output.deinit(std.testing.allocator);
    try std.testing.expectError(error.InvalidMirTargetTypeFacts, appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &missing, &missing_output, "llvm_condition_member_result.mc", .{}, false, .riscv64, null));
    ...
    var stale_output: std.ArrayList(u8) = .empty;
    defer stale_output.deinit(std.testing.allocator);
    try std.testing.expectError(error.UnsupportedLlvmEmission, appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &stale, &stale_output, "llvm_condition_member_result.mc", .{}, false, .riscv64, null));
}
...
test "LLVM boolean expressions require MIR result types" {
    ...
    var complete_output: std.ArrayList(u8) = .empty;
    defer complete_output.deinit(std.testing.allocator);
    try appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &complete, &complete_output, "llvm_boolean_expression_result.mc", .{}, false, .riscv64, null);
    ...
    var missing_output: std.ArrayList(u8) = .empty;
    defer missing_output.deinit(std.testing.allocator);
    try std.testing.expectError(error.InvalidMirTargetTypeFacts, appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &missing, &missing_output, "llvm_boolean_expression_result.mc", .{}, false, .riscv64, null));
    ...
    var stale_output: std.ArrayList(u8) = .empty;
    defer stale_output.deinit(std.testing.allocator);
    try std.testing.expectError(error.UnsupportedLlvmEmission, appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &stale, &stale_output, "llvm_boolean_expression_result.mc", .{}, false, .riscv64, null));
}
...
test "LLVM inferred local unary expressions require MIR types" {
    ...
    var complete_output: std.ArrayList(u8) = .empty;
    defer complete_output.deinit(std.testing.allocator);
    try appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &complete, &complete_output, "llvm_inferred_local_unary_types.mc", .{}, false, .riscv64, null);
    ...
    var missing_output: std.ArrayList(u8) = .empty;
    defer missing_output.deinit(std.testing.allocator);
    try std.testing.expectError(error.InvalidMirTargetTypeFacts, appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &missing, &missing_output, "llvm_inferred_local_unary_types.mc", .{}, false, .riscv64, null));
    ...
    var stale_output: std.ArrayList(u8) = .empty;
    defer stale_output.deinit(std.testing.allocator);
    try std.testing.expectError(error.UnsupportedLlvmEmission, appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &stale, &stale_output, "llvm_inferred_local_unary_types.mc", .{}, false, .riscv64, null));
}
...
test "LLVM inferred local direct calls require MIR types" {
    ...
    var complete_output: std.ArrayList(u8) = .empty;
    defer complete_output.deinit(std.testing.allocator);
    try appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &complete, &complete_output, "llvm_inferred_local_call_types.mc", .{}, false, .riscv64, null);
    try std.testing.expect(std.mem.indexOf(u8, complete_output.items, "%count") != null);
    ...
    var missing_output: std.ArrayList(u8) = .empty;
    defer missing_output.deinit(std.testing.allocator);
    try std.testing.expectError(error.InvalidMirTargetTypeFacts, appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &missing, &missing_output, "llvm_inferred_local_call_types.mc", .{}, false, .riscv64, null));
    ...
    var stale_output: std.ArrayList(u8) = .empty;
    defer stale_output.deinit(std.testing.allocator);
    try std.testing.expectError(error.UnsupportedLlvmEmission, appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &stale, &stale_output, "llvm_inferred_local_call_types.mc", .{}, false, .riscv64, null));
    ...
    try std.testing.expectError(error.InvalidMirTargetTypeFacts, appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &missing_call_result, &missing_call_result_output, "llvm_inferred_local_call_types.mc", .{}, false, .riscv64, null));
    ...
    try std.testing.expectError(error.UnsupportedLlvmEmission, appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &stale_call_result, &stale_call_result_output, "llvm_inferred_local_call_types.mc", .{}, false, .riscv64, null));
}
...
test "LLVM inferred local Result direct calls require MIR types" {
    ...
    var complete_output: std.ArrayList(u8) = .empty;
    defer complete_output.deinit(std.testing.allocator);
    try appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &complete, &complete_output, "llvm_inferred_local_result_call_types.mc", .{}, false, .riscv64, null);
    ...
    var missing_output: std.ArrayList(u8) = .empty;
    defer missing_output.deinit(std.testing.allocator);
    try std.testing.expectError(error.InvalidMirTargetTypeFacts, appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &missing, &missing_output, "llvm_inferred_local_result_call_types.mc", .{}, false, .riscv64, null));
    ...
    var stale_output: std.ArrayList(u8) = .empty;
    defer stale_output.deinit(std.testing.allocator);
    try std.testing.expectError(error.UnsupportedLlvmEmission, appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &stale, &stale_output, "llvm_inferred_local_result_call_types.mc", .{}, false, .riscv64, null));
}
...
test "LLVM inferred local indirect calls require MIR types" {
    ...
    var complete_output: std.ArrayList(u8) = .empty;
    defer complete_output.deinit(std.testing.allocator);
    try appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &complete, &complete_output, "llvm_inferred_local_indirect_call_types.mc", .{}, false, .riscv64, null);
    try std.testing.expect(std.mem.indexOf(u8, complete_output.items, "%result") != null);
    ...
    var missing_output: std.ArrayList(u8) = .empty;
    defer missing_output.deinit(std.testing.allocator);
    try std.testing.expectError(error.InvalidMirTargetTypeFacts, appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &missing, &missing_output, "llvm_inferred_local_indirect_call_types.mc", .{}, false, .riscv64, null));
    ...
    var stale_output: std.ArrayList(u8) = .empty;
    defer stale_output.deinit(std.testing.allocator);
    try std.testing.expectError(error.UnsupportedLlvmEmission, appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &stale, &stale_output, "llvm_inferred_local_indirect_call_types.mc", .{}, false, .riscv64, null));
}
...
test "LLVM inferred local atomic and MaybeUninit calls require MIR types" {
    ...
    var complete_output: std.ArrayList(u8) = .empty;
    defer complete_output.deinit(std.testing.allocator);
    try appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &complete, &complete_output, "llvm_builtin_inferred_local_types.mc", .{}, false, .riscv64, null);
    ...
    for ([_][]const u8{ "atomic_inferred_locals", "maybe_uninit_inferred_local" }) |name| {
        ...
        var missing_output: std.ArrayList(u8) = .empty;
        defer missing_output.deinit(std.testing.allocator);
        try std.testing.expectError(error.InvalidMirTargetTypeFacts, appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &missing, &missing_output, "llvm_builtin_inferred_local_types.mc", .{}, false, .riscv64, null));
        ...
        var stale_output: std.ArrayList(u8) = .empty;
        defer stale_output.deinit(std.testing.allocator);
        try std.testing.expectError(error.UnsupportedLlvmEmission, appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &stale, &stale_output, "llvm_builtin_inferred_local_types.mc", .{}, false, .riscv64, null));
    }
}
...
test "LLVM inferred local phys calls require MIR types" {
    ...
    var complete_output: std.ArrayList(u8) = .empty;
    defer complete_output.deinit(std.testing.allocator);
    try appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &complete, &complete_output, "llvm_inferred_phys_local_type.mc", .{}, false, .riscv64, null);
    ...
    var missing_output: std.ArrayList(u8) = .empty;
    defer missing_output.deinit(std.testing.allocator);
    try std.testing.expectError(error.InvalidMirTargetTypeFacts, appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &missing, &missing_output, "llvm_inferred_phys_local_type.mc", .{}, false, .riscv64, null));
    ...
    var stale_output: std.ArrayList(u8) = .empty;
    defer stale_output.deinit(std.testing.allocator);
    try std.testing.expectError(error.UnsupportedLlvmEmission, appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &stale, &stale_output, "llvm_inferred_phys_local_type.mc", .{}, false, .riscv64, null));
}
...
test "LLVM inferred local bitcast calls require MIR types" {
    ...
    var complete_output: std.ArrayList(u8) = .empty;
    defer complete_output.deinit(std.testing.allocator);
    try appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &complete, &complete_output, "llvm_inferred_bitcast_local_type.mc", .{}, false, .riscv64, null);
    ...
    var missing_output: std.ArrayList(u8) = .empty;
    defer missing_output.deinit(std.testing.allocator);
    try std.testing.expectError(error.InvalidMirTargetTypeFacts, appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &missing, &missing_output, "llvm_inferred_bitcast_local_type.mc", .{}, false, .riscv64, null));
    ...
    var stale_output: std.ArrayList(u8) = .empty;
    defer stale_output.deinit(std.testing.allocator);
    try std.testing.expectError(error.UnsupportedLlvmEmission, appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &stale, &stale_output, "llvm_inferred_bitcast_local_type.mc", .{}, false, .riscv64, null));
}
...
test "LLVM inferred local byte-view calls require MIR types" {
    ...
    var complete_output: std.ArrayList(u8) = .empty;
    defer complete_output.deinit(std.testing.allocator);
    try appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &complete, &complete_output, "llvm_inferred_byte_view_local_types.mc", .{}, false, .riscv64, null);
    ...
    for ([_][]const u8{ "inferred_byte_view", "inferred_byte_equal" }) |name| {
        ...
        var missing_output: std.ArrayList(u8) = .empty;
        defer missing_output.deinit(std.testing.allocator);
        try std.testing.expectError(error.InvalidMirTargetTypeFacts, appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &missing, &missing_output, "llvm_inferred_byte_view_local_types.mc", .{}, false, .riscv64, null));
        ...
        var stale_output: std.ArrayList(u8) = .empty;
        defer stale_output.deinit(std.testing.allocator);
        try std.testing.expectError(error.UnsupportedLlvmEmission, appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &stale, &stale_output, "llvm_inferred_byte_view_local_types.mc", .{}, false, .riscv64, null));
    }
}
...
test "LLVM inferred local enum raw calls require MIR types" {
    ...
    var complete_output: std.ArrayList(u8) = .empty;
    defer complete_output.deinit(std.testing.allocator);
    try appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &complete, &complete_output, "llvm_inferred_enum_raw_local_type.mc", .{}, false, .riscv64, null);
    ...
    var missing_output: std.ArrayList(u8) = .empty;
    defer missing_output.deinit(std.testing.allocator);
    try std.testing.expectError(error.InvalidMirTargetTypeFacts, appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &missing, &missing_output, "llvm_inferred_enum_raw_local_type.mc", .{}, false, .riscv64, null));
    ...
    var stale_output: std.ArrayList(u8) = .empty;
    defer stale_output.deinit(std.testing.allocator);
    try std.testing.expectError(error.UnsupportedLlvmEmission, appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &stale, &stale_output, "llvm_inferred_enum_raw_local_type.mc", .{}, false, .riscv64, null));
}
...
test "LLVM inferred local conversion calls require MIR types" {
    ...
    var complete_output: std.ArrayList(u8) = .empty;
    defer complete_output.deinit(std.testing.allocator);
    try appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &complete, &complete_output, "llvm_inferred_conversion_local_type.mc", .{}, false, .riscv64, null);
    try std.testing.expect(std.mem.indexOf(u8, complete_output.items, "%narrowed") != null);
    ...
    var missing_output: std.ArrayList(u8) = .empty;
    defer missing_output.deinit(std.testing.allocator);
    try std.testing.expectError(error.InvalidMirTargetTypeFacts, appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &missing, &missing_output, "llvm_inferred_conversion_local_type.mc", .{}, false, .riscv64, null));
    ...
    var stale_output: std.ArrayList(u8) = .empty;
    defer stale_output.deinit(std.testing.allocator);
    try std.testing.expectError(error.UnsupportedLlvmEmission, appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &stale, &stale_output, "llvm_inferred_conversion_local_type.mc", .{}, false, .riscv64, null));
}
...
test "LLVM inferred local reflection calls require MIR types" {
    ...
    var complete_output: std.ArrayList(u8) = .empty;
    defer complete_output.deinit(std.testing.allocator);
    try appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &complete, &complete_output, "llvm_inferred_reflection_local_type.mc", .{}, false, .riscv64, null);
    ...
    var missing_output: std.ArrayList(u8) = .empty;
    defer missing_output.deinit(std.testing.allocator);
    try std.testing.expectError(error.InvalidMirTargetTypeFacts, appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &missing, &missing_output, "llvm_inferred_reflection_local_type.mc", .{}, false, .riscv64, null));
    ...
    var stale_output: std.ArrayList(u8) = .empty;
    defer stale_output.deinit(std.testing.allocator);
    try std.testing.expectError(error.UnsupportedLlvmEmission, appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &stale, &stale_output, "llvm_inferred_reflection_local_type.mc", .{}, false, .riscv64, null));
}
...
test "LLVM inferred local semantic escape calls require MIR types" {
    ...
    var complete_output: std.ArrayList(u8) = .empty;
    defer complete_output.deinit(std.testing.allocator);
    try appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &complete, &complete_output, "llvm_inferred_semantic_escape_local_type.mc", .{}, false, .riscv64, null);
    ...
    var missing_output: std.ArrayList(u8) = .empty;
    defer missing_output.deinit(std.testing.allocator);
    try std.testing.expectError(error.InvalidMirTargetTypeFacts, appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &missing, &missing_output, "llvm_inferred_semantic_escape_local_type.mc", .{}, false, .riscv64, null));
    ...
    var stale_output: std.ArrayList(u8) = .empty;
    defer stale_output.deinit(std.testing.allocator);
    try std.testing.expectError(error.UnsupportedLlvmEmission, appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &stale, &stale_output, "llvm_inferred_semantic_escape_local_type.mc", .{}, false, .riscv64, null));
}
...
test "LLVM inferred local raw result calls require MIR types" {
    ...
    var complete_output: std.ArrayList(u8) = .empty;
    defer complete_output.deinit(std.testing.allocator);
    try appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &complete, &complete_output, "llvm_inferred_raw_local_types.mc", .{}, false, .riscv64, null);
    try std.testing.expect(std.mem.indexOf(u8, complete_output.items, "%value") != null);
    try std.testing.expect(std.mem.indexOf(u8, complete_output.items, "%pointer") != null);
    ...
    for ([_][]const u8{ "inferred_raw_load", "inferred_raw_ptr" }) |name| {
        ...
        var missing_output: std.ArrayList(u8) = .empty;
        defer missing_output.deinit(std.testing.allocator);
        try std.testing.expectError(error.InvalidMirTargetTypeFacts, appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &missing, &missing_output, "llvm_inferred_raw_local_types.mc", .{}, false, .riscv64, null));
        ...
        var stale_output: std.ArrayList(u8) = .empty;
        defer stale_output.deinit(std.testing.allocator);
        try std.testing.expectError(error.UnsupportedLlvmEmission, appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &stale, &stale_output, "llvm_inferred_raw_local_types.mc", .{}, false, .riscv64, null));
    }
}
...
test "LLVM inferred local dyn dispatch calls require MIR types" {
    ...
    var complete_output: std.ArrayList(u8) = .empty;
    defer complete_output.deinit(std.testing.allocator);
    try appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &complete, &complete_output, "llvm_inferred_local_dyn_dispatch_call_types.mc", .{}, false, .riscv64, null);
    try std.testing.expect(std.mem.indexOf(u8, complete_output.items, "%result") != null);
    ...
    var missing_output: std.ArrayList(u8) = .empty;
    defer missing_output.deinit(std.testing.allocator);
    try std.testing.expectError(error.InvalidMirTargetTypeFacts, appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &missing, &missing_output, "llvm_inferred_local_dyn_dispatch_call_types.mc", .{}, false, .riscv64, null));
    ...
    try std.testing.expectError(error.InvalidMirTargetTypeFacts, appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &missing_argument, &missing_argument_output, "llvm_inferred_local_dyn_dispatch_call_types.mc", .{}, false, .riscv64, null));
    ...
    var stale_output: std.ArrayList(u8) = .empty;
    defer stale_output.deinit(std.testing.allocator);
    try std.testing.expectError(error.UnsupportedLlvmEmission, appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &stale, &stale_output, "llvm_inferred_local_dyn_dispatch_call_types.mc", .{}, false, .riscv64, null));
    ...
    try std.testing.expectError(error.UnsupportedLlvmEmission, appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &stale_argument, &stale_argument_output, "llvm_inferred_local_dyn_dispatch_call_types.mc", .{}, false, .riscv64, null));
}
...
test "LLVM ordinary direct calls require MIR result and argument types" {
    ...
    var complete_output: std.ArrayList(u8) = .empty;
    defer complete_output.deinit(std.testing.allocator);
    try appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &complete, &complete_output, "llvm_direct_call_type_facts.mc", .{}, false, .riscv64, null);
    ...
    var missing_result_output: std.ArrayList(u8) = .empty;
    defer missing_result_output.deinit(std.testing.allocator);
    try std.testing.expectError(error.InvalidMirTargetTypeFacts, appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &missing_result, &missing_result_output, "llvm_direct_call_type_facts.mc", .{}, false, .riscv64, null));
    ...
    var missing_argument_output: std.ArrayList(u8) = .empty;
    defer missing_argument_output.deinit(std.testing.allocator);
    try std.testing.expectError(error.InvalidMirTargetTypeFacts, appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &missing_argument, &missing_argument_output, "llvm_direct_call_type_facts.mc", .{}, false, .riscv64, null));
    ...
    var stale_output: std.ArrayList(u8) = .empty;
    defer stale_output.deinit(std.testing.allocator);
    try std.testing.expectError(error.UnsupportedLlvmEmission, appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &stale, &stale_output, "llvm_direct_call_type_facts.mc", .{}, false, .riscv64, null));
}
...
test "LLVM indirect calls require MIR callee signature facts" {
    ...
    var complete_output: std.ArrayList(u8) = .empty;
    defer complete_output.deinit(std.testing.allocator);
    try appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &complete, &complete_output, "llvm_indirect_call_signature_facts.mc", .{}, false, .riscv64, null);
    ...
    for ([_][]const u8{ "invoke_pointer", "invoke_closure" }) |name| {
        ...
        var output: std.ArrayList(u8) = .empty;
        defer output.deinit(std.testing.allocator);
        try std.testing.expectError(error.InvalidMirTargetTypeFacts, appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &missing, &output, "llvm_indirect_call_signature_facts.mc", .{}, false, .riscv64, null));
        ...
        var stale_output: std.ArrayList(u8) = .empty;
        defer stale_output.deinit(std.testing.allocator);
        try std.testing.expectError(error.UnsupportedLlvmEmission, appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &stale, &stale_output, "llvm_indirect_call_signature_facts.mc", .{}, false, .riscv64, null));
    }
}
...
test "LLVM rejects prebuilt MIR with missing cpu pause call target facts" {
    ...
    try std.testing.expectError(
        error.InvalidMirCallTargetFacts,
        appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &module_mir, &output, "llvm_missing_cpu_pause_call_target_facts.mc", .{}, false, .riscv64, null),
    );
}
...
test "LLVM rejects prebuilt MIR with missing fence call target facts" {
    ...
    try std.testing.expectError(
        error.InvalidMirCallTargetFacts,
        appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &module_mir, &output, "llvm_missing_fence_call_target_facts.mc", .{}, false, .riscv64, null),
    );
}
...
test "LLVM rejects prebuilt MIR with stale call target facts" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try std.testing.expectError(error.InvalidMirCallTargetFacts, appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &module_mir, &output, "llvm_stale_call_target_facts.mc", .{}, false, .riscv64, null));
}
...
test "LLVM rejects prebuilt MIR with stale integer facts" {
    ...
    try std.testing.expectError(
        error.InvalidMirIntegerFacts,
        appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &module_mir, &output, "llvm_stale_integer_facts.mc", .{}, false, .riscv64, null),
    );
}
...
test "LLVM ordinary global scalar accesses lower to unordered atomics" {
    ...
    try expectContainsAny(raw_many_offset_zero_body, &.{ "zext i32 0 to i64", ", i64 0" });
    ...
    try expectContainsAny(raw_many_offset_one_body, &.{ "zext i32 1 to i64", ", i64 1" });
    ...
    try expectContainsAny(raw_many_offset_unknown_body, &.{ "zext i32 0 to i64", ", i64 0" });
...
test "LLVM raw-many zero direct local without MIR fact lowers conservatively" {
    ...
    try expectContainsAny(body, &.{ "zext i32 0 to i64", ", i64 0" });
    try expectContains(body, "load atomic i32, ptr %");
    ...
    try expectContainsAny(assignment_body, &.{ "zext i32 0 to i64", ", i64 0" });
    try expectContains(assignment_body, "load atomic i32, ptr %");
    try expectContains(assignment_body, " unordered, align 4");
    try expectNotContains(assignment_body, "load i32, ptr %");
}
...
test "LLVM check elision is scoped to the current function" {
    ...
    var output: std.ArrayList(u8) = .empty;
    defer output.deinit(std.testing.allocator);
    try appendLlvmWithSourcePathTest(std.testing.allocator, module, &output, "combined.mc", true);
...
test "LLVM backend reuses prebuilt verified MIR without changing output" {
    ...
    try appendLlvmWithSourcePathTest(std.testing.allocator, parsed.module, &rebuilt_output, "llvm_prebuilt_mir.mc", true);
    ...
    try appendLlvmCheckedMirTest(std.testing.allocator, parsed.module, &module_mir, &prebuilt_output, "llvm_prebuilt_mir.mc", .{ .optimize = true }, false, .riscv64, &reporter);
...
test "LLVM unsupported diagnostics use nearest source span for generated nodes" {
    ...
    var early_metadata = try declaration_artifacts.EarlyDeclarationArtifacts.collectFromDecls(std.testing.allocator, module.decls);
