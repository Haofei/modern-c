...
pub fn operandEmitType(ctx: TypeQueryContext, expr: ast.Expr, locals: ?*std.StringHashMap(LocalInfo)) ?ast.TypeExpr {
    switch (expr.kind) {
        ...
        // bounded data-address place family. Other address expressions remain
        // unsupported here rather than rediscovered from backend-local state.
...
pub fn derefPointeeType(ctx: TypeQueryContext, expr: ast.Expr, locals: ?*std.StringHashMap(LocalInfo)) ?ast.TypeExpr {
    return switch (expr.kind) {
        ...
        .member, .index => blk: {
            ...
            break :blk pointeeTypeFromPointerLike(ctx, ty);
        },
        .grouped => |inner| if (expr.span.line == 0 and expr.span.column == 0)
