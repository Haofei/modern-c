...
fn backendLower(
    ...
    _ = ctx;
    return appendCProfileWithVerifiedProgram(allocator, request.program, request.out, request.opts.profile, request.opts.source_path, request.opts.checks, request.opts.stub_asm, request.opts.reporter) catch |err| backend_mod.lowerErrorFromAny(err);
}
...
pub fn appendLayoutAssertsWithMirArtifacts(
    ...
    return lower_c_emitter.appendLayoutAsserts(allocator, typed_mir, out, struct_names);
}
...
pub fn appendStructDeclsWithMirArtifacts(
    ...
    return lower_c_emitter.appendStructDecls(allocator, typed_mir, out, struct_names);
}
...
pub fn appendCProfileWithMirArtifacts(
    ...
    return appendCProfileWithVerifiedProgram(allocator, program, out, profile, source_path, checks, stub_asm, reporter);
}
