...
///
/// For each struct in `struct_names`, this writes `_Static_assert(sizeof(T) == N, …)` plus a
/// `_Static_assert(offsetof(T, field) == K, …)` for every field, using the SAME layout the C
/// backend computes for emission (`comptimeStructLayout`). A C runtime that hand-mirrors one of
/// these structs includes the header; any drift between the hand-written C struct and MC's
/// authoritative layout (a dropped/reordered field, a retyped field, padding change) then fails
/// to compile. This retires the `Virtq`-drift class (MC struct gained fields the C mirror lacked
/// → the MC code wrote past the C `static Virtq` in BSS → corruption → boot hang).
///
/// A requested name that is not a (non-MMIO) struct in the module is an error: the caller named a
/// struct that does not exist (or is MMIO/opaque), which would silently guard nothing.
...
///
/// Where `appendLayoutAsserts` (A1) keeps a hand-written C mirror and only asserts its layout,
/// this writes the actual `typedef struct { ... }` for each named struct — field order, C types,
/// and any nested array/struct wrappers — using the SAME emission the C backend uses for a full
/// module (`emitStruct`/`emitArrayType`/`cTypeFor`). The C runtime includes this header and drops
/// its hand-maintained copy, so the MC struct is the only source of truth and drift is structurally
/// impossible (there is no second declaration to diverge). The A1 `_Static_assert`s are appended too
/// (belt-and-suspenders: they now check the generated decl against MC's computed layout).
///
/// Only the named structs and the by-value aggregates they transitively embed (nested structs and
/// the `mc_array_*` wrappers MC arrays lower to) are emitted, so the header stays minimal.
...
const CEmitter = struct {
    ...
    fn emitNamedStructDecls(self: *CEmitter, struct_names: []const []const u8) !void {
        ...
        // Emit the closure in by-value dependency order (same algorithm as `emitOrderedAggregates`).
