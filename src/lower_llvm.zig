...
const LlvmEmitter = struct {
    ...
    fn emitCollectedCallableDeclarations(self: *LlvmEmitter) !void {
        ...
        for (self.mir_module.functions) |fn_mir| {
            ...
            if (try self.emitSimpleMirFunction(function, fn_mir, render_attrs)) {
                ...
                fallback_census.record(.llvm, .fallback, self.source_path, fn_mir);
                try self.emitFunction(function, body, render_attrs);
            } else {
                fallback_census.record(.llvm, .unsupported, self.source_path, fn_mir);
    ...
    fn emitSimpleMirFunction(self: *LlvmEmitter, function: anytype, fn_mir: mir.Function, render_attrs: anytype) !bool {
        ...
        const scalar_switch_return_plan = if (local_aggregate_place_update_return_plan == null and local_aggregate_assignment_return_plan == null and place_return_plan == null and simple_trap == null and simple_assert == null)
            ...
            null;
        const direct_call_projected_return_plan = if (local_aggregate_place_update_return_plan == null and local_aggregate_assignment_return_plan == null and place_return_plan == null and scalar_switch_return_plan == null and simple_trap == null and simple_assert == null)
        ...
        const statement_plan = if (local_aggregate_place_update_return_plan == null and local_aggregate_assignment_return_plan == null and simple_trap == null and simple_assert == null and simple_return == null and simple_void_body == null and simple_conditional_statement_return == null and simple_conditional_return == null and simple_enum_switch_return == null and simple_loop_return == null and indirect_call_return_plan == null and logical_return_plan == null)
            ...
            null;
        if (simple_trap == null and simple_assert == null and while_control_plan == null and sequence_foreach_return_plan == null and local_aggregate_place_update_return_plan == null and local_aggregate_assignment_return_plan == null and direct_call_projected_return_plan == null and simple_return == null and simple_void_body == null and simple_conditional_statement_return == null and simple_conditional_return == null and simple_enum_switch_return == null and simple_loop_return == null and place_return_plan == null and scalar_switch_return_plan == null and indirect_call_return_plan == null and logical_return_plan == null and statement_plan == null) return false;
            ...
            switch (body) {
                ...
                .loop_statements => |loop| {
                    ...
                    if (loop.body_returns) {
                        ...
                        try self.out.print(self.allocator, "  br i1 {s}, label %{s}, label %{s}{s}\n", .{ condition, body_target, after_target, try self.debugCallSuffix() });
                    }
                    try self.out.print(self.allocator, "{s}:\n", .{after_label});
                    try self.emitReturnVoid(sig_facts.name.span);
                },
        ...
        for (block.instructions) |instruction| {
            if (instruction.kind == .call) {
                ...
                result.statements[result.count] = .{ .global_store = .{
                    .name = name,
                    .value = self.simpleMirGlobalStoreValueAtSource(function, fn_mir, name, value_source) orelse return null,
    ...
    fn simpleMirPointerParamFieldStore(self: *LlvmEmitter, function: anytype, fn_mir: mir.Function, block: mir.Block, assign: mir.Instruction) ?SimpleMirParamFieldStore {
    ...
    fn simpleMirGlobalStoreValueAtSource(self: *LlvmEmitter, function: anytype, fn_mir: mir.Function, store_name: []const u8, value_source: mir.SourcePoint) ?SimpleMirGlobalStoreValue {
        const global_ty = self.global_types.get(store_name) orelse return null;
        const expected_type_name = type_bridge.typeName(self.resolveAliasType(global_ty));
        return if (self.simpleMirCheckedBinaryAtSource(function, fn_mir, value_source)) |binary|
            ...
            .{ .checked_unary = unary }
        else if (if (expected_type_name) |name| self.simpleMirWrappingBinaryAtSource(function, fn_mir, value_source, name) else null) |binary|
            .{ .wrapping_binary = binary }
        else if (if (expected_type_name) |name| self.simpleMirUncheckedBinaryAtSource(function, fn_mir, value_source, name, store_name) else null) |binary|
            .{ .wrapping_binary = binary }
    ...
    fn emitSimpleMirVoidStatements(self: *LlvmEmitter, statements: SimpleMirVoidStatements, default_span: diagnostics.Span) !diagnostics.Span {
        ...
        for (statements.statements[0..statements.count]) |statement| switch (statement) {
            ...
            .param_field_store => |store| {
                const span = spanFromMirSourcePoint(store.source);
                return_span = span;
                const struct_decl = self.struct_types.get(store.struct_name) orelse return error.UnsupportedLlvmEmission;
                if (store.field_index >= struct_decl.fields.len) return error.UnsupportedLlvmEmission;
                const field_ty = struct_decl.fields[store.field_index].ty;
                const field_ptr = try self.nextTemp();
                const struct_ty = type_bridge.simpleNameType(store.struct_name, span);
                try self.out.print(self.allocator, "  {s} = getelementptr {s}, ptr %{s}, i32 0, i32 {d}{s}\n", .{
                    field_ptr,
                    try self.llvmType(struct_ty),
                    store.param_name,
                    store.field_index,
                    try self.debugCallSuffix(),
                });
                try self.emitOrdinaryStore(field_ty, try self.llvmType(field_ty), try self.simpleMirArgValue(store.value, span), field_ptr, false);
            },
        };
        return return_span;
    }
    ...
    fn emitSimpleMirVoidStatementSources(self: *LlvmEmitter, function: anytype, fn_mir: mir.Function, statements: SimpleMirVoidStatementSources, default_span: diagnostics.Span) !diagnostics.Span {
        ...
        for (statements.sources[0..statements.count]) |statement| switch (statement) {
            ...
            .global_store => |store| {
                ...
                const global_ty = self.global_types.get(store.name) orelse return error.UnsupportedLlvmEmission;
                const ptr = try std.fmt.allocPrint(self.scratch.allocator(), "@{s}", .{store.name});
                const value = self.simpleMirGlobalStoreValueAtSource(function, fn_mir, store.name, store.value_source) orelse return error.UnsupportedLlvmEmission;
    ...
    fn simpleMirGlobalStoreValue(self: *LlvmEmitter, value: SimpleMirGlobalStoreValue, expected_ty: anytype, span: diagnostics.Span) ![]const u8 {
        return switch (value) {
            ...
            .wrapping_binary => |binary| try self.emitSimpleMirWrappingBinary(binary, span),
            .explicit_cast => |cast| try self.emitSimpleMirExplicitCastReturn(cast, span),
            .conversion => |conversion| try self.emitSimpleMirConversionReturn(conversion, span),
    ...
    fn simpleMirVoidStatementSourcesTrapCount(self: *LlvmEmitter, function: anytype, fn_mir: mir.Function, sources: SimpleMirVoidStatementSources) ?usize {
        ...
        for (sources.sources[0..sources.count]) |source| {
            count += switch (source) {
                ...
                .global_store => |store| simpleMirGlobalStoreValueTrapCount(self.simpleMirGlobalStoreValueAtSource(function, fn_mir, store.name, store.value_source) orelse return null),
            };
        }
        return count;
    }
