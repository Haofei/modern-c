...
pub fn buildOptFromResolvedDecls(allocator: std.mem.Allocator, resolved_decls: []const module_parser.ResolvedDecl, options: BuildOptions) !Module {
    return buildOptFromDeclItems(allocator, resolved_decls, options);
}

fn declFromBuildItem(item: anytype) ast.Decl {
...
fn buildOptFromDeclItems(allocator: std.mem.Allocator, decl_items: anytype, options: BuildOptions) !Module {
    ...
    try eval.collectConstGlobalsFromDeclItemsWithOptions(allocator, decl_items, &const_fns, &const_globals, .{
    ...
    var pointer_return_summaries = try collectDirectGlobalPointerReturnSummaries(allocator, decl_items, &globals, &enums, &structs, &packed_bits, &aliases);
    defer pointer_return_summaries.deinit();

    var aggregate_return_facts = try collectDirectAggregateReturnPointerFacts(allocator, decl_items, &globals, &enums, &structs, &packed_bits, &aliases, &pointer_return_summaries);
...
fn collectDirectGlobalPointerReturnSummaries(allocator: std.mem.Allocator, decl_items: anytype, globals: *const std.StringHashMap(ValueType), enums: *const std.StringHashMap(EnumSummary), structs: *const std.StringHashMap(StructSummary), packed_bits: *const std.StringHashMap(PackedBitsSummary), aliases: *const std.StringHashMap(ast.TypeExpr)) !std.StringHashMap(PointerReturnProvenanceSummary) {
...
const FunctionBuilder = struct {
    ...
    fn init(allocator: std.mem.Allocator, fn_decl: ast.FnDecl, attrs: []const ast.Attr, drop_glue_facts: []const DropGlueFact, type_ownership_facts: []const TypeOwnershipFact, summaries: *const std.StringHashMap(FunctionSummary), enums: *const std.StringHashMap(EnumSummary), structs: *const std.StringHashMap(StructSummary), unions: *const std.StringHashMap(UnionSummary), packed_bits: *const std.StringHashMap(PackedBitsSummary), aliases: *const std.StringHashMap(ast.TypeExpr), traits: *const std.StringHashMap(ast.TraitDecl), const_fns: *const std.StringHashMap(eval.ComptimeFunction), const_globals: *const std.StringHashMap(eval.ComptimeValue), globals: *const std.StringHashMap(ValueType), global_type_exprs: *const std.StringHashMap(ast.TypeExpr), mutable_globals: *const std.StringHashMap(void), pointer_return_summaries: *const std.StringHashMap(PointerReturnProvenanceSummary)) !FunctionBuilder {
        ...
        for (fn_decl.params) |param| {
            ...
            if (param_ty == .nullable_value and !try builder.internExecutableValueOptionalType(param_ty))
                builder.executable_supported = false;
            if (!try builder.internExecutableTypeExpr(param_ty, param.ty))
                builder.executable_supported = false;
    ...
    fn executableExpressionComplete(
        ...
        return switch (expression.operation) {
            ...
            .optional_some => |operand| optional: {
                ...
                var shape: ?mir_model.ExecutableAggregateType = null;
                for (self.executable_aggregate_types.items) |candidate| if (candidate.type_id.eql(expression.type_id)) {
                    shape = candidate;
                    break;
                };
                const optional_shape = shape orelse break :optional false;
                if (optional_shape.construction != .declared_struct or optional_shape.ty != .nullable_value or optional_shape.field_count != 2) break :optional false;
                const payload = self.executable_expressions.items[operand.index()];
                break :optional sameValueType(payload.result_ty, optional_shape.field_types[1]) and
                    payload.type_id.eql(optional_shape.field_type_ids[1]);
            },
            .optional_none => optional: {
                if (expression.result_ty != .nullable_value) break :optional false;
                for (self.executable_aggregate_types.items) |candidate| {
                    if (!candidate.type_id.eql(expression.type_id)) continue;
                    break :optional candidate.construction == .declared_struct and candidate.ty == .nullable_value and candidate.field_count == 2;
                }
                break :optional false;
            },
            ...
            .result => |operation| result: {
                ...
                const payload = self.executable_expressions.items[operation.payload.index()];
                for (self.executable_result_types.items) |shape| {
                    if (!shape.type_id.eql(expression.type_id)) continue;
                    if (!sameValueType(shape.ty, expression.result_ty)) break :result false;
                    break :result if (operation.is_ok)
    ...
    fn executableEnumRawComplete(
        ...
        const operand_id = call.arguments[0];
        if (!operand_id.isValid() or operand_id.index() >= self.executable_expressions.items.len) return false;
        const operand = self.executable_expressions.items[operand_id.index()];
    ...
    fn ensureExecutableExprAsInner(
        ...
        const operation: ExecutableExpression.Operation = switch (expr.kind) {
            ...
            .binary => |node| binary: {
                ...
                const left = if (mirIsComparisonBinary(node.op))
                ...
                try self.contextualizeExecutableLiteral(left, operand_ty);
                try self.contextualizeExecutableLiteral(right, operand_ty);
            ...
            .cast => |node| cast: {
                ...
                // of inventing a backend cast from `comptime_*`.
                switch (self.executable_expressions.items[operand.index()].operation) {
                    .literal => try self.contextualizeExecutableLiteral(operand, result_ty),
                    else => {},
                }
                const operand_ty = self.executable_expressions.items[operand.index()].result_ty;
            ...
            .call => |node| call: {
                ...
                if (try self.executableBuiltinCallKind(node)) |kind| {
                    ...
                    if (result_target_type) |target_type| {
                        ...
                        result_ty = target.result_ty;
                        if (!try self.internExecutableEnumType(target.source_ty))
                            break :call self.unsupportedExecutableExpression(.unsupported_call);
                    ...
                    const receiver, const receiver_ty = switch (kind) {
                        ...
                        .wrap_residue => .{
                            (memberExpr(node.callee.*) orelse
                                break :call self.unsupportedExecutableExpression(.unsupported_call)).base.*,
                            (domain_target orelse break :call self.unsupportedExecutableExpression(.unsupported_call)).domain_ty,
                        },
                        .enum_raw => .{
                            (memberExpr(node.callee.*) orelse
                                break :call self.unsupportedExecutableExpression(.unsupported_call)).base.*,
                            (enum_raw_target orelse break :call self.unsupportedExecutableExpression(.unsupported_call)).source_ty,
                        },
                        else => .{ null, null },
                    };
                    ...
                    if (receiver) |base| {
                        call_value.arguments[0] = try self.ensureExecutableExprAs(base, receiver_ty.?);
                        argument_index = 1;
                    }
                    for (node.args, 0..) |argument, source_index| {
                        call_value.arguments[argument_index] = if (kind == .result_ok or kind == .result_err) result_arg: {
                            const target_type = result_target_type orelse
                                break :call self.unsupportedExecutableExpression(.unsupported_call);
                            const payload_type = resultPayloadTypeExprAlias(target_type, if (kind == .result_ok) "ok" else "err", self.aliases) orelse
                                break :call self.unsupportedExecutableExpression(.unsupported_call);
                    ...
                    if (kind == .result_ok or kind == .result_err) {
                        if (argument_count != 1) break :call self.unsupportedExecutableExpression(.unsupported_call);
                        break :call .{ .result = .{
                            .is_ok = kind == .result_ok,
                            .payload = call_value.arguments[0],
                        } };
                    }
                ...
                if (directCalleeName(node.callee.*)) |callee_name| if (self.isKnownDirectCall(node.callee.*, callee_name)) {
                    ...
                    for (node.args, 0..) |argument, index| {
                        ...
                        if (summary) |callee_summary| if (index < callee_summary.params.len) {
                            if (!try self.internExecutableTypeExpr(parameter_ty orelse .unknown, callee_summary.params[index].ty))
                                break :call self.unsupportedExecutableExpression(.unsupported_call);
                        };
    ...
    fn executableCallableSignature(self: *FunctionBuilder, type_expr: ast.TypeExpr) !?mir_model.ExecutableCallSignature {
        ...
        for (function.params, 0..) |parameter, index| {
            ...
            signature.parameter_types[index] = parameter_ty;
            signature.parameter_type_ids[index] = try self.internTypeId(parameter_ty);
            if (!try self.internExecutableTypeExpr(parameter_ty, parameter)) {
                self.executable_supported = false;
                return null;
            }
        }
    ...
    fn internExecutableValueOptionalType(self: *FunctionBuilder, ty: ValueType) !bool {
        const payload_name = switch (ty) {
            .nullable_value => |name| name,
            else => return false,
        };
        const payload_ty = valueTypeFromTypeNameAlias(payload_name, self.enums, self.structs, self.packed_bits);
        if (payload_ty == .unknown or payload_ty == .value or payload_ty == .nullable_value) return false;
        const type_id = try self.internTypeId(ty);
        const payload_type_id = try self.internTypeId(payload_ty);
        const bool_type_id = try self.internTypeId(.bool);
        for (self.executable_aggregate_types.items) |aggregate| {
            if (!aggregate.type_id.eql(type_id)) continue;
            return sameValueType(aggregate.ty, ty) and aggregate.construction == .declared_struct and
                aggregate.field_count == 2 and sameValueType(aggregate.field_types[0], .bool) and
                sameValueType(aggregate.field_types[1], payload_ty) and
                aggregate.field_type_ids[0].eql(bool_type_id) and
                aggregate.field_type_ids[1].eql(payload_type_id);
        }
        var aggregate: mir_model.ExecutableAggregateType = .{
            .type_id = type_id,
            .ty = ty,
        ...
        aggregate.field_spellings[0] = "present";
        aggregate.field_spellings[1] = "value";
        aggregate.field_types[0] = .bool;
        aggregate.field_types[1] = payload_ty;
        aggregate.field_type_ids[0] = bool_type_id;
        aggregate.field_type_ids[1] = payload_type_id;
        try self.executable_aggregate_types.append(self.allocator, aggregate);
        if (!try self.internExecutableEnumType(payload_ty)) return false;
        switch (payload_ty) {
            .struct_ => |name| {
                const summary = self.structs.get(name) orelse return false;
                if (!try self.internExecutableAggregateTypeDepth(payload_ty, executableAggregateConstruction(summary), summary.fields, 0)) return false;
            },
            else => {},
        }
        return true;
    ...
    fn internExecutableResultType(self: *FunctionBuilder, ty: ValueType, type_expr: ast.TypeExpr) !bool {
        const generic = switch (aggregateTargetTypeAlias(type_expr, self.aliases).kind) {
            .generic => |value| value,
            else => return false,
        };
    ...
    /// is a qualification, so comparing `*mut T` with `*const T` uses the
    /// const-qualified shape. A nullable operand makes the common shape
    /// nullable. The operands are then materialized through explicit MIR casts
    /// instead of leaving either backend to reconcile source types.
    fn executableComparisonPointerType(left: ValueType, right: ValueType) ?ValueType {
        const left_shape = switch (left) {
            .pointer => |shape| shape,
            .nullable_pointer => |shape| shape,
            else => return null,
        };
        const right_shape = switch (right) {
            .pointer => |shape| shape,
            .nullable_pointer => |shape| shape,
            // `null` is contextual and has no useful standalone shape.
            .unknown, .value => return left,
            else => return null,
        };
        if (left_shape.kind != right_shape.kind or !std.mem.eql(u8, left_shape.child, right_shape.child)) return null;
        const common_shape: mir_model.PointerShape = .{
            .kind = left_shape.kind,
            .mutability = if (left_shape.mutability == .@"const" or right_shape.mutability == .@"const") .@"const" else left_shape.mutability,
            .child = left_shape.child,
        };
        return if (left == .nullable_pointer or right == .nullable_pointer)
            .{ .nullable_pointer = common_shape }
        else
            .{ .pointer = common_shape };
    }
    ...
    fn executableBuiltinCallKind(self: *FunctionBuilder, call: anytype) !?CallTargetKind {
        if (resultConstructorCallTag(call)) |tag|
            return if (std.mem.eql(u8, tag, "ok")) .result_ok else .result_err;
        ...
        switch (expression.operation) {
            .literal => |literal| switch (literal) {
                    ...
                    std.meta.activeTag(target_ty) == .closed_enum or std.meta.activeTag(target_ty) == .open_enum)
                {
                    ...
                    expression.type_id = try self.internTypeId(target_ty);
                },
                .float => {
                    if (std.meta.activeTag(target_ty) == .float) {
                        expression.result_ty = target_ty;
                        expression.type_id = try self.internTypeId(target_ty);
                    }
                },
                .boolean => {
                    if (std.meta.activeTag(target_ty) == .bool) {
                        expression.result_ty = target_ty;
                        expression.type_id = try self.internTypeId(target_ty);
                    }
                },
    ...
    fn buildContractBlock(self: *FunctionBuilder, contract: ast.ContractBlock, stmt_span: ast.Span) !bool {
        ...
        if (!terminated) {
            ...
            // statement on which to place an end marker. Cleanup/control flow
            // is already explicit in the enclosing CFG.
        }
        return terminated;
    }
        ...
        while (index > 0) {
            ...
            switch (expression.operation) {
                ...
                .address_of => |address| address_bounds: {
                    ...
                    const place = self.executable_places.items[address.place.index()];
                    if (place.projection_count != 1) break :address_bounds;
                    const projection = switch (place.projections[0]) {
                        .index => |value| value,
    ...
    fn addDiscardOwnershipEvent(self: *FunctionBuilder, target: CallTargetKind, argument: ast.Expr, call_span: ast.Span) !void {
        ...
        const root_type_symbol_id = switch (target) {
            .drop => blk: {
                const drop_glue_identity = self.discardArgumentDropGlueIdentity(argument) orelse return;
                break :blk drop_glue_identity.resource_symbol_id;
            },
            .forget_unchecked => self.localRootTypeSymbol(root),
            else => return,
        };
    ...
    fn appendForElementTargetTypeFact(self: *FunctionBuilder, target_ty: ast.TypeExpr, result_ty: ValueType, span: ast.Span, binding_name: []const u8) !void {
        try self.appendTargetTypeFact(.for_element, target_ty, result_ty, span);
        const binding_id = try self.internValueId(binding_name);
        const instructions = &self.blocks.items[self.current].instructions;
...
fn inferredLocalTypeFactEligible(builder: *FunctionBuilder, maybe_initializer: ?ast.Expr) bool {
    ...
    return switch (initializer.kind) {
        ...
        .binary => |node| mirIsArithmeticBinary(node.op) or mirIsBitwiseBinary(node.op) or mirIsComparisonBinary(node.op) or mirIsLogicalBinary(node.op),
