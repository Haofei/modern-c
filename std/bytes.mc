...
pub fn byte_reader(base: PAddr, len: usize) -> ByteReader {
    return .{ .base = base, .len = len };
}

pub fn br_len(r: *ByteReader) -> usize {
    return r.len;
}

// Are there at least `n` bytes available starting at `off` (no overflow)?
pub fn br_has(r: *ByteReader, off: usize, n: usize) -> bool {
    if off > r.len {
        return false;
    }
...
pub fn br_u8(r: *ByteReader, off: usize) -> u8 {
    br_check(r, off, 1);
    unsafe {
...
pub fn br_le16(r: *ByteReader, off: usize) -> u16 {
    let b0: u16 = br_u8(r, off) as u16;
    let b1: u16 = br_u8(r, off + 1) as u16;
    return b0 | (b1 << 8);
}

pub fn br_le32(r: *ByteReader, off: usize) -> u32 {
...
pub fn br_le64(r: *ByteReader, off: usize) -> u64 {
    let lo: u64 = br_le32(r, off) as u64;
    let hi: u64 = br_le32(r, off + 4) as u64;
    return lo | (hi << 32);
}

pub fn br_be16(r: *ByteReader, off: usize) -> u16 {
    let b0: u16 = br_u8(r, off) as u16;
    let b1: u16 = br_u8(r, off + 1) as u16;
    return (b0 << 8) | b1;
}

pub fn br_be32(r: *ByteReader, off: usize) -> u32 {
    let b0: u32 = br_u8(r, off) as u32;
    let b1: u32 = br_u8(r, off + 1) as u32;
    let b2: u32 = br_u8(r, off + 2) as u32;
    let b3: u32 = br_u8(r, off + 3) as u32;
...
pub fn br_try_u8(r: *ByteReader, off: usize) -> Result<u8, BytesError> {
    if !br_has(r, off, 1) {
...
pub fn br_try_be16(r: *ByteReader, off: usize) -> Result<u16, BytesError> {
    if !br_has(r, off, 2) {
        return err(.OutOfBounds);
    }
...
pub fn br_try_be32(r: *ByteReader, off: usize) -> Result<u32, BytesError> {
    if !br_has(r, off, 4) {
        return err(.OutOfBounds);
    }
...
pub fn br_try_le16(r: *ByteReader, off: usize) -> Result<u16, BytesError> {
    if !br_has(r, off, 2) {
        return err(.OutOfBounds);
    }
    let b0: u16 = br_load_u8(r, off) as u16;
...
pub fn br_try_le32(r: *ByteReader, off: usize) -> Result<u32, BytesError> {
    if !br_has(r, off, 4) {
        return err(.OutOfBounds);
    }
    let b0: u32 = br_load_u8(r, off) as u32;
...
pub fn br_try_le64(r: *ByteReader, off: usize) -> Result<u64, BytesError> {
    if !br_has(r, off, 8) {
        return err(.OutOfBounds);
    }
    var lo: u32 = 0;
...
pub fn br_validate_len(r: *ByteReader, off: usize, claimed: usize) -> Result<usize, BytesError> {
...
pub fn byte_writer(base: PAddr, len: usize) -> ByteWriter {
    return .{ .base = base, .len = len };
}
...
pub fn bw_u8(w: *ByteWriter, off: usize, value: u8) -> void {
    bw_check(w, off, 1);
    unsafe {
...
pub fn bw_be16(w: *ByteWriter, off: usize, value: u16) -> void {
...
pub fn bw_be32(w: *ByteWriter, off: usize, value: u32) -> void {
