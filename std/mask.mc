...
pub fn mask32_zero() -> Mask32 {
    return .{ .bits = 0 };
}
pub fn mask32_from(bits: u32) -> Mask32 {
    return .{ .bits = bits };
}
pub fn mask32_raw(m: *mut Mask32) -> u32 {
    return m.bits;
}

// Set / clear / test bit `b` (0..31). An out-of-range bit is ignored.
pub fn mask32_set(m: *mut Mask32, b: u32) -> void {
    if b < 32 {
        m.bits = m.bits | wrapping_shl_u32(1, b);
    }
}
#[irq_context]
pub fn mask32_clear(m: *mut Mask32, b: u32) -> void {
    if b < 32 {
        m.bits = m.bits & (~wrapping_shl_u32(1, b));
    }
}
pub fn mask32_contains(m: *mut Mask32, b: u32) -> bool {
    if b >= 32 {
        return false;
    }
...
pub fn mask32_is_empty(m: *mut Mask32) -> bool {
    return m.bits == 0;
}

// Remove and return the lowest set bit's index, or 32 if the set is empty.
pub fn mask32_take_first(m: *mut Mask32) -> u32 {
    var b: u32 = 0;
    while b < 32 {
