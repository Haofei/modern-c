...
pub fn alloc(len: usize) -> CpuBuffer {
...
pub fn clean_for_device(b: CpuBuffer) -> DeviceBuffer {
    let dev: DmaAddr = b.dev_addr;
    let cpu: PAddr = b.cpu_addr;
    let n: usize = b.len;
...
pub fn invalidate_for_cpu(b: DeviceBuffer) -> CpuBuffer {
    let dev: DmaAddr = b.dev_addr;
    let n: usize = b.len;
...
pub fn device_addr(b: *DeviceBuffer) -> DmaAddr {
    return b.dev_addr;
}
...
pub fn cpu_addr(b: *CpuBuffer) -> PAddr {
    return b.cpu_addr;
}

pub fn cpu_len(b: *CpuBuffer) -> usize {
    return b.len;
}
...
pub fn write_u8(b: *CpuBuffer, offset: usize, value: u8) -> void {
    if offset >= b.len {
        unreachable; // out of bounds
    }
    unsafe {
...
pub fn read_u8(b: *CpuBuffer, offset: usize) -> u8 {
    if offset >= b.len {
        unreachable; // out of bounds
    }
    unsafe {
...
pub fn write_be16(b: *CpuBuffer, offset: usize, value: u16) -> void {
...
pub fn read_be16(b: *CpuBuffer, offset: usize) -> u16 {
    let hi: u16 = read_u8(b, offset) as u16;
    let lo: u16 = read_u8(b, offset + 1) as u16;
    return (hi << 8) | lo;
}

pub fn write_be32(b: *CpuBuffer, offset: usize, value: u32) -> void {
...
pub fn read_be32(b: *CpuBuffer, offset: usize) -> u32 {
...
pub fn write_le32(b: *CpuBuffer, offset: usize, value: u32) -> void {
    cpu_check(b, offset, 4); // full-width check up front: no partial store before a trap
...
pub fn write_le64(b: *CpuBuffer, offset: usize, value: u64) -> void {
...
pub fn read_le32(b: *CpuBuffer, offset: usize) -> u32 {
    let b0: u32 = read_u8(b, offset) as u32;
    let b1: u32 = read_u8(b, offset + 1) as u32;
    let b2: u32 = read_u8(b, offset + 2) as u32;
    let b3: u32 = read_u8(b, offset + 3) as u32;
